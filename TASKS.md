...
Create the next deterministic adversarial first-wave **authority / ambiguous external-action replay** fixture. Prove exact-action authority cannot be reused for a materially different command/target, an ambiguous or interrupted external effect is not automatically replayed, and duplicate/reordered completion cannot execute the side effect twice. Compare exact current Hi `42c1cd6` and pre-reset Hi `e8c1a7d`; measure ambiguous_side_effect_replay_count, duplicate execution acceptance, authority hash/attempt identity, restart behavior, and completion blocking. Do not perform a real external action; use deterministic host/tool failure injection.
