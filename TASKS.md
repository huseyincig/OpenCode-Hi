...
Create the next pinned real-host first-wave fixture for **two independent same-model implementation work units** with deterministic acceptance and disjoint mutable surfaces. Run the same OpenCode 1.18.18 + `opencode-go/deepseek-v4-flash` variant `low` protocol through vanilla OpenCode, reproducible pre-reset Hi `e8c1a7d`, and current Hi, minimum 3 repetitions each if the pilot is mechanically stable. Measure verified success, workers/parallelism, model/tool calls, exact step-finish usage, wall time, context transferred, duplicate work and bounded diff. This scenario exists to test whether WorkGraph/scheduler topology provides measurable value where parallelism is actually justified; do not reward extra workers by itself. Keep all receipts isolated under `/workspace/Reference/benchmarks/` and continue to label OpenCode monetary cost as derived, not provider-billed.
