...
Create the next deterministic adversarial first-wave **restart / stale callback / duplicate-dispatch** fixture. Simulate persistence restore with an in-flight child and prove old-session callbacks cannot be accepted as current work, restart reconciliation preserves the same task/worker identity, scheduler ownership is reconciled before resume, and repeated resume/late callbacks cannot execute the same attempt twice. Compare exact current Hi `42c1cd6` and pre-reset Hi `e8c1a7d`; measure stale_callback_accept_count, duplicate_dispatch_count, attempt/generation identity, child/session reuse or replacement, scheduler reservations, and completion blocking.
