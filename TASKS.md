...
Create the next first-wave **context-heavy bounded investigation** fixture. Exercise a large but deterministic repository/context surface where only a small subset is relevant. Measure how much context is projected/transferred, whether iterative retrieval expands only after a concrete information gap, whether unrelated artifacts/transcripts are excluded, and whether the correct target/evidence is found without unbounded delegation or repeated reads. Compare exact current Hi `42c1cd6`, pre-reset Hi `e8c1a7d`, and vanilla only where external host behavior is materially comparable. Prefer deterministic context-projection/retrieval measurement first; add real-host repetitions only if model behavior is necessary to answer the product question.
