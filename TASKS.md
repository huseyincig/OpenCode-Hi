...
Create the next deterministic adversarial first-wave **provider/child failure and recovery** fixture. Exercise a child/provider failure after dispatch and prove bounded recovery semantics: classify failure, preserve task/attempt identity, avoid duplicate execution, release/reconcile scheduler state correctly, and either resume/escalate with materially differentiated execution or settle a precise terminal blocker. Compare exact current Hi `42c1cd6` and pre-reset Hi `e8c1a7d`; use a real provider only if in-process host-port failure injection cannot mechanically answer the recovery contract. Measure recovery correctness, retries/replans, duplicate dispatch, stale callbacks, orphan/cleanup state, and completion blocking.
