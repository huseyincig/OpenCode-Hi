...
Create the next pinned real-host first-wave **dependency/fan-in scheduler** fixture. Use two upstream work units whose outputs are both required by a downstream integration/verification unit, with explicit dependency edges and deterministic acceptance proving the fan-in unit cannot dispatch early, cannot consume stale/partial sibling output, and completes after both prerequisites settle. Compare vanilla OpenCode 1.18.18, reproducible pre-reset Hi `e8c1a7d`, and exact current Hi `42c1cd6` with the same `opencode-go/deepseek-v4-flash` variant `low` protocol, minimum 3 repetitions each if mechanically stable. Preserve exact step-finish usage, task/dependency state, dispatch timing, workers/peak concurrency, wall time, context transfer and bounded diff; label monetary cost as OpenCode-derived, not provider-billed. Retain dependency/fan-in scheduling only if correctness/predictability or measured efficiency is demonstrated.
