...
Create the next deterministic adversarial first-wave **mutation-after-verification freshness** fixture. Prove that a relevant post-verification mutation invalidates the affected claim and blocks completion until fresh evidence is obtained; an unknown mutation surface fails closed; and an unrelated known-surface mutation does not unnecessarily invalidate an independent claim when exact scope is available. Compare exact current Hi `42c1cd6` and pre-reset Hi `e8c1a7d` mechanically, including verification-obligation/gate reopening and evidence invalidation state. Use real-host repetitions only if deterministic runtime evidence leaves a material unanswered question.
