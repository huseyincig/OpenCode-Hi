...
Create the final deterministic first-wave **plugin/config coexistence preservation** fixture. Seed foreign/host config fields, explicit user agent/tool choices, unknown-but-valid host/plugin sections, and foreign transform output/markers. Apply Hi config/setup/transform paths and prove Hi is additive or scoped: it must not overwrite foreign settings, claim `default_agent`, erase unknown sections, suppress explicit user tool choices, or duplicate its own projection on repeated hooks. Compare exact current Hi `42c1cd6` and pre-reset Hi `e8c1a7d`; record any reset-specific regression/improvement mechanically.
