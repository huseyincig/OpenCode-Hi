...
- cutover verification is green: plugin build PASS, focused composition/coexistence 24/24 PASS, affected runtime/tool-surface set 45/45 PASS, architecture lint 22/22 PASS, full plugin suite 906/906 PASS.

## Exact Next Action

Checkpoint the coexistence-safe primary tool-visibility cutover without staging unrelated user-owned validation/release/script files. Then rerun the exact same trivial-localized-work fixture 3 times using the checkpointed current Hi with no benchmark-only tool overrides, the same OpenCode 1.18.18 + `opencode-go/deepseek-v4-flash` variant `low`, and the same receipt/acceptance protocol. Compare against the already pinned 3x vanilla, pre-reset and pre-cutover current receipts. Keep the cutover only if correctness remains 3/3 and repeated execution evidence demonstrates a useful reduction without removing required control-plane behavior; otherwise revert/simplify only this cutover. Do not present OpenCode-derived monetary values as provider-billed cost.
