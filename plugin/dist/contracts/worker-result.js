...
export function normalizeWorkerResult(raw) {
    ...
    if (malformedFindings && !open.includes('review-finding-contract-invalid'))
        open.unshift('review-finding-contract-invalid');
    const needs = Array.isArray(x.needs_context) ? x.needs_context.map(boundedIssueText).slice(0, 30) : [];
    if (malformedFindings && !needs.some(item => item.startsWith('review-finding-contract-retry:')))
        needs.unshift('review-finding-contract-retry: return each finding with canonical rf-* id, reviewer_role, bounded project scope, and evidence_refs that name evidence.kind values from the same WorkerResult; opaque ev_* ids belong on the evidence claim, not the finding');
