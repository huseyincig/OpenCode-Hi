...
export function assertRecord(value, field) {
    if (!isRecord(value))
        throw new ContractValidationError(field, 'must be a plain object');
    return value;
}
export function assertStrictKeys(value, allowed, required, field) {
    const allow = new Set(allowed);
    for (const key of Object.keys(value))
        if (!allow.has(key))
            throw new ContractValidationError(`${field}.${key}`, 'unknown field');
    for (const key of required)
        if (!(key in value))
            throw new ContractValidationError(`${field}.${key}`, 'required field missing');
}
...
export function assertCanonicalId(value, field = 'id') {
    const id = assertNonEmptyString(value, field);
    if (!CANONICAL_ID.test(id))
        throw new ContractValidationError(field, 'must use canonical lowercase technical ID syntax');
    return id;
}
export function assertPositiveInteger(value, field) {
    if (!Number.isInteger(value) || Number(value) < 1)
        throw new ContractValidationError(field, 'must be a positive integer');
    return Number(value);
}
...
export function assertContentHash(value, field) {
    ...
    if (record.algorithm !== 'sha256')
        throw new ContractValidationError(`${field}.algorithm`, 'must be sha256');
    if (typeof record.value !== 'string' || !/^[a-f0-9]{64}$/.test(record.value))
        throw new ContractValidationError(`${field}.value`, 'must be 64 lowercase hexadecimal SHA-256 characters');
...
function normalizeCanonical(value, path) {
    ...
    if (typeof value === 'number') {
        if (!Number.isFinite(value))
            throw new ContractValidationError(path, 'non-finite numbers are not canonical');
        return value;
    }
    ...
    if (isRecord(value)) {
        ...
        for (const key of Object.keys(value).sort()) {
            ...
            if (item === undefined)
                throw new ContractValidationError(`${path}.${key}`, 'undefined is not canonical');
