...
export function auditHiToolNamespace(names) {
...
export function assertHiToolNamespace(names) { const a = auditHiToolNamespace(names); if (!a.ok)
    throw new Error(`Hi tool namespace violation: collisions=${a.collisions.join(',') || 'none'}; nonNamespaced=${a.nonNamespaced.join(',') || 'none'}`); }
