 ...
 */
export function projectBuiltinPrimaryHiToolVisibility(config) {
    const existingAgents = record(config.agent), agents = existingAgents ?? {}, targets = [], defaultHidden = [], explicitPreserved = [], diagnostics = [];
    for (const name of ['build', 'plan']) {
        const current = agents[name];
        if (current !== undefined && !record(current)) {
            diagnostics.push(`primary-tool-visibility-skipped:${name}:agent-shape`);
            continue;
        }
        const agent = record(current) ?? {};
        const existingTools = agent.tools;
        if (existingTools !== undefined && !record(existingTools)) {
            diagnostics.push(`primary-tool-visibility-skipped:${name}:tools-shape`);
            continue;
        }
        ...
        for (const id of HI_PRIMARY_HIDDEN_TOOL_IDS) {
            if (Object.prototype.hasOwnProperty.call(tools, id)) {
                explicitPreserved.push(`${name}:${id}`);
                continue;
            }
            tools[id] = false;
            defaultHidden.push(`${name}:${id}`);
        }
        agent.tools = tools;
        agents[name] = agent;
        targets.push(name);
    }
    if (!existingAgents && targets.length)
        config.agent = agents;
