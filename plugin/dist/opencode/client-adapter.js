...
function injectedClientTransport(client) {
    const transport = client?._client, config = typeof transport?.getConfig === 'function' ? transport.getConfig() : undefined;
...
async function listMessagesV2(client, sessionID, limit, endpoint) {
...
export async function listMessages(client, sessionID, limit = 20, endpoint = {}) {
    const edge = client, inherited = injectedClientTransport(client), hasCurrentTransport = Boolean(endpoint.serverUrl || inherited.baseUrl);
    if (typeof edge?.session?.messages !== 'function')
        return hasCurrentTransport ? listMessagesV2(client, sessionID, limit, endpoint) : [];
    const response = await edge.session.messages({ path: { id: sessionID }, query: { limit } });
    const error = messageReadError(response);
    if (error) {
        if (hasCurrentTransport && structuredMessageCompatibilityError(error))
            return listMessagesV2(client, sessionID, limit, endpoint);
        throw error;
    }
    return normalizeMessageList(response);
}
