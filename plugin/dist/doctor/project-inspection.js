...
function stripJsonc(text) {
    ...
    while (i < text.length) {
        ...
        if (c === '/' && text[i + 1] === '*') {
            i += 2;
            while (i + 1 < text.length && text.slice(i, i + 2) !== '*/')
                i++;
            i += 2;
            continue;
        }
...
function readJson(path) { try {
    ...
    return JSON.parse(path.endsWith('.jsonc') ? stripJsonc(text) : text);
}
catch {
    return undefined;
} }
function childDirsWith(root, file) { try {
    return readdirSync(root).filter((name) => existsSync(join(root, name, file))).sort();
}
catch {
    return [];
} }
function sha256(path) { try {
    return createHash('sha256').update(readFileSync(path)).digest('hex');
}
catch {
    return undefined;
} }
function localPluginFiles(root) { try {
    return readdirSync(root).filter((name) => /\.(?:js|mjs|cjs|ts)$/i.test(name)).map((name) => join(root, name)).sort();
}
catch {
    return [];
} }
...
export function inspectProject(directory) {
    ...
    try {
        for (const name of readdirSync(join(directory, '.opencode', 'agents')))
            if (name.endsWith('.md'))
                agentDefinitions.push(name.slice(0, -3));
    }
    catch { }
    ...
    if (configPath && item?.after_sha256) {
        ...
        if (actual)
            configDrift = actual !== item.after_sha256;
    }
    if (configPath && !config)
        warnings.push('OpenCode config exists but could not be parsed');
    if (ownership && typeof ownership.schema !== 'number')
        warnings.push('Ownership state has no numeric schema');
    if (ownership && ownershipSchemaValid === false)
