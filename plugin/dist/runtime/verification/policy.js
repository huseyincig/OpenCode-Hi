...
function minimumVerificationKinds(intent) {
...
export function verificationPolicyFor(intent) { const independentReview = intent.risk === 'high' || intent.requiredCapabilities.includes('independent-review') || intent.requiredCapabilities.includes('security-review'); return { requiredKinds: minimumVerificationKinds(intent), requireFresh: true, requireReview: independentReview, allowWorkerReportedEvidence: false }; }
function effectiveVerificationKinds(m, obligation) {
    const obligationKinds = [...new Set((obligation?.requiredEvidence ?? []).map(canonical))];
    if (obligationKinds.length)
        return obligationKinds;
    const policyKinds = [...new Set(m.execution.verification_policy.requiredKinds.map(canonical))];
    if (policyKinds.length)
        return policyKinds;
    if (obligation?.kind === 'verification' || m.execution.obligations.some(o => o.kind === 'verification'))
        return minimumVerificationKinds(m.identity.intent);
    return [];
}
...
export function verificationKindAdmittedForMission(m, actual) { const required = effectiveVerificationKinds(m); return required.length === 0 || required.some(kind => verificationKindSatisfiesRequirement(kind, actual)); }
...
export function latestBlockingVerificationEvidence(m, obligationID) { const obligation = obligationID ? m.execution.obligations.find(o => o.id === obligationID) : undefined, requiredKinds = effectiveVerificationKinds(m, obligation), current = [...m.execution.evidence.items].filter(e => !e.invalidated_at && evidenceAllowedForVerification(m, e, obligationID)).sort((a, b) => b.observed_at - a.observed_at); for (const e of current) {
    ...
    if (!matched.length)
        continue;
    const superseded = matched.every(r => current.some(candidate => candidate.observed_at > e.observed_at && evidenceVerdictPassed(candidate.pass, candidate.outcome) && verificationKindSatisfiesRequirement(r, candidate.kind)));
    if (!superseded)
        return e;
} return undefined; }
