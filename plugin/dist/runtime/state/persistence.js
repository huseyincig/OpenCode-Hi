...
export const RUNTIME_STATE_SCHEMA = 8;
...
function validMissionTrajectory(identity, execution) {
...
function validMission(value) {
    ...
    if (!isRecord(identity) || !isRecord(execution) || !isRecord(continuation) || !isRecord(context) || !isRecord(vcs) || !isRecord(authority) || !isRecord(release) || !isRecord(methodology))
        return false;
    if (typeof identity.mission_id !== 'string' || typeof identity.session_id !== 'string' || typeof identity.objective !== 'string')
        return false;
    ...
    if (!['low', 'medium', 'high', 'authority-boundary'].includes(String(identity.risk)))
        return false;
    if (typeof identity.created_at !== 'number' || typeof identity.updated_at !== 'number')
        return false;
    if (!validVerificationPolicy(execution.verification_policy))
        return false;
    if (identity.semantic_assessment.status === 'assessed' && identity.intent.taskKind === 'unclassified')
        return false;
    if (identity.semantic_assessment.status === 'pending' && identity.semantic_assessment.phase === 'initial' && ((execution.obligations?.length ?? 0) > 0 || (execution.tasks?.length ?? 0) > 0 || (execution.workers?.length ?? 0) > 0 || (methodology.methodology_needs?.length ?? 0) > 0))
        return false;
    if ((!Array.isArray(execution.obligations) || !execution.obligations.every(validObligation)) || !Array.isArray(execution.tasks) || !execution.tasks.every(isTaskContract) || !Array.isArray(execution.workers) || !execution.workers.every(isWorkerContract) || !recordArray(execution.ledger))
        return false;
    if (!validMissionTrajectory(identity, execution))
        return false;
    if (!stringArray(execution.blockers) || !stringArray(execution.constraints) || typeof execution.native_todos_incomplete !== 'number' || !Array.isArray(execution.gates) || !execution.gates.every(validGate))
        return false;
    if (!isRecord(execution.evidence) || typeof execution.evidence.fresh !== 'boolean' || !Array.isArray(execution.evidence.items) || !execution.evidence.items.every(isEvidenceItemContract) || (execution.evidence.last_mutation_at !== undefined && typeof execution.evidence.last_mutation_at !== 'number'))
        return false;
    if (typeof continuation.generation !== 'number' || typeof continuation.iteration !== 'number' || typeof continuation.continuation_budget !== 'number' || typeof continuation.continuation_active !== 'boolean')
        return false;
    ...
    for (const field of ['continuation_reason', 'last_action_id', 'active_action_id', 'interrupted_reason'])
        if (continuation[field] !== undefined && typeof continuation[field] !== 'string')
            return false;
    if (continuation.pending_nudge !== undefined && !isRecord(continuation.pending_nudge))
        return false;
    if (!Array.isArray(context.context_artifacts) || !context.context_artifacts.every(validContextArtifact))
        return false;
    if (!stringArray(vcs.changed_files) || !Array.isArray(vcs.temporary_mutations) || !vcs.temporary_mutations.every(validTemporaryMutation))
        return false;
    ...
    if (vcs.preexisting_user_baseline_captured !== undefined && typeof vcs.preexisting_user_baseline_captured !== 'boolean')
        return false;
    if (vcs.git_topology_owned_files !== undefined && !stringArray(vcs.git_topology_owned_files))
        return false;
    if (typeof authority.pending_permissions !== 'number' || !stringArray(authority.pending_permission_ids))
        return false;
    ...
    if (authority.authority !== undefined && !isAuthorityStateContract(authority.authority))
        return false;
    if (authority.applied_actions !== undefined && !isRecord(authority.applied_actions))
        return false;
    if (release.release_chain !== undefined && !isRecord(release.release_chain))
        return false;
    if (!Array.isArray(methodology.methodology_needs) || !methodology.methodology_needs.every(validMethodologyNeed) || !stringArray(methodology.parent_loaded_methodologies))
        return false;
    return true;
}
