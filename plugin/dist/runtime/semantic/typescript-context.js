...
function extractTypeScript(source, names = [], maxChars = 5000) {
...
export class TypeScriptSemanticContextAdapter {
    languageIds() { return ['typescript', 'typescriptreact']; }
    supports(file) { return /\.tsx?$/i.test(file); }
    extract(input) { return extractTypeScript(input.source, input.names ?? [], input.maxChars); }
}
export const TYPE_SCRIPT_SEMANTIC_CONTEXT_ADAPTER = new TypeScriptSemanticContextAdapter();
export const SEMANTIC_CONTEXT_ADAPTERS = [TYPE_SCRIPT_SEMANTIC_CONTEXT_ADAPTER];
export function extractTypeScriptSemanticContext(source, names = [], maxChars = 5000) { return TYPE_SCRIPT_SEMANTIC_CONTEXT_ADAPTER.extract({ source, file: 'inline.ts', names, maxChars }); }
export function semanticContextsForTargets(projectRoot, targets, consumerTaskRef, maxChars = 3000, adapters = SEMANTIC_CONTEXT_ADAPTERS) {
    const root = resolve(projectRoot), out = [];
    let used = 0;
    for (const target of [...new Set(targets)].slice(0, 6)) {
        const adapter = adapters.find(candidate => candidate.supports(target));
        if (!adapter)
            continue;
        ...
        try {
            ...
            if (left < 128)
                break;
            const maxText = Math.min(1400, Math.max(0, left - 96)), r = adapter.extract({ source, file: target, names: [], maxChars: maxText });
            if (!r.text)
                continue;
            const languageIds = adapter.languageIds();
            if (!languageIds.includes('typescript') && !languageIds.includes('typescriptreact'))
                continue;
...
export function typescriptSemanticContextsForTargets(projectRoot, targets, consumerTaskRef, maxChars = 3000) { return semanticContextsForTargets(projectRoot, targets, consumerTaskRef, maxChars, [TYPE_SCRIPT_SEMANTIC_CONTEXT_ADAPTER]); }
