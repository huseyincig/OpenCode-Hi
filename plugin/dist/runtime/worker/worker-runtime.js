...
export function createTask(m, input) { const now = Date.now(), id = uid('t'), task = { id, mission_id: m.identity.mission_id, objective: input.objective, status: 'created', role: input.role, category: input.category, scope: input.scope ?? [], constraints: input.constraints ?? [], dependencies: input.dependencies ?? [], requiredEvidence: input.requiredEvidence ?? [], ...(input.requiredEvidenceOrigin ? { requiredEvidenceOrigin: input.requiredEvidenceOrigin } : {}), ...(input.verificationCases?.length ? { verification_cases: input.verificationCases.map(c => ({ ...c, required_browser_actions: [...c.required_browser_actions], ...(c.source_units?.length ? { source_units: [...c.source_units] } : {}) })) } : {}), obligation_ids: [...new Set(input.obligationIds ?? [])], context_artifacts: (input.contextReferences ?? []).map(ref => bindContextReference(ref, id)), execution_profile: input.executionProfile, gate_ids: [], external_action_requirements: [...m.identity.intent.requestedExternalActions], created_at: now, updated_at: now }; m.execution.tasks.push(task); appendLedger(m, 'task.created', { task_id: task.id, payload: { role: task.role, category: task.category, dependencies: task.dependencies, obligation_ids: task.obligation_ids, required_evidence_origin: task.requiredEvidenceOrigin } }); return task; }
...
export function isResourceOnlyProcessSupportTask(task) { return Boolean(task?.execution_profile?.process_lifecycle === true && !task.requiredEvidence.length && !task.obligation_ids.length); }
...
export function beginWorkerAttempt(task, worker, at = Date.now()) { worker.attempt = (worker.attempt ?? 0) + 1; worker.started_at = at; delete worker.last_result_digest; delete worker.last_result_at; delete worker.pending_native_permission_denial; delete worker.pending_host_assistant_result; delete worker.pending_text_transport_recovery; worker.attempt_prompt_message_id = workerAttemptPromptMessageID(worker, at); worker.updated_at = at; task.updated_at = at; }
...
} m.vcs.changed_files = [...new Set([...m.vcs.changed_files, ...result.changed_files])]; retireTaskResultIssues(m, task.id, supersededIssues, result.open_issues); if (result.status !== 'DONE' && !isResourceOnlyProcessSupportTask(task))
