...
function responseFor(decision, value) {
    ...
    if (kind === 'choice') {
        ...
        if (selected.length !== 1 || !decision.response_schema.choices?.includes(selected[0]))
            return undefined;
    ...
    if (kind === 'authority-protocol') {
        ...
        if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed))
            return undefined;
        const obj = parsed, keys = Object.keys(obj).sort();
        if (keys.join(',') !== 'authority_ref,decision_id,response' || obj.decision_id !== decision.decision_id || obj.authority_ref !== decision.authority_ref)
            return undefined;
        ...
        if (typeof obj.response !== 'string' || !allowed.includes(obj.response))
            return undefined;
...
export class ChatHumanDecisionTransport {
    ...
    open(decision) {
        ...
        if (existing && existing.handle.state === 'OPEN') {
            existing.decision = structuredClone(decision);
    ...
    async await(decisionId) {
        ...
        if (!entry)
            return { status: 'CANCELLED', decision_id: decisionId };
        if (entry.response)
            return { status: 'RESPONDED', response: structuredClone(entry.response) };
        if (entry.handle.state === 'CANCELLED')
            return { status: 'CANCELLED', decision_id: decisionId };
        return new Promise(resolve => {
            let timer;
            const done = (result) => { if (timer)
                clearTimeout(timer); entry.waiters.delete(done); resolve(result); };
    ...
    cancel(decisionId) {
        const entry = this.#entries.get(decisionId);
        if (!entry || entry.handle.state !== 'OPEN')
            return;
    ...
    respond(decisionId, value) {
        const entry = this.#entries.get(decisionId);
        if (!entry || entry.handle.state !== 'OPEN' || entry.decision.status !== 'OPEN')
            return undefined;
        const response = responseFor(entry.decision, value);
        if (!response)
            return undefined;
        entry.response = response;
        entry.handle = { ...entry.handle, state: 'RESPONDED' };
        for (const waiter of [...entry.waiters])
