...
function planForLevel(level) {
    ...
    if (level === 4)
        return { level: 4, action: 'alternate-plan', prompt: 'Re-plan only the blocked obligation using a materially different bounded strategy. Preserve completed tasks, current evidence, and the same mission contract.' };
    if (level === 5)
        return { level: 5, action: 'fresh-worker', prompt: 'Current context/strategy is exhausted. Use one bounded fresh worker if justified, carrying only the blocker, completed-work summary, affected scope, and evidence.' };
