...
export function continuationPrompt(m, action) { const open = m.obligations.filter(o => o.status === 'open').map(o => o.summary).slice(0, 3); return ['Hi runtime: mission is still active.', `Open obligation: ${open.join(' | ') || 'none'}.`, action, 'Resume from current state. Do not restart planning. Do not create duplicate tasks.'].join('\n'); }
