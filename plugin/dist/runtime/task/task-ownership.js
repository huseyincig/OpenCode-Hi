...
export function taskOwnedObligationsClosed(m, task) {
...
export function taskHasSatisfiedSettledOwnership(m, task) {
    if (!taskOwnedObligationsClosed(m, task) || !task.result || !UNRESOLVED_RESULT_STATUSES.has(task.result.status))
        return false;
...
export function taskControlStatus(m, task) {
    return taskHasSatisfiedSettledOwnership(m, task) && ['waiting', 'blocked'].includes(task.status) ? 'completed' : task.status;
}
export function taskPendingForControl(m, task) { return CONTROL_PENDING_TASK_STATUSES.has(taskControlStatus(m, task)); }
export function taskResultRequiresReconciliation(m, task) { return task.status !== 'cancelled' && !taskHasSatisfiedSettledOwnership(m, task) && !taskHasDelegatedReviewFindingRework(m, task) && Boolean(task.result && ['FIX_REQUIRED', 'NEEDS_CONTEXT'].includes(task.result.status)); }
/**
 ...
 */
export function reconcileSatisfiedTaskArtifacts(m, reason = 'canonical-obligation-closed') {
    const reconciled = [];
    for (const task of m.execution.tasks) {
        if (!taskHasSatisfiedSettledOwnership(m, task))
            continue;
        ...
        if (retiredIssues.length)
            m.execution.blockers = m.execution.blockers.filter(blocker => !retiredIssues.includes(blocker));
        const retiredNeeds = m.methodology.methodology_needs.filter(need => need.task_id === task.id);
        if (retiredNeeds.length)
            m.methodology.methodology_needs = m.methodology.methodology_needs.filter(need => need.task_id !== task.id);
        if (!retiredIssues.length && !retiredNeeds.length)
            continue;
