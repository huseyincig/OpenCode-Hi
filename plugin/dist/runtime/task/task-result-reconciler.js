...
export class TaskResultReconciler {
    ...
    applyResult(m, workerID, result) {
        ...
        if (readOnlyRole && result.changed_files.length && !readOnlyObservedMutation && readOnlyNativeDelta !== undefined) {
            appendLedger(m, 'worker.read-only-changed-files-claim-ignored', { task_id: task.id, worker_id: worker.id, payload: { reported: result.changed_files.slice(0, 40), reason: 'native-diff-proves-zero-read-only-mutation' } });
            normalizedResult = { ...result, changed_files: [] };
        }
        ...
        const constraintViolations = [...new Map(effectiveResult.changed_files.flatMap(file => deniedMutationAtoms(m.execution.constraint_atoms, file).map(atom => [`${atom.id}:${normFile(file)}`, { atom, file: normFile(file) }]))).values()];
        ...
        if (worker.role === 'visual-qa' && requiredVerificationCases.length && effectiveResult.status === 'DONE') {
            const coverage = effectiveResult.verification_coverage ?? [], coverageIDs = coverage.map(c => c.case_id), duplicateCoverageIDs = [...new Set(coverageIDs.filter((id, index) => coverageIDs.indexOf(id) !== index))], byID = new Map(coverage.map(c => [c.case_id, c])), actionForRef = (ref) => { const event = [...m.execution.ledger].reverse().find(event => event.type === 'browser.observation-recorded' && event.task_id === task.id && event.worker_id === worker.id && event.payload?.evidence_ref === ref); return typeof event?.payload?.action === 'string' ? event.payload.action : undefined; }, failures = [], unboundDetails = [];
            ...
            for (const required of requiredVerificationCases) {
                ...
                const refs = [...new Set(claim.evidence_refs)], supportByRef = refs.map(ref => m.execution.evidence.items.find(item => item.id === ref)), support = supportByRef.filter((item) => Boolean(item)), actions = new Set(refs.map(actionForRef).filter((x) => Boolean(x))), missingActions = required.required_browser_actions.filter(action => !actions.has(action)), validItem = (item) => item.trusted_source_class === 'browser-observation' && item.kind === 'browser-evidence' && !item.invalidated_at && item.outcome !== 'failed' && item.pass !== false && item.task_id === task.id && evidenceClaimApplicability(m, item).applicable, invalidRefs = refs.filter((_, index) => { const item = supportByRef[index]; return !item || !validItem(item); }), validRefs = refs.length > 0 && invalidRefs.length === 0 && support.length === refs.length;
                if (!validRefs) {
                    const corrections = invalidRefs.flatMap(from => { if (!/^ev_[a-z0-9_]+$/i.test(from))
                        return []; const matches = currentBrowserRefs.filter(to => to !== from && to.startsWith(from)); return matches.length === 1 ? [{ from, to: matches[0] }] : []; });
                    unboundDetails.push({ case_id: required.id, invalid_refs: invalidRefs.slice(0, 20), unique_prefix_corrections: corrections.slice(0, 20) });
                }
        ...
        const reconciledEvidence = effectiveResult.evidence.map(e => {
            ...
            if (claimedPassed && genericVerifierClaimKinds.has(e.kind)) {
                ...
                return { ...rest, outcome: 'pending', reason: 'worker-claim-unverified: canonical PASS requires an exact runtime/host verification observation' };
            }
        ...
        for (const e of effectiveResult.evidence) {
            ...
            if (e.kind === 'decision-evidence' && worker.role !== 'repository-explorer' && evidenceVerdictPassed(e.pass, e.outcome)) {
                ...
                if (!worker.session_id || !stateHash || !/^[a-f0-9]{64}$/i.test(stateHash)) {
                    appendLedger(m, 'decision.evidence-unbound', { task_id: task.id, worker_id: worker.id, payload: { reason: 'decision-observation-requires-exact-session-state', session_id: worker.session_id } });
                    continue;
                }
        ...
        if (effectiveResult.status === 'DONE' && resourceOnlyProcessSupport && !terminalProcessCapabilityBlocker) {
            ...
            if (!retained.length || missingOrigins.length) {
                ...
                appendLedger(m, 'process.resource-retention-admitted', { task_id: task.id, worker_id: worker.id, payload: { process_ids: retained.map(process => process.process_id), service_origins: retainedOrigins, policy: 'same-mission-parent-custody-until-dependent-verification-complete' } });
        }
        ...
        if (effectiveResult.status === 'DONE') {
            ...
            for (const id of task.obligation_ids) {
                ...
                if (owned.kind === 'verification') {
                    if (verificationSatisfied(m, owned.id, this.projectRoot).ok) {
                        owned.status = 'closed';
                        owned.closedAt = now;
                    }
                }
                ...
                else if (owned.kind === 'implementation') {
                    const changed = [...new Set([...m.vcs.changed_files, ...effectiveResult.changed_files])], coverage = assessRequiredTargetCoverage(owned.requiredTargets ?? [], changed);
                    if (!coverage.missing.length) {
                        owned.status = 'closed';
                        owned.closedAt = now;
                    }
                    else
                        appendLedger(m, 'implementation.required-targets-uncovered', { task_id: task.id, worker_id: worker.id, payload: { obligation: owned.id, required: coverage.required, covered: coverage.covered, missing: coverage.missing, changed_files: changed.slice(0, 60) } });
                }
                else {
                    owned.status = 'closed';
                    owned.closedAt = now;
