...
export class TaskRuntime {
    ...
    async start(m, input = {}) {
        ...
        const askGatedTools = Object.entries(profile.permission_profile.native?.decisions ?? {}).filter(([, value]) => value === 'ask').map(([name]) => name).sort(), askGatedInstruction = askGatedTools.length ? `host ask-gated tools: ${askGatedTools.join(', ')}. Use an ask-gated tool only when the scoped implementation itself cannot proceed without it. Do not invoke ask-gated tools solely to obtain mission verification; finish the allowed implementation and let Hi keep parent/control-plane verification open.` : undefined;
