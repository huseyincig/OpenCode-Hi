...
export class TaskRuntime {
    ...
    async start(m, input = {}) {
        ...
        const run = () => this.registry.dedupeSpawn(worker.fingerprint, async () => { let lastError = new Error('No runtime model available'); for (let i = 0; i < chain.length; i++) {
            ...
            try {
                ...
                const spawned = await this.#child.createForTask(m.session_id, `Hi · ${role} · ${objective.slice(0, 60)}`, role, model === 'host-default' ? undefined : model, variant, input.forkFromSession), child = spawned.child;
    ...
    async reconcileUserConstraint(m, text) {
        ...
        for (const worker of m.workers.filter(w => ['created', 'queued', 'starting', 'busy', 'ready'].includes(w.status))) {
            ...
            try {
                this.recordModelProjection(worker, model, variant);
                const child = await this.#child.create(m.session_id, `Hi · ${worker.role} · constraint update · ${task.objective.slice(0, 45)}`, worker.role, model === 'host-default' ? undefined : model, variant);
                if (!child?.id)
