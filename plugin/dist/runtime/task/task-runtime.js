...
export class TaskRuntime {
    ...
    async settleHostIdleRuntimeError(m, worker, error) {
        ...
        if (error.name === 'StructuredOutputError') {
            let result = resultContractFailure(`OpenCode rejected the child result because it did not satisfy the schema-bound WorkerResult transport contract. ${error.message}`, 'worker-result-contract-invalid:structured-output');
            appendLedger(m, 'worker.result-contract-retryable', { task_id: task.id, worker_id: worker.id, payload: { model: worker.model, attempt: worker.attempt, generation: m.continuation.generation, transport: 'opencode-json-schema', error: error.message, event_path: 'session-error' } });
            ...
            return { applied: true, reason: 'host-terminal-structured-result-contract-failed', result, wakeResult: 'FIX_REQUIRED' };
        }
        ...
        const hostInterrupted = worker.last_runtime_failure_kind === 'host-interruption';
        if (!hostInterrupted)
            worker.restart_reconcile_pending = false;
        if (recovery === 'RECOVERED')
            return hostInterrupted ? { applied: true, reason: 'host-idle-interruption-recoverable', wakeResult: 'HOST_INTERRUPTION', failureKind: worker.last_runtime_failure_kind } : { applied: true, reason: 'host-idle-runtime-fallback', wakeResult: 'RUNTIME_FALLBACK', failureKind: worker.last_runtime_failure_kind };
    ...
    async settleHostIdleAssistantResult(m, worker, assistant) {
        ...
        if (assistant.error?.name === 'StructuredOutputError')
            return this.settleHostIdleRuntimeError(m, worker, assistant.error);
        if (assistant.error)
            return this.settleHostIdleRuntimeError(m, worker, assistant.error);
        if (!assistant.model && !assistant.text && assistant.structured === undefined)
        ...
        if (assistant.structured !== undefined) {
            ...
            result = parseWorkerResult(assistant.text);
        ...
        if (unparseableWorkerContract) {
            ...
            appendLedger(m, 'worker.result-contract-retryable', { task_id: task.id, worker_id: worker.id, payload: { model: worker.model, attempt: worker.attempt, generation: m.continuation.generation, transport: 'text-compatibility' } });
        }
        if (!effective.ok)
    ...
    async start(m, input = {}) {
        ...
        if (resumeTask && isHiReviewerRole(role)) {
            const predecessors = m.execution.obligations.filter(obligation => obligation.status === 'open' && (obligation.kind === 'analysis' || obligation.kind === 'implementation') && !resumeTask.obligation_ids.includes(obligation.id));
            if (predecessors.length)
                throw new Error(`Hi reviewer task ${resumeTask.id} cannot resume before canonical predecessor rework closes: ${predecessors.map(obligation => obligation.id).join(', ')}. Complete writer-owned remediation first, then re-run this exact reviewer task for fresh verification.`);
        }
        ...
        const scope = scopeAdmission.scope, objective = implicitProcessSupport && !input.resumeTaskId ? `Establish only the bounded runtime process resource/readiness for scoped target(s): ${scope.length ? scope.join(', ') : 'mission-local runtime target'}${explicitBrowserRequiredOrigins.length ? `; required service origin(s): ${explicitBrowserRequiredOrigins.join(', ')}` : ''}. Confirm the owned persistent process/service is ready, retain it RUNNING with cleanup_state=ACTIVE for downstream mission verification, return its ProcessContract/service origin, then return DONE without killing or cleaning that retained service. Parent control-plane custody performs teardown only after dependent verification/open obligations no longer need it. Do not perform mission/browser/test/review verification.` : requestedObjective;
        ...
        if (exactCorrectiveImplementationResume && staleExplorationClearance)
            appendLedger(m, 'task.resume.exploration-revalidation-owned', { task_id: resumeTask.id, worker_id: resumeWorker.id, payload: { reason: clearanceFreshness.reason, scope: resumeTask.scope.slice(0, 40), policy: 'same-task-continuation-must-revalidate-current-bounded-source-before-further-mutation' } });
        ...
        if (existing) {
            ...
            if (existing.status === 'ready' && existing.session_id && oldTask && (existing.restart_reconcile_pending || Boolean(oldTask.result && ['FIX_REQUIRED', 'NEEDS_CONTEXT', 'BLOCKED'].includes(oldTask.result.status)))) {
                const hazardBeforeResume = recoveryModelHazard(m);
                if (hazardBeforeResume.same_model_exhausted && hazardBeforeResume.worker_id === existing.id && hazardBeforeResume.model === existing.model) {
                    const escalated = hazardBeforeResume.open ? await this.#recovery.recoverStagnation(m, 3, 'model-escalation') : false;
                    if (escalated)
                        return { task_id: oldTask.id, worker_id: existing.id, session_id: existing.session_id, model: existing.model, methodologies: existing.selected_methodologies, selection_reason: ['behavioral-recovery:model-escalation-after-repeated-same-failure-correction'], readiness: 'READY', preconditions: preflight.items };
                    appendLedger(m, 'worker.same-model-correction-exhausted', { task_id: oldTask.id, worker_id: existing.id, payload: { reason: hazardBeforeResume.reason, attempts: hazardBeforeResume.attempts, failure_signature: hazardBeforeResume.failure_signature, recovery_candidates: hazardBeforeResume.recovery_candidates } });
                    throw new Error(`Same-model corrective recovery is exhausted for the unchanged failure on task ${oldTask.id}. Use a mechanically different parent strategy${hazardBeforeResume.recovery_candidates.length ? ' or an admitted recovery-only model candidate' : ''}; do not resume the same task/model again.`);
                }
                ...
                const issues = (oldTask.result?.open_issues ?? []).join(' | '), missing = (oldTask.result?.needs_context ?? []).join(' | '), sourceRevalidation = exactCorrectiveImplementationResume && staleExplorationClearance ? `RECOVERY SOURCE REVALIDATION: prior repository clearance is stale after implementation began. This is continuation of the existing task/session, not fresh implementation admission. Before any further mutation, inspect the current diff/state and re-read the current bounded task scope (${oldTask.scope.join(', ') || 'none'}) plus only read-only repository context needed to reconcile it. Preserve valid prior work; do not widen mutation scope or restart planning. If the current source makes the canonical target or contract genuinely ambiguous, return FIX_REQUIRED or NEEDS_CONTEXT with the exact conflict instead of guessing.` : '', visualAttemptFreshness = existing.role === 'visual-qa' && (oldTask.verification_cases?.length ?? 0) > 0, freshEvidence = m.execution.evidence.items.filter(e => !e.invalidated_at && evidenceVerdictPassed(e.pass, e.outcome) && (e.task_id === oldTask.id || e.obligation_ids?.some(id => oldTask.obligation_ids.includes(id)))).filter(e => !oldTask.requiredEvidence.length || oldTask.requiredEvidence.some(kind => kind === e.kind || (kind === 'visual-check' && e.kind === 'visual-evidence'))).filter(e => !visualAttemptFreshness || !['browser-evidence', 'visual-evidence', 'accessibility-evidence'].includes(e.kind)).slice(-8).map(e => `${e.kind}: ${e.summary}`).join(' | '), visualResumeFreshness = visualAttemptFreshness ? `VISUAL CORRECTIVE ATTEMPT FRESHNESS: this resume opens a new worker attempt. Prior browser/visual/accessibility evidence is historical only and MUST NOT be treated or cited as current-attempt proof. Rerun every required browser action for every verification case in this new attempt and return only the NEW full evidence_ref tokens from those observations; do not reuse prior-attempt evidence_refs.` : '', reviewScope = isHiReadOnlyChildRole(existing.role) ? `Scoped rereview only: previous findings=${issues || 'none'}; task scope=${oldTask.scope.join(',') || 'none'}; task-owned fresh evidence=${freshEvidence || 'none'}.` : '', reviewVerdictContract = isHiReviewerRole(existing.role) && oldTask.requiredEvidence.includes('review-evidence') ? 'REVIEWER CLOSURE CONTRACT: this bounded task passes only by returning evidence.kind="review-evidence" with outcome="passed". Mission verification kinds not listed in TASK REQUIRED EVIDENCE belong to other owners and must not block or be claimed by this reviewer. Do not call parent-only Hi control-plane tools; return the structured WorkerResult claim only.' : '', processResumeContext = ownedProcessResumeContext(m, oldTask, existing), resumeExitRequirements = existing.selected_methodologies.flatMap(name => { const item = catalog.find(x => x.name === name); return item ? [`${name}: ${item.exitRequirements.join(', ')}`] : []; });
                ...
                existing.restart_reconcile_pending = false;
                appendLedger(m, nextModel !== previousModel ? 'worker.model-escalated' : 'worker.resumed', { task_id: oldTask.id, worker_id: existing.id, payload: { status: oldTask.result?.status ?? 'HOST_INTERRUPTED', model: nextModel, correction_level: correctionLevel, recovery_progress_signature: hazardBeforeResume.progress_signature } });
            }
        ...
        const verificationCases = role === 'visual-qa' ? [...new Map(obligationIds.flatMap(id => m.execution.obligations.find(o => o.id === id)?.verificationCases ?? []).map(c => [c.id, c])).values()].map(c => ({ ...c, required_browser_actions: [...c.required_browser_actions], ...(c.source_units?.length ? { source_units: [...c.source_units] } : {}) })) : [];
        ...
        const task = createTask(m, { objective, role, category, scope, constraints, dependencies, requiredEvidence, requiredEvidenceOrigin, obligationIds, verificationCases, contextReferences: selectedContextReferences, executionProfile: profile });
    ...
    async resume(m, taskID) {
        ...
        const missionVerificationKinds = new Set(m.execution.verification_policy.requiredKinds), ownsVerification = task.obligation_ids.some(id => m.execution.obligations.some(o => o.id === id && o.kind === 'verification')), legacyImplicitExplorerEvidence = task.requiredEvidenceOrigin === undefined && task.role === 'repository-explorer' && !ownsVerification && task.requiredEvidence.length > 0 && task.requiredEvidence.every(kind => missionVerificationKinds.has(kind));
        if (legacyImplicitExplorerEvidence) {
            const removedEvidence = [...task.requiredEvidence];
            task.requiredEvidence = [];
            if (task.execution_profile)
                task.execution_profile.task.required_evidence = [];
            task.requiredEvidenceOrigin = 'role-default';
            task.updated_at = Date.now();
            worker.fingerprint = workerFingerprint(task.role, task.category, worker.model, m.identity.intent.taskKind, task.objective, { scope: task.scope, constraints: task.constraints, dependencies: task.dependencies, requiredEvidence: task.requiredEvidence, obligationIds: task.obligation_ids });
        ...
        if (worker.status !== 'ready' || (!worker.restart_reconcile_pending && (!task.result || !['FIX_REQUIRED', 'NEEDS_CONTEXT', 'BLOCKED'].includes(task.result.status))))
    ...
    async reconcileIdleAwaitResult(m, worker) {
        ...
        try {
            ...
            appendLedger(m, 'worker.await-idle-result-deferred', { task_id: worker.task_id, worker_id: worker.id, payload: { session_id: worker.session_id, reason: 'assistant-result-read-failed', error: String(error).slice(0, 500) } });
            return false;
        }
        const settled = await this.settleHostIdleAssistantResult(m, worker, assistant);
        if (!settled.applied) {
            appendLedger(m, 'worker.await-idle-result-deferred', { task_id: worker.task_id, worker_id: worker.id, payload: { session_id: worker.session_id, reason: settled.reason } });
            return false;
        }
    ...
    async awaitTask(m, id, timeoutMs = 30_000) {
        ...
        if (['completed', 'failed', 'cancelled', 'blocked'].includes(status))
            return { ...current, status, terminal: true, changed: false, timed_out: false, live_status: 'idle', progress_observed: false };
        const worker = current?.worker;
        if (!worker)
            return { ...current, status, terminal: false, changed: false, timed_out: false, live_status: 'unknown', progress_observed: false };
        const before = await this.observeWorkerLiveness(m, worker);
        if (before.live_status === 'idle' && await this.reconcileIdleAwaitResult(m, worker)) {
            current = this.peek(m, id);
            status = current?.task?.status ?? current?.worker?.status ?? 'unknown';
            return { ...current, status, terminal: ['completed', 'failed', 'cancelled', 'blocked'].includes(status), changed: true, timed_out: false, live_status: 'idle', progress_observed: false };
        }
        const changed = await this.registry.waitForChange(worker.id, timeoutMs);
        current = this.peek(m, id);
        status = current?.task?.status ?? current?.worker?.status ?? 'unknown';
        let terminal = ['completed', 'failed', 'cancelled', 'blocked'].includes(status), observed = terminal ? { live_status: 'idle', progress_observed: false } : await this.observeWorkerLiveness(m, current?.worker ?? worker);
        if (!terminal && observed.live_status === 'idle' && await this.reconcileIdleAwaitResult(m, current?.worker ?? worker)) {
            current = this.peek(m, id);
            status = current?.task?.status ?? current?.worker?.status ?? 'unknown';
            terminal = ['completed', 'failed', 'cancelled', 'blocked'].includes(status);
            observed = { live_status: 'idle', progress_observed: false };
            return { ...current, status, terminal, changed: true, timed_out: false, ...observed };
        }
        if (!changed && !terminal && observed.progress_observed)
            appendLedger(m, 'worker.await-progress-observed', { task_id: current?.task?.id, worker_id: (current?.worker ?? worker).id, payload: { session_id: (current?.worker ?? worker).session_id, attempt: (current?.worker ?? worker).attempt, live_status: observed.live_status } });
