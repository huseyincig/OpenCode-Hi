...
const TASK_EVIDENCE_METHODOLOGY_SIGNALS = {
    'visual-check': ['verification.visual'],
};
export function reconcileTaskEvidenceMethodologyNeeds(mission, projectRoot, input) {
    const required = new Set(input.requiredEvidence.map(item => String(item).trim()).filter(Boolean)), activated = [];
                ...
                for (const entry of methodologiesForSignal(signal, projectRoot)) {
                    ...
                    if (existing) {
                        if (!existing.obligation_id) {
                            ...
                            appendLedger(mission, 'methodology.obligation-reconciled', { payload: { name: existing.name, signal: existing.signal, producer: existing.producer, obligation_id: obligation.id, reason: 'current-open-obligation-evidence-contract' } });
                        }
                        continue;
                    }
