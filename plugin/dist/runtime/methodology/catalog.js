import { HI_METHODOLOGY_LIMITS, HI_METHODOLOGY_POLICY, HI_METHODOLOGY_SIGNAL_CATALOG } from '../../generated/methodology-policy.js';
import { discoverProjectMethodologyPolicies } from './project-policy.js';
function builtinEntry(item) {
    return {
        name: item.name,
        provider: 'hi',
...
function projectEntry(item) {
    ...
    const triggerSources = [...new Set(activationSignals.map(signal => HI_METHODOLOGY_SIGNAL_CATALOG[signal].trigger_source))];
    return {
        name: item.name,
        provider: 'project',
...
export function methodologyCatalog(projectRoot) {
...
export function methodologyCatalogEntry(name, projectRoot) {
    const builtin = BUILTIN_BY_NAME.get(name);
    if (builtin) {
        if (projectRoot && discoverProjectMethodologyPolicies(projectRoot).some(item => item.name === name))
            return undefined;
