...
export function normalizeIntent(text, repo) {
    ...
    if (taskKind === 'bug-fix')
        ...
        requiredCapabilities.push('repository-analysis', 'review');
    else if (taskKind === 'performance')
        requiredCapabilities.push('repository-analysis', 'performance-analysis', 'implementation');
    else if (taskKind === 'release-readiness')
    ...
    if (PLAN.test(objective) && scope !== 'local')
        requiredCapabilities.push('implementation-planning');
    if (DESIGN.test(objective) && (scope !== 'local' || /design|architecture|mimari|tasarım/i.test(objective)))
        requiredCapabilities.push('design-exploration');
    if (SOURCE_VERIFY.test(objective))
        requiredCapabilities.push('source-verification');
    if (REVIEW_FEEDBACK.test(objective))
        requiredCapabilities.push('review-feedback');
    if (API_DESIGN.test(objective))
        requiredCapabilities.push('api-interface-design');
    if (WORKSPACE_ISOLATION.test(objective))
    ...
    if (SECURITY.test(objective)) {
        ...
        requiredCapabilities.push('critical-validation');
    if (VISUAL.test(objective))
        requiredCapabilities.push('visual-qa');
    if (scope === 'multi-stream')
        requiredCapabilities.push('multi-stream-delegation');
    ...
    let likelyVerification;
    if (taskKind === 'review')
        likelyVerification = ['review-evidence'];
    else if (taskKind === 'release-readiness') {
        likelyVerification = [...(hasTest ? ['targeted-tests'] : ['changed-surface-sanity']), ...(staticKind ? [staticKind] : []), ...(buildKind ? [buildKind] : [])];
    }
    else if (VISUAL.test(objective)) {
        likelyVerification = ['changed-surface-sanity', 'visual-check'];
    }
        ...
        if (risk === 'high' || scope === 'repo-wide' || scope === 'multi-stream') {
            if (staticKind)
                likelyVerification.push(staticKind);
            if (buildKind)
                likelyVerification.push(buildKind);
        }
        else if (scope === 'multi-file' && staticKind)
            likelyVerification.push(staticKind);
    }
    else {
