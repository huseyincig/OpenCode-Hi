...
export function renderSemanticAssessmentGate(m) {
    ...
    return [
        ...
        `all keys required:material=true|false;message_kind=${messageKinds};task_kind=implementation|bug-fix|diagnosis|review|performance|release-readiness;scope:local|multi-file|repo-wide|external|multi-stream;risk=low|medium|high|authority-boundary;ambiguity:none|resolvable|contract-critical;dependency_class:independent|sequential|external-gated|unknown|independent-multi;continuation_required=bool;required_capabilities=C;requested_external_actions=X[];likely_verification=V[];verification_cases[];nonvisual_request_units[];capability_request_units={}${semantic.phase === 'followup' ? ';constraint_atoms[]' : ''}`,
        ...atomLines,
        ...
        'mutation_targets⊆likely_targets;write;exclude context/source-of-truth;scope/dependency describe material change units;user-unchanged test files excluded;one change+verification != sequential',
        'diagnosis=no-write;review=no-write;fix/project=>bug-fix+C:implementation;docs=>C:documentation;verify!=test-authoring;intent.debugging=>diagnosis+repository-analysis;independent-review=explicit/risk;interactive-process=persistent;X=>risk=authority-boundary',
        phaseRule,
    ].join('\n');
}
