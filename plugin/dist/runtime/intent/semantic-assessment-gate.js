...
export function renderSemanticAssessmentGate(m) {
    ...
    return [
        ...
        ...atomLines,
        `C=${SEMANTIC_CAPABILITIES.join('|')};X=${SEMANTIC_EXTERNAL_ACTIONS.join('|')};V=${SEMANTIC_VERIFICATION_KINDS.join('|')}. visual-check=>verification_cases=1..16 {id:vc_<slug>,subject,required_browser_actions:A[]} one per explicit browser outcome;${semantic.phase === 'followup' ? 'resume/constraint verification_cases=[] keeps prior; ' : ''}A=open|navigate|click|type|key|inspect|viewport|screenshot|wait|close;reload=navigate+inspect;intent_signals=[] by default;intent.<slug>:intent.tdd;unknown signals reject;capability-named signals reject.`,
        'scope and dependency_class describe material implementation/change work units;exclude test files that the user says must remain unchanged;one implementation change followed by verification is not a sequential dependency.',
