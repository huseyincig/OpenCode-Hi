...
export const SEMANTIC_CAPABILITIES = ['implementation', 'repository-analysis', 'review', 'verification', 'independent-review', 'security-review', 'visual-qa', 'design-exploration', 'multi-stream-delegation', 'source-verification', 'qa-review', 'dependency-change'];
...
export function technicalTargets(text) { return [...text.matchAll(PATH)].map(m => m[1]).filter(Boolean).slice(0, 12); }
...
function enumList(value, allowed, max = 40) { const items = stringList(value, max), set = new Set(allowed), unknown = items.filter(x => !set.has(x)); if (unknown.length)
    throw new Error(`unsupported semantic enum value(s): ${unknown.join(', ')}`); return items; }
...
export function parseSemanticIntentAssessment(raw) {
    ...
    const v = x;
    const taskKinds = ['implementation', 'bug-fix', 'review', 'performance', 'release-readiness'];
