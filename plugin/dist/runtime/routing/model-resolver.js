...
export function resolveModel(category, availableInput, config, explicit, role, hostConfig, feedback = {}) {
    ...
    if (!reason.length)
        reason.push(`${category} category`);
    reason.push('write-capable', 'runtime available', 'routing policy allowed', `${config.routing.strategy} scoring`, `expected-completion-cost-aware`, `current-mission-failure-history-aware`, primaryVariant ? `variant:${primaryVariant}` : 'variant:host/default', `fallbacks=${fallbacks.length}`);
    if (nativePolicySources.length)
        reason.push(`opencode-provider-policy:${nativePolicySources.join('+')}`);
