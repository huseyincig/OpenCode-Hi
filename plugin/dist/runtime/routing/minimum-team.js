export function minimumTeamFor(intent, verification, primaryMode = 'auto') {
    const caps = new Set(intent.requiredCapabilities), reviewRequired = verification?.requireReview === true || intent.risk === 'high', primary = primaryMode === 'auto' ? 'working-manager' : primaryMode;
    ...
    if (caps.has('design-exploration'))
        roles.push('architect');
    if ((intent.taskKind === 'diagnosis' || caps.has('repository-analysis')) && intent.scope !== 'local')
        roles.push('repository-explorer');
    if (productionImplementation)
        roles.push('coder');
    if (caps.has('documentation'))
        roles.push('technical-writer');
    if (caps.has('test-authoring'))
        roles.push('test-engineer');
    if (caps.has('visual-qa'))
        roles.push('visual-qa');
    if (caps.has('security-review') && (reviewRequired || intent.taskKind === 'review'))
        roles.push('security-reviewer');
    else if ((intent.taskKind === 'review' || caps.has('independent-review') || caps.has('qa-review')) && reviewRequired)
