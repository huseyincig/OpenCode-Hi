...
export function deriveMissionModelFeedback(m, role, category, window = DEFAULT_WINDOW) {
    ...
    for (const worker of workers) {
        const finalModel = String(worker.effective_model ?? worker.model ?? '');
        for (const transition of worker.fallback_history ?? []) {
            if (!transition.from)
                continue;
            inc(retries, transition.from);
            if (transition.from !== finalModel) {
                inc(samples, transition.from);
                inc(failures, transition.from);
            }
        }
    }
    for (const [model, xs] of Object.entries(latencies))
        average_latency_ms[model] = Math.round(xs.reduce((a, b) => a + b, 0) / xs.length);
    const models = new Set([...Object.keys(samples), ...Object.keys(retries)]), confidence_by_model = Object.fromEntries([...models].map(model => [model, confidence(samples[model] ?? 0)]));
