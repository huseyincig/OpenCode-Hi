export function resolveExecutionMode(intent, m) { if (m?.execution.execution_mode === 'team')
    ...
    return { mode: 'single', reason: ['authority boundary forbids speculative parallel work'] }; if (intent.scope === 'multi-stream' || intent.dependencyClass === 'independent-multi')
    return { mode: 'parallel', reason: [intent.scope === 'multi-stream' ? 'structured multi-stream scope proves independent workstreams' : 'structured independent-multi dependency class proves independent workstreams'] }; if (intent.scope === 'local' || intent.requiredCapabilities.length <= 1)
