...
export function adjudicateCompletion(m) { const reasons = []; syncMissionGates(m); if (missionRequiresReleaseCreate(m.objective)) {
    if (m.release_chain?.release?.outcome !== 'success')
        reasons.push('release-chain:release-not-completed');
    else if (!m.release_chain?.release?.remote_verified)
        reasons.push('release-chain:release-remote-unverified');
} if (missionRequiresPackagePublish(m.objective)) {
    if (m.release_chain?.package?.outcome !== 'success')
    ...
    reasons.push(`blockers:${m.blockers.slice(0, 6).join('|')}`); const open = m.obligations.filter(o => o.status === 'open'); if (open.length)
    reasons.push(`open-obligations:${open.map(o => o.id).join(',')}`); const authorityGate = m.gates.find(g => g.kind === 'user-authority' && g.status !== 'closed'); if (authorityGate)
    return { complete: false, reasons: [...reasons, `authority:${authorityGate.status}`], next: 'USER_ACTION_REQUIRED' }; const rollback = m.gates.find(g => g.kind === 'rollback' && g.status !== 'closed'); if (rollback)
    return { complete: false, reasons: [...reasons, 'temporary-rollback-pending'], next: 'USER_ACTION_REQUIRED' }; const prereq = m.gates.find(g => g.kind === 'prerequisite-task' && g.status !== 'closed'); if (prereq)
    reasons.push('prerequisite-task-pending'); const verificationRequired = m.obligations.some(o => o.kind === 'verification'); if (verificationRequired) {
...
} if (m.tasks.some(t => t.result?.status === 'FIX_REQUIRED' || t.result?.status === 'NEEDS_CONTEXT'))
