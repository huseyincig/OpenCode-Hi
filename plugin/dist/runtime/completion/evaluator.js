...
export function evaluateCompletion(m) { const reasons = []; syncMissionGates(m); if (missionRequiresReleaseCreate(m)) {
    ...
    reasons.push('pending-task'); if (m.execution.native_todos_incomplete > 0)
    ...
    reasons.push(`methodology-needs:${[...new Set(m.methodology.methodology_needs.map(n => n.name))].slice(0, 6).join('|')}`); const open = m.execution.obligations.filter(o => o.status === 'open'); if (open.length)
