...
export function evaluateCompletion(m, projectRoot) { reconcileSatisfiedTaskArtifacts(m, 'completion-evaluation'); const reasons = [], verification = verificationClaimsSatisfied(m, projectRoot), reviews = reviewClaimsSatisfied(m, projectRoot); syncMissionGates(m, projectRoot, { verification, review: reviews }); if (missionRequiresReleaseCreate(m)) {
    ...
    return { complete: false, reasons: ['semantic-assessment-pending'], next: 'CONTINUE' }; if (m.continuation.awaiting_user_followup)
    return { complete: false, reasons: ['user-followup-required'], next: 'USER_ACTION_REQUIRED' }; if (m.authority.human_decision?.status === 'OPEN' && m.authority.human_decision.semantic_type !== 'authority_request')
    ...
    reasons.push('process-cleanup-pending'); if (m.execution.blockers.length)
