...
export function evaluateCompletion(m) { const reasons = []; syncMissionGates(m); if (missionRequiresReleaseCreate(m)) {
    ...
    reasons.push(`blockers:${m.blockers.slice(0, 6).join('|')}`); if (m.methodology_needs.length)
    reasons.push(`methodology-needs:${[...new Set(m.methodology_needs.map(n => n.name))].slice(0, 6).join('|')}`); const open = m.obligations.filter(o => o.status === 'open'); if (open.length)
