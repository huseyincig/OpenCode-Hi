...
function result(id, before, after, evidence) {
...
const obs = (x) => ({ modelCalls: 1, agentCount: 1, toolCalls: 2, contextChars: 1200, skills: 0, verificationActions: 1, delegations: 0, retries: 0, humanInteractions: 0, productiveActions: 3, totalActions: 3, elapsedUnits: 3, ...x });
export function runDeterministicBenchmarks() {
...
export function runSchedulerEconomicsBenchmarks() {
    ...
    const existing = [{ id: 'writer-a', mission_id: 'benchmark-mission', objective: 'a', status: 'running', role: 'coder', category: 'standard', scope: ['src/shared'], constraints: [], dependencies: [], requiredEvidence: [], obligation_ids: [], context_artifacts: [], gate_ids: [], external_action_requirements: [], created_at: 1, updated_at: 1 }];
...
export function runRecoveryGovernorAblation() {
    ...
    const identity = (plan) => `${plan.level}:${plan.action}`;
