...
export class ContextArtifactStore {
    ...
    getBinary(id) {
        ...
        if (!item || item.content_ref !== 'inline-body')
            return undefined;
        let manifest;
        try {
        ...
        if (!manifest || typeof manifest !== 'object' || Array.isArray(manifest))
            return undefined;
        const x = manifest, mime = typeof x.media_type === 'string' ? x.media_type : '', sha = typeof x.byte_sha256 === 'string' ? x.byte_sha256 : '', size = x.byte_size, filename = typeof x.file === 'string' ? x.file : '';
        if (!mime || mime.length > 120 || !/^[a-f0-9]{64}$/.test(sha) || !Number.isInteger(size) || Number(size) <= 0 || Number(size) > 10 * 1024 * 1024)
            return undefined;
        const prefix = `${id}.`;
        if (!filename.startsWith(prefix))
            return undefined;
        const extension = filename.slice(prefix.length);
        if (!/^[A-Za-z0-9]{1,12}$/.test(extension))
            return undefined;
        const path = durableArtifactBinaryPath(this.projectRoot, item.kind, id, extension);
        if (!existsSync(path))
            return undefined;
        const bytes = readFileSync(path);
        if (bytes.byteLength !== Number(size) || createHash('sha256').update(bytes).digest('hex') !== sha)
            return undefined;
