import { adjudicateCompletion } from '../completion/adjudicator.js';
...
export function evaluateIdle(m, now = Date.now()) {
    ...
    if (continuationFailures > 0) {
        const instruction = 'The previous HHC continuation delivery failed at the OpenCode runtime/transport layer. Retry the same bounded continuation without changing strategy or counting this as reasoning stagnation.';
    ...
    if (m.tasks.some(t => t.result && ['FIX_REQUIRED', 'NEEDS_CONTEXT'].includes(t.result.status))) {
    ...
    const permissionBlocker = m.blockers.find(x => x.startsWith('permission-failure:'));
    ...
    const providerBlocker = m.blockers.find(x => x.startsWith('provider-failure:'));
    if (providerBlocker) {
        m.stagnation_count = 0;
        return { decision: 'USER_ACTION_REQUIRED', reason: providerBlocker, reason_code: 'provider-failure-blocked' };
    }
    const completion = adjudicateCompletion(m);
    ...
    if (completion.next === 'VERIFY') {
        ...
        if (latest?.outcome === 'environment-issue') {
            m.stagnation_count = 0;
            const instruction = 'Verification could not run because of an environment/tooling issue. Recover the environment or use an equivalent allowed verification path; do not modify product code merely to make an unavailable verifier run, and do not count this as reasoning stagnation.';
            setRuntimeNudge(m, instruction, 'verification-environment-issue');
            return { decision: 'RECOVER', reason: latest.reason ?? 'verification-environment-issue', reason_code: 'verification-environment-issue', prompt: continuationPrompt(m, instruction) };
        }
        ...
        const instruction = 'Verification is required and current evidence is stale or missing. Run the minimum sufficient verification.';
    ...
    if (m.iteration >= m.continuation_budget || recovery.action === 'user-action')
        return { decision: 'USER_ACTION_REQUIRED', reason: 'bounded-autonomy-exhausted', reason_code: 'bounded-autonomy-exhausted' };
    if (recovery.level > 0) {
...
export function continuationPrompt(m, action) { const open = m.obligations.filter(o => o.status === 'open').map(o => o.summary).slice(0, 3); return ['HHC runtime: mission is still active.', `Open obligation: ${open.join(' | ') || 'none'}.`, action, 'Resume from current state. Do not restart planning. Do not create duplicate tasks.'].join('\n'); }
export function shouldCountStagnation(decision) { return decision.reason_code === 'open-obligation' || decision.reason_code === 'contract-ambiguity-repo-first' || decision.reason_code === 'stagnation-recovery'; }
