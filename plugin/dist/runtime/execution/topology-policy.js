...
export function decideTopology(intent, config = DEFAULT_TOPOLOGY_POLICY, m) {
    if (m?.execution.execution_mode === 'team')
        return { mode: 'multi-agent', executionMode: 'parallel', agentCount: Math.max(2, m.execution.topology?.parallelism ?? 2), parallelism: Math.max(1, m.execution.topology?.parallelism ?? 2), reason: ['legacy team execution mode normalized to scheduler-owned parallel topology'] };
