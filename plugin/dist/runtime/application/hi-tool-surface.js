...
export function createHiToolSurface(input) {
    ...
    const awaitTool = tool({ description: 'Wait for one bounded Hi task state change or terminal result without model/shell polling. Default 30s, max 60s. A wait timeout is never a stuck/terminal claim: live_status and progress_observed report the exact child liveness reconciliation. If timed_out is true, return control and do not busy-loop or cancel a healthy active child.', args: { id: tool.schema.string(), timeout_ms: tool.schema.number().optional() }, execute: async (a, c) => { const m = store.get(c?.sessionID); if (!m)
            ...
            appendLedger(m, 'worker.await-timeout', { task_id: x.task?.id, worker_id: x.worker.id, payload: { session_id: x.worker.session_id, attempt: x.worker.attempt, timeout_ms: timeout, live_status: x.live_status, progress_observed: x.progress_observed === true } }); return JSON.stringify({ status: x.status, terminal: x.terminal, changed: x.changed, timed_out: x.timed_out, live_status: x.live_status, progress_observed: x.progress_observed === true, result: x?.task?.result, control: projectControlDecision(m, projectRoot) }); } });
    const cancelTool = tool({ description: 'Cancel one retained non-running Hi task/worker. Model-facing cancellation is fail-closed for an active child: busy/retry work must be awaited, idle-but-unreconciled work must be reconciled, and unknown liveness must not be aborted. User STOP and runtime-owned recovery/conflict cancellation use canonical internal paths instead.', args: { id: tool.schema.string() }, execute: async (a, c) => { const m = store.get(c?.sessionID); if (!m)
            return 'false'; const admission = await tasks.modelCancelAdmission(m, String(a.id)); if (!admission.allowed)
            return JSON.stringify({ status: 'BLOCKED', reason: admission.reason, task_id: admission.task_id, worker_id: admission.worker_id, live_status: admission.live_status, retry_same_cancel: false, next: admission.reason === 'healthy-worker-active' ? 'hi_task_await' : admission.reason === 'child-result-reconcile-required' ? 'hi_task_peek' : 'reconcile-liveness' }); return String(await tasks.cancel(m, a.id)); } });
    ...
    const processSpawnTool = tool({ description: 'Spawn one process owned by the current admitted process-lifecycle child through the native OpenCode PTY lifecycle. Parent sessions cannot proxy this execution: use hi_task_start/hi_task_await to control the exact task and let that child invoke hi_process_spawn. command is argv[0] only; put every argument in args_json (example: command="python3", args_json="[\"app.py\"]"). For explicit shell semantics use command="bash" with args_json="[\"-c\",\"...\"]"; Hi never silently splits a shell-like command string. timeout_ms is an optional HARD wall-clock termination deadline, not a readiness/wait budget: omit timeout_ms for a server/watcher/service that must remain alive while verification uses it. Never increase a finite timeout and replay the same healthy persistent command as a substitute for persistence. For a persistent local HTTP(S) service that will be browser-verified, pass service_origins with the exact loopback origin(s) when known; Hi also reconciles local origins mechanically from bounded process output. Native permission ask remains a real OpenCode permission request.', args: { worker_id: tool.schema.string().optional(), command: tool.schema.string(), args_json: tool.schema.string().optional(), cwd: tool.schema.string().optional(), timeout_ms: tool.schema.number().optional(), title: tool.schema.string().optional(), service_origins: tool.schema.string().optional() }, execute: async (a, c) => { let cx; try {
            ...
            return JSON.stringify(await processRuntime.spawn(m, { worker_id: workerID, command: admitted.command, args, cwd: String(a.cwd ?? c?.directory ?? projectRoot), timeout_ms: a.timeout_ms === undefined ? undefined : Number(a.timeout_ms), title: a.title ? String(a.title) : undefined, service_origins: a.service_origins ? normalizeBrowserAllowedOrigins(String(a.service_origins).split(',').map((x) => x.trim()).filter(Boolean).slice(0, 8)) : undefined, ask: async (request) => c.ask({ permission: request.permission, patterns: request.patterns, always: request.always, metadata: request.metadata }) }));
        }
        catch (error) {
