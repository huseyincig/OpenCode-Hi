...
import { formatAutonomousMissionStatus } from '../ux/autonomous-mission.js';
...
export function createHiToolSurface(input) {
    ...
    const statusTool = tool({ description: 'Show bounded goal-oriented Hi mission status: current work, control action, verification, authority and blockers. This is recomputed from canonical Mission state and excludes diagnostic logs/ledger payloads.', args: {}, execute: async (_args, c) => { const m = store.get(c?.sessionID); return m ? formatAutonomousMissionStatus(m, projectRoot) : 'Hi: no active mission'; } });
    ...
    const superProductTool = tool({ description: 'Show one bounded read-only product/integration readiness view over the current Mission, models, project intelligence, economics and ecosystem compatibility. It creates no registry, approval, routing or completion authority.', args: {}, execute: async (_args, c) => { const m = store.get(c?.sessionID); if (!m)
    ...
    const mutationTool = tool({ description: 'Register a temporary execution mutation only when this primary owns a deterministic rollback path. Prefer native session revert for project-local tracked experiments; exact command rollback is available only to a command-capable primary such as working-manager, never the read-only manager.', args: { kind: tool.schema.string(), description: tool.schema.string(), rollback_command: tool.schema.string().optional(), native_revert: tool.schema.boolean().optional(), session_id: tool.schema.string().optional(), message_id: tool.schema.string().optional() }, execute: async (a, c) => { const m = store.get(c?.sessionID); if (!m)
            return 'No active Hi mission'; const mode = a.native_revert ? 'native-revert' : 'command', messageID = a.message_id ? String(a.message_id).trim() : String(c?.messageID ?? '').trim(); if (mode === 'command' && m.execution.primary_mode === 'manager') {
            appendLedger(m, 'temporary-mutation.registration-blocked', { payload: { reason: 'read-only-primary-command-rollback-unavailable', primary_mode: m.execution.primary_mode, rollback_mode: mode } });
            return JSON.stringify({ status: 'BLOCKED', reason: 'command-rollback-unavailable-for-read-only-primary', primary_mode: m.execution.primary_mode, rollback_mode: mode, retry_same_call: false, alternative: 'Use native_revert when OpenCode session-revert is supported, or let a command-capable execution owner register and own the exact rollback. Do not create an unreachable rollback gate.' });
        } if (mode === 'native-revert' && !messageID) {
            appendLedger(m, 'temporary-mutation.registration-blocked', { payload: { reason: 'native-revert-requires-exact-message-id', rollback_mode: mode, session_id: a.session_id ? String(a.session_id) : c?.sessionID } });
            return JSON.stringify({ status: 'BLOCKED', reason: 'native-revert-requires-exact-message-id', rollback_mode: mode, retry_same_call: false, instruction: 'Provide the exact OpenCode message_id that owns this temporary mutation before registering native revert. Hi will not infer a latest/current message after mutation admission.' });
        } if (mode === 'native-revert' && hostCapabilityByID(capabilities.contracts, 'session-revert')?.status !== 'SUPPORTED') {
            ...
            return JSON.stringify({ status: 'USER_ACTION_REQUIRED', reason: 'capability-unavailable', capability: 'session-revert', blocker: marker, detail, alternative: m.execution.primary_mode === 'manager' ? 'Delegate temporary mutation ownership to a command-capable execution owner when an exact rollback is required.' : 'register an exact command rollback when safe and available' });
            ...
            clearCapabilityUnavailable(m, 'session-revert'); return JSON.stringify(registerTemporaryMutation(m, { kind: String(a.kind), description: String(a.description), rollback_command: a.rollback_command ? String(a.rollback_command) : undefined, rollback_mode: mode, session_id: a.session_id ? String(a.session_id) : c?.sessionID, message_id: messageID || undefined })); } });
