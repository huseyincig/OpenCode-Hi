...
export function createRuntimeServices(input) {
    ...
    let browserAvailable = false, browserBootstrapStatus, browserToolReceipt;
    ...
    const ensureBrowserAvailable = async () => {
        if (ports.browserTool)
            try {
                ...
                return { available: false, attempted: browserBootstrapStatus?.attempted, reason: String(error), implementationId: ports.browserTool.implementationId, receiptPath: browserToolReceipt?.receipt_path };
            }
        if (browserAvailable)
            return { available: true, attempted: false };
        if (ports.bootstrapBrowser)
            browserBootstrapStatus = await ports.bootstrapBrowser();
        const health = await browserExecutor.health();
        setBrowserAvailable(health.available);
        return { available: health.available, attempted: browserBootstrapStatus?.attempted, reason: health.available ? undefined : (browserBootstrapStatus?.reason ?? health.reason) };
    };
    ...
    const getEcosystemView = (selectedMcpServers = []) => ecosystemIntegrationView({ hostCapabilities: ports.hostCapabilities, hostConfig: getHostConfig(), selectedMcpServers, operationalToolReceipts: [getBrowserToolReceipt()] });
