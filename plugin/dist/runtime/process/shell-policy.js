...
export function evaluateShellCommand(command) {
    ...
    if (SECRET_SENSITIVE.some(r => r.test(c)))
    ...
    if (CATASTROPHIC_FILESYSTEM.some(r => r.test(c)))
        return { decision: 'USER_ACTION_REQUIRED', command: c, reason: 'catastrophic filesystem mutation requires explicit user action', human_decision_type: 'operational_action', reason_code: 'destructive-filesystem-action' };
    if (IRREVERSIBLE_EXTERNAL.some(r => r.test(c)))
        return { decision: 'USER_ACTION_REQUIRED', command: c, reason: 'irreversible external deletion/destruction requires explicit user action', human_decision_type: 'operational_action', reason_code: 'irreversible-external-action' };
