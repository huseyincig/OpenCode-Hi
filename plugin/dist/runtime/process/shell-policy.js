...
function commandBasename(value) { return (value ?? '').replace(/^.*[\\/]/, '').toLowerCase(); }
function commandView(text) {
    ...
    const executable = commandBasename(tokens[i]);
    if (!executable)
        return;
    return { executable, args: tokens.slice(i + 1), assignments };
}
function interactiveAssessment(view, dialect) {
    if (!view)
        return false;
    const { executable, args } = view, a0 = args[0]?.toLowerCase(), a1 = args[1]?.toLowerCase();
    if (executable === 'select' && dialect === 'posix')
        return true;
    if (executable === 'ssh')
        return !args.some(token => /^-[^-]*T/.test(token));
    if (executable === 'passwd')
        return true;
    if (['npm', 'pnpm', 'yarn'].includes(executable) && a0 === 'login')
        return true;
    if (executable === 'gh' && a0 === 'auth' && a1 === 'login')
        return true;
    if (executable === 'az' && a0 === 'login')
        return true;
    if (executable === 'gcloud' && (a0 === 'login' || a0 === 'auth' && a1 === 'login'))
        return true;
    if (executable === 'aws' && (a0 === 'login' || a0 === 'sso' && a1 === 'login' || a0 === 'configure' && a1 === 'sso'))
        return true;
    return false;
}
...
function secretSensitiveAssessment(view) {
    ...
    for (let i = 0; i < args.length; i++) {
        const token = args[i];
        if (token === '-p' && plainSecretValue(args[i + 1]))
            return true;
        if (/^-p.+/.test(token) && plainSecretValue(token.slice(2)))
            return true;
    }
    return false;
}
...
export function evaluateShellCommand(command) {
    ...
    for (const fragment of projection.fragments) {
        ...
        if (interactiveAssessment(view, fragment.dialect))
        ...
        if (secretSensitiveAssessment(view))
