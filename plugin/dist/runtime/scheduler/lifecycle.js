...
function result(state, accepted, reason, reservation) {
    return { accepted, reason, state, reservation };
}
function exactAttempt(reservation, attempt) { return sameExecutionAttempt(reservation.attempt, attempt); }
function exactHost(reservation, hostExecutionId) {
    return reservation.hostExecutionId === undefined || reservation.hostExecutionId === hostExecutionId;
}
function updateReservation(state, reservation) {
...
function removeReservation(state, id) {
...
export function reduceSchedulerLifecycle(input, event) {
    const state = clone(input);
    if (event.type === 'RESTART_QUARANTINE') {
        const reservations = state.reservations.map(item => ({ ...item, phase: 'RECONCILING', updatedAt: event.at }));
        if (reservations.every((item, index) => item.phase === state.reservations[index]?.phase))
            return result(state, true, 'already-quarantined');
    ...
    if (event.type === 'RESERVE') {
        if (event.missionId !== state.missionId)
            return result(state, false, 'mission-mismatch');
        if (event.attempt.executionUnitId === '')
            return result(state, false, 'execution-unit-missing');
        ...
        if (!sameExecutionAttempt(event.attempt, expected))
            return result(state, false, 'attempt-identity-invalid');
        ...
        if (same)
            return result(state, true, 'already-reserved', same);
        const occupied = state.reservations.find(item => item.executionUnitId === event.attempt.executionUnitId);
        if (occupied)
            return result(state, false, 'execution-unit-already-reserved', occupied);
        const reservation = { reservationId: id, missionId: event.missionId, workNodeId: event.workNodeId, executionUnitId: event.attempt.executionUnitId, workerId: event.workerId, attempt: clone(event.attempt), phase: 'RESERVED', resource: clone(event.resource), ticket: state.nextTicket, reservedAt: event.at, updatedAt: event.at };
    ...
    if (!current)
        return result(state, false, 'reservation-not-found');
    if (!exactAttempt(current, event.attempt))
        return result(state, false, 'stale-attempt', current);
    if (event.type === 'HOST_BOUND') {
        if (current.phase !== 'RESERVED' && current.phase !== 'RUNNING' && current.phase !== 'RECONCILING')
            return result(state, false, `invalid-phase:${current.phase}`, current);
        if (!event.hostExecutionId)
            return result(state, false, 'host-execution-id-missing', current);
        if (current.hostExecutionId && current.hostExecutionId !== event.hostExecutionId)
            return result(state, false, 'host-execution-mismatch', current);
        const next = { ...current, phase: 'RUNNING', hostExecutionId: event.hostExecutionId, updatedAt: event.at };
    ...
    if (event.type === 'RECONCILE') {
        ...
        if (event.outcome === 'UNKNOWN')
            return result(state, true, 'reconcile-unknown', current);
        if (event.outcome === 'NOT_STARTED') {
            if (current.hostExecutionId)
                return result(state, false, 'not-started-conflicts-with-host-binding', current);
            return result(removeReservation(state, current.reservationId), true, 'reconciled-not-started', current);
        }
        if (event.outcome === 'ACTIVE') {
            if (!event.hostExecutionId)
                return result(state, false, 'active-host-execution-required', current);
            if (current.hostExecutionId && current.hostExecutionId !== event.hostExecutionId)
                return result(state, false, 'host-execution-mismatch', current);
            const next = { ...current, phase: 'RUNNING', hostExecutionId: event.hostExecutionId, updatedAt: event.at };
        ...
        if (!event.hostExecutionId)
            return result(state, false, 'terminal-host-execution-required', current);
        if (current.hostExecutionId && current.hostExecutionId !== event.hostExecutionId)
            return result(state, false, 'host-execution-mismatch', current);
        const next = { ...current, phase: 'SETTLING', hostExecutionId: event.hostExecutionId, updatedAt: event.at };
        return result(updateReservation(state, next), true, 'reconciled-terminal', next);
    }
    if (!exactHost(current, event.hostExecutionId))
        return result(state, false, 'stale-host-execution', current);
    if (event.type === 'BEGIN_SETTLEMENT') {
        if (current.phase === 'SETTLING')
            return result(state, true, 'already-settling', current);
        if (current.phase !== 'RUNNING' && current.phase !== 'RECONCILING')
            return result(state, false, `invalid-phase:${current.phase}`, current);
        if (current.hostExecutionId && !event.hostExecutionId)
            return result(state, false, 'host-execution-required', current);
        const next = { ...current, phase: 'SETTLING', updatedAt: event.at };
 ...
 */
export function planSchedulerAdmissions(snapshot, state, limit = Number.POSITIVE_INFINITY) {
    const validation = validateWorkGraph(snapshot.graph);
    if (!validation.ok)
        return { ok: false, reasons: validation.reasons, executionUnitIds: [] };
    if (state.missionId !== snapshot.graph.missionId)
        return { ok: false, reasons: ['scheduler-mission-mismatch'], executionUnitIds: [] };
    const reserved = new Set(state.reservations.map(item => item.executionUnitId)), selected = [], selectedSet = new Set();
    ...
    while (selected.length < limit) {
        ...
        const candidates = decision.units
            .filter(item => {
            if (item.disposition !== 'RUNNABLE' || reserved.has(item.executionUnitId) || selectedSet.has(item.executionUnitId))
                return false;
            const unit = units.get(item.executionUnitId), node = unit ? nodes.get(unit.workNodeId) : undefined;
            return node?.status === 'created' || node?.status === 'queued';
        })
            .sort((a, b) => {
...
export function reserveSchedulerUnit(snapshot, state, input) {
    const existing = state.reservations.find(item => item.executionUnitId === input.executionUnitId);
    if (existing && sameExecutionAttempt(existing.attempt, input.attempt))
        return result(clone(state), true, 'already-reserved', clone(existing));
    const plan = planSchedulerAdmissions(snapshot, state);
    if (!plan.ok)
        return result(clone(state), false, `invalid-snapshot:${plan.reasons.join(',')}`);
    if (!plan.executionUnitIds.includes(input.executionUnitId))
        return result(clone(state), false, 'unit-not-admitted');
    const unit = snapshot.graph.executionUnits.find(item => item.id === input.executionUnitId);
    if (!unit)
        return result(clone(state), false, 'execution-unit-not-found');
    if (input.attempt.executionUnitId !== unit.id)
        return result(clone(state), false, 'attempt-unit-mismatch');
