...
function exactSkill(root, name) { const raw = join(root, name, 'SKILL.md'); if (!existsSync(raw))
    return undefined; try {
    const actual = canonical(raw);
    if (!confined(root, actual))
        return { path: actual, valid: false };
    const fm = parseFrontmatter(readFileSync(actual, 'utf8'));
    return { path: actual, valid: fm.name === name && Boolean(fm.description) };
}
catch {
    return { path: resolve(raw), valid: false };
} }
function configuredSkillPaths(hostConfig) { const skills = hostConfig.skills && typeof hostConfig.skills === 'object' && !Array.isArray(hostConfig.skills) ? hostConfig.skills : {}; return [...new Set((Array.isArray(skills.paths) ? skills.paths : []).filter((x) => typeof x === 'string' && x.trim().length > 0).map(x => canonical(x.trim())))]; }
function nativeSkillRoots(projectRoot, hiRoot, hostConfig) { const home = process.env.HOME ?? process.env.USERPROFILE ?? '', configDir = process.env.OPENCODE_CONFIG_DIR ? resolve(process.env.OPENCODE_CONFIG_DIR) : join(home, '.config', 'opencode'), raw = [{ root: join(projectRoot, '.opencode', 'skills'), provider: 'project' }, { root: join(projectRoot, '.claude', 'skills'), provider: 'project' }, { root: join(projectRoot, '.agents', 'skills'), provider: 'project' }, { root: join(hiRoot, 'skills'), provider: 'hi' }, { root: join(configDir, 'skills'), provider: 'personal' }, { root: join(home, '.claude', 'skills'), provider: 'personal' }, { root: join(home, '.agents', 'skills'), provider: 'personal' }, ...configuredSkillPaths(hostConfig).map(root => ({ root, provider: 'personal' }))], out = [], seen = new Set(); for (const item of raw) {
    const root = canonical(item.root);
    if (seen.has(root))
        continue;
    seen.add(root);
    out.push({ root, provider: item.provider });
} return out; }
/**
 ...
 */
export function methodologySkillCandidates(requestedNames, projectRoot, hiRoot, hostConfig, catalog = builtinMethodologyCatalog()) {
    const roots = nativeSkillRoots(projectRoot, hiRoot, hostConfig), out = [];
    for (const name of [...new Set(requestedNames)]) {
        ...
        if (intended)
            out.push({ name, provider: policy.provider, path: intended.path, valid: intended.valid, enabled: true, orchestrationRisk: false, canonicalForMethodology: true });
        const seen = new Set(intended ? [intended.path] : []);
        for (const root of roots) {
            const hit = exactSkill(root.root, name);
            if (!hit || seen.has(hit.path))
                continue;
            seen.add(hit.path);
            out.push({ name, provider: root.provider, path: hit.path, valid: hit.valid, enabled: true, orchestrationRisk: true, canonicalForMethodology: false });
        }
    }
    return out;
}
