...
function jsonVersion(path) { try {
    const j = JSON.parse(readFileSync(path, 'utf8'));
    return typeof j?.version === 'string' ? j.version.trim() : undefined;
}
catch {
    return undefined;
} }
function sha256File(path) { try {
    return createHash('sha256').update(readFileSync(path)).digest('hex');
}
catch {
    return undefined;
} }
function digestManifestInputs(root, files) { try {
    const h = createHash('sha256'), base = resolve(root);
    for (const rel0 of Object.keys(files).sort()) {
        const rel = rel0.replace(/\\/g, '/').replace(/^\.\//, '');
        let full = resolve(base, rel);
        if (!existsSync(full) && /^SBOM-[^/]+\.json$/.test(rel))
            full = resolve(base, 'dist', rel);
        if (full !== base && !full.startsWith(base + '/') && !full.startsWith(base + '\\'))
            return undefined;
        const actual = sha256File(full);
        if (!actual || actual !== files[rel0])
            return undefined;
        h.update(rel + '\0');
        h.update(actual + '\0');
    }
...
function packageJson(root) { try {
    return JSON.parse(readFileSync(join(root, 'package.json'), 'utf8'));
}
catch {
    return undefined;
} }
function packageStateHash(root, files) { try {
    const base = resolve(root), h = createHash('sha256');
    for (const rel0 of [...new Set(files.map(String))].sort()) {
        const rel = rel0.replace(/\\/g, '/').replace(/^\.\//, '');
...
function packageLockVersion(root) { for (const path of [join(root, 'package-lock.json'), join(root, 'plugin', 'package-lock.json')]) {
    if (!existsSync(path))
        continue;
    try {
        const j = JSON.parse(readFileSync(path, 'utf8'));
        const rootVersion = j?.packages?.['']?.version;
        return typeof rootVersion === 'string' ? rootVersion : typeof j?.version === 'string' ? j.version : undefined;
    }
    catch {
        return undefined;
    }
...
function validateSupplyChain(root, manifest) { const issues = [], sc = manifest?.supply_chain, g = dependencyGraph(root); if (manifest?.schema < 5 || sc?.schema !== 1 || typeof sc?.dependency_graph_sha256 !== 'string' || typeof sc?.sbom !== 'string' || typeof sc?.third_party_notices_sha256 !== 'string')
    ...
    issues.push('sbom-missing-or-invalid');
} const notices = join(root, 'THIRD_PARTY_NOTICES.md'), text = existsSync(notices) ? readFileSync(notices, 'utf8') : ''; if (sha256File(notices) !== sc.third_party_notices_sha256)
    issues.push('third-party-notices-drift'); for (const d of g?.direct ?? []) {
    if (!text.includes('`' + d.name + '`') || (d.license !== 'UNKNOWN' && !text.includes(d.license)))
        issues.push('third-party-notices-incomplete:' + d.name);
} return issues; }
function changelogHasVersion(path, version) { try {
    return new RegExp(`^##\\s+(?:\\[)?v?${version.replace(/[.*+?^${}()|[\\]\\]/g, '\\$&')}(?:\\])?(?:\\s|$)`, 'mi').test(readFileSync(path, 'utf8'));
}
catch {
    return false;
} }
function releaseAssets(command, root) { const t = tokens(command); if (t[0] !== 'gh' || t[1] !== 'release' || t[2] !== 'create')
    return []; const valueFlags = new Set(['--target', '--title', '--notes', '--notes-file', '--repo', '-R', '--discussion-category']); let tagSeen = false; const out = []; for (let i = 3; i < t.length; i++) {
    const x = t[i];
    if (valueFlags.has(x)) {
        i++;
        continue;
    ...
    const path = /^(?:[A-Za-z]:[\\/]|\/)/.test(x) ? x : resolve(root, x);
    if (existsSync(path))
        out.push(path);
} return out; }
function inspectReleaseQuality(root, command) {
    const tag = parseReleaseTag(command), version = releaseVersion(tag), issues = [], assets = [];
    if (!version) {
        issues.push('release-tag-version-unparseable');
    ...
    if (existsSync(vf) && readFileSync(vf, 'utf8').trim() !== version)
        issues.push('VERSION-mismatch');
    const pv = jsonVersion(pkg);
    if (pv && pv !== version)
        issues.push('package-version-mismatch');
    const ppv = jsonVersion(ppkg);
    if (ppv && ppv !== version)
        issues.push('plugin-package-version-mismatch');
    if (existsSync(cl) && !changelogHasVersion(cl, version))
        issues.push('CHANGELOG-version-missing');
    let manifest;
    const mp = join(root, 'dist', `RELEASE-MANIFEST-${version}.json`);
    if (existsSync(mp))
        try {
            manifest = JSON.parse(readFileSync(mp, 'utf8'));
        }
        catch {
            issues.push('release-manifest-invalid');
        }
    if (manifest) {
        const prov = manifest?.provenance, files = manifest?.files;
        if (manifest?.schema < 5 || prov?.schema !== 1 || prov?.builder !== 'scripts/release-build.py' || prov?.deterministic_zip !== true || typeof prov?.inputs_sha256 !== 'string' || !files || typeof files !== 'object')
            issues.push('release-provenance-missing');
        else {
            const current = digestManifestInputs(root, files);
            if (!current || current !== prov.inputs_sha256)
                issues.push('release-provenance-source-drift');
        }
        issues.push(...validateSupplyChain(root, manifest));
    }
    for (const path of releaseAssets(command, root)) {
        const digest = sha256File(path), a = { path, sha256: digest, manifest_match: undefined };
        if (manifest && basename(path) === manifest.archive) {
            a.manifest_match = typeof manifest.archive_sha256 === 'string' && digest === manifest.archive_sha256;
            if (!a.manifest_match)
                issues.push(`asset-sha256-mismatch:${basename(path)}`);
        }
        assets.push(a);
    }
...
function parseTagPushExpectation(command) {
    ...
    if (!spec)
        return undefined;
    const rhs = (spec.includes(':') ? spec.split(':').pop() : spec) ?? '';
    if (rhs.startsWith('refs/tags/'))
        return { remote, tag: rhs.replace(/^refs\/tags\//, '') };
    if (/^v?\d+\.\d+\.\d+(?:[-+][0-9A-Za-z.-]+)?$/.test(rhs))
...
function ghReleasePositional(command, verb) { const p = ghCommandParts(command); if (p.sub !== 'release' || p.rest[0] !== verb)
    ...
    if (x.startsWith('--') || x === '--latest' || x === '--web')
        continue;
    if (x.startsWith('-'))
        continue;
    return x;
} return undefined; }
function parseReleaseTag(command) { return ghReleasePositional(command, 'create'); }
function parseReleaseTarget(command) { const t = tokens(command); for (let i = 0; i < t.length; i++) {
    if (t[i] === '--target' && t[i + 1])
        return t[i + 1];
    if (t[i].startsWith('--target='))
...
function parseLsRemote(command, output) {
    ...
    for (const line of text.split(/\r?\n/)) {
        const m = line.trim().match(/^([0-9a-f]{40})\s+(.+)$/i);
        if (m)
            return { remote, ref: m[2], hash: m[1] };
    }
...
function parseRemoteTagProbe(command, output) {
    ...
    const tag = base.replace(/^refs\/tags\//, '');
    if (!tag)
        return undefined;
    const text = typeof output?.stdout === 'string' ? output.stdout : typeof output?.output === 'string' ? output.output : '';
    let direct_hash, peeled_hash;
    for (const line of text.split(/\r?\n/)) {
        const m = line.trim().match(/^([0-9a-f]{40})\s+(.+)$/i);
        if (!m)
            continue;
...
function parseReleaseView(command, output) {
    const tag = ghReleasePositional(command, 'view');
    if (!tag)
        return undefined;
    const text = typeof output?.stdout === 'string' ? output.stdout : typeof output?.output === 'string' ? output.output : '';
    try {
        const j = JSON.parse(text);
        return { tag: typeof j?.tagName === 'string' ? j.tagName : tag, target: typeof j?.targetCommitish === 'string' ? j.targetCommitish : undefined, assets: Array.isArray(j?.assets) ? j.assets.map((a) => ({ name: String(a?.name ?? a?.label ?? ''), size: typeof a?.size === 'number' ? a.size : undefined })).filter((a) => a.name) : undefined };
    }
    catch {
        return undefined;
    }
...
function parseRegistryView(command, output) { const p = npmLikeCommandParts(command); if (!['npm', 'pnpm'].includes(p.exe ?? '') || p.sub !== 'view')
    ...
    const j = JSON.parse(text);
    const dist = j?.dist ?? {};
    let name = typeof j?.name === 'string' ? j.name : undefined, version = typeof j?.version === 'string' ? j.version : undefined;
    if (!name || !version) {
        const m = spec.match(/^(@[^/]+\/[^@]+|[^@]+)@(.+)$/);
        if (m) {
            name ??= m[1];
    ...
    return { name, version, integrity: typeof dist?.integrity === 'string' ? dist.integrity : typeof j?.['dist.integrity'] === 'string' ? j['dist.integrity'] : undefined, shasum: typeof dist?.shasum === 'string' ? dist.shasum : typeof j?.['dist.shasum'] === 'string' ? j['dist.shasum'] : undefined };
}
catch {
    return undefined;
} }
...
function reevaluatePushRemote(m) {
    ...
    if (!p.expected_remote && p.observed_remote)
        p.expected_remote = p.observed_remote;
    if (!p.expected_ref && p.observed_ref)
        p.expected_ref = p.observed_ref;
    const ok = !!p.expected_remote && !!p.expected_ref && p.local_head.toLowerCase() === p.remote_hash.toLowerCase() && p.observed_remote === p.expected_remote && p.observed_ref === p.expected_ref;
    p.remote_verified = ok;
    p.remote_verified_at = ok ? Date.now() : undefined;
    if (ok) {
        clearReleaseBlockers(m, 'release-chain:push-remote-');
        appendLedger(m, 'release-chain.push.remote-verified', { payload: { remote: p.expected_remote, ref: p.expected_ref, hash: p.local_head } });
    }
    else {
        addBlocker(m, 'push-remote-drift');
        appendLedger(m, 'release-chain.push.remote-drift', { payload: { expected_remote: p.expected_remote, expected_ref: p.expected_ref, local_head: p.local_head, observed_remote: p.observed_remote, observed_ref: p.observed_ref, remote_hash: p.remote_hash } });
    }
}
function reevaluateTagPushRemote(m) {
    ...
    t.remote_verified = ok;
    t.remote_verified_at = ok ? Date.now() : undefined;
    if (ok) {
        clearReleaseBlockers(m, 'release-chain:tag-push-');
        appendLedger(m, 'release-chain.tag-push.remote-verified', { payload: { remote: t.expected_remote, tag: t.expected_tag, commit: t.expected_commit, annotated: !!t.peeled_tag_hash } });
    }
    else if (tagHash) {
        addBlocker(m, 'tag-push-remote-drift');
...
function reevaluatePackageRemote(m) { const p = m.release_chain?.package; if (!p || p.outcome !== 'success' || !p.name || !p.version || !p.registry_version)
    ...
    addBlocker(m, 'package-remote-drift');
...
function reevaluateReleaseRemote(m) {
    ...
    r.remote_verified = ok;
    r.remote_verified_at = ok ? Date.now() : undefined;
    if (ok) {
        clearReleaseBlockers(m, 'release-chain:release-remote-');
        appendLedger(m, 'release-chain.release.remote-verified', { payload: { tag: r.expected_tag, commit: r.expected_commit, remote: r.expected_remote, annotated: !!r.peeled_tag_hash, target: r.observed_target, assets_verified: r.assets_verified, assets: r.observed_assets?.map(a => a.name) } });
    }
    else if (r.view_verified !== undefined || tagHash) {
        addBlocker(m, 'release-remote-drift');
...
export function noteLocalReleaseMutation(m, command, success) {
...
export function assertReleaseChainPrecondition(m, command, projectRoot) {
    if (isPackagePublish(command)) {
        ...
        if (versionFile && version && versionFile !== version)
            issues.push('package-version-mismatch');
        if (lockVersion && version && lockVersion !== version)
            issues.push('package-lock-version-mismatch');
        if (!pp?.pack_verified_at || pp.name !== name || pp.version !== version || !pp.pack_state_hash)
            issues.push('package-pack-unverified');
        else if (packageStateHash(projectRoot, pp.pack_files ?? []) !== pp.pack_state_hash)
            issues.push('package-surface-changed-after-pack');
        if (issues.length) {
        ...
        clearReleaseBlockers(m, 'release-chain:package-version-');
        clearReleaseBlockers(m, 'release-chain:package-lock-');
        clearReleaseBlockers(m, 'release-chain:package-identity-');
        appendLedger(m, 'release-chain.package.preflight-verified', { payload: { name, version, pack_integrity: pp?.pack_integrity, pack_shasum: pp?.pack_shasum, pack_state_hash: pp?.pack_state_hash } });
        return;
    }
    ...
    if (projectRoot) {
        const q = inspectReleaseQuality(projectRoot, command);
        if (!q.ok) {
    ...
    if (isGitPush(command)) {
        ...
        if (tagExp) {
            ...
            if (outcome !== 'success')
                ...
                addBlocker(m, 'tag-push-remote-unverified');
            appendLedger(m, `release-chain.tag-push.${outcome}`, { payload: { command: c.slice(0, 180), expected_remote: tagExp.remote, expected_tag: tagExp.tag, expected_commit: commit } });
            return;
        }
        ...
        if (outcome !== 'success')
            ...
            addBlocker(m, 'push-remote-unverified');
        appendLedger(m, `release-chain.push.${outcome}`, { payload: { command: c.slice(0, 180), expected_remote: exp?.remote, expected_ref: exp?.ref } });
        return;
    }
    if (isPackagePublish(command)) {
        ...
        if (outcome !== 'success')
            ...
            addBlocker(m, 'package-remote-unverified');
        appendLedger(m, `release-chain.package.${outcome}`, { payload: { command: c.slice(0, 180), name: pkg?.name, version: pkg?.version, pack_integrity: pkg?.pack_integrity } });
        return;
    }
    if (isReleaseCreate(command)) {
        ...
        if (outcome !== 'success')
            ...
            addBlocker(m, 'release-remote-unverified');
...
export function recordRemoteReleaseVerification(m, command, output, projectRoot) {
    const exit = (() => { for (const v of [output?.metadata?.exit, output?.exit]) {
        if (typeof v === 'number')
            return v;
    ...
    if (registry && m.release_chain?.package) {
        ...
        appendLedger(m, 'release-chain.package.registry-probe', { payload: registry });
        reevaluatePackageRemote(m);
        return;
    }
    ...
    if (tagProbe) {
        ...
        if (tp && (!tp.expected_tag || tp.expected_tag === tagProbe.tag)) {
            ...
            tp.peeled_tag_hash = tagProbe.peeled_hash;
            appendLedger(m, 'release-chain.tag-push.tag-probe', { payload: { remote: tagProbe.remote, tag: tagProbe.tag, direct_hash: tagProbe.direct_hash, peeled_hash: tagProbe.peeled_hash } });
        ...
        if (m.release_chain?.release) {
            ...
            appendLedger(m, 'release-chain.release.tag-probe', { payload: { remote: tagProbe.remote, tag: tagProbe.tag, direct_hash: tagProbe.direct_hash, peeled_hash: tagProbe.peeled_hash } });
            reevaluateReleaseRemote(m);
            handled = true;
        }
        if (handled)
            return;
    ...
    if (remote && m.release_chain?.push) {
        ...
        appendLedger(m, 'release-chain.push.remote-probe', { payload: { remote: remote.remote, ref: remote.ref, hash: remote.hash } });
        reevaluatePushRemote(m);
        return;
    }
    ...
    if (rv && m.release_chain?.release) {
        ...
        r.view_verified = !!r.expected_tag && rv.tag === r.expected_tag && (!r.expected_target || !rv.target || (hex40(r.expected_target) ? rv.target.toLowerCase() === r.expected_target.toLowerCase() : rv.target === r.expected_target));
        appendLedger(m, r.view_verified ? 'release-chain.release.view-verified' : 'release-chain.release.view-drift', { payload: { expected_tag: r.expected_tag, expected_target: r.expected_target, observed_tag: rv.tag, target: rv.target, assets: rv.assets?.map(a => a.name) } });
        reevaluateReleaseRemote(m);
    }
}
