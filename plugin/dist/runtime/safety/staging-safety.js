...
export function recordPreexistingUserBaseline(m, baseline) {
    if (!baseline || m.vcs.preexisting_user_baseline_captured)
        return;
    m.vcs.preexisting_user_changes = { ...baseline };
    m.vcs.preexisting_user_baseline_captured = true;
...
export function recordStagingInspection(m, command, output) {
    ...
    m.vcs.staging_safety = { verified_files: files, verified_at: Date.now(), source: command.slice(0, 180) };
    if (m.vcs.git_topology_pending && !m.vcs.git_topology_pending.ownership_captured) {
        const conflict = m.vcs.git_topology_pending.conflict_files ?? [];
        m.vcs.git_topology_owned_files = [...new Set([...files, ...conflict])];
        m.vcs.git_topology_pending.ownership_captured = true;
        appendLedger(m, 'git.topology.staged-owned', { payload: { files: m.vcs.git_topology_owned_files.slice(0, 80), operation: m.vcs.git_topology_pending.command.slice(0, 180), policy: 'first-cached-set-plus-known-conflicts' } });
    }
...
export function recordGitStatusInspection(m, command, output) {
    ...
    m.vcs.git_topology_safety = { clean: files.length === 0, verified_files: files, verified_at: Date.now(), source: command.slice(0, 180) };
    appendLedger(m, 'git.worktree.inspected', { payload: { clean: files.length === 0, files: files.slice(0, 80), count: files.length } });
}
export function invalidateGitTopologyProof(m) { m.vcs.git_topology_safety = undefined; }
export function beginGitTopologyMutation(m, command) { m.vcs.git_topology_pending = { command: command.slice(0, 300), started_at: Date.now() }; m.vcs.git_topology_owned_files = []; appendLedger(m, 'git.topology.started', { payload: { command: command.slice(0, 180) } }); }
export function completeGitTopologyMutation(m, command, success, text) { if (!isGitTopologyMutation(command))
    return; if (success) {
    m.execution.blockers = m.execution.blockers.filter(b => !b.startsWith('git-topology-conflict:'));
    ...
    if (!m.execution.blockers.includes(blocker))
        m.execution.blockers.push(blocker);
    const conflicts = [...text.matchAll(/(?:conflict[^\n]*?in|merge conflict in)\s+([^\s\r\n]+)/ig)].map(x => normFile(String(x[1] ?? '').replace(/[,:;]+$/, ''))).filter(Boolean);
    if (m.vcs.git_topology_pending)
        m.vcs.git_topology_pending.conflict_files = [...new Set([...(m.vcs.git_topology_pending.conflict_files ?? []), ...conflicts])];
...
export function clearGitTopologyOwnershipAfterCommit(m) { m.vcs.git_topology_pending = undefined; m.vcs.git_topology_owned_files = []; m.execution.blockers = m.execution.blockers.filter(b => !b.startsWith('git-topology-conflict:')); appendLedger(m, 'git.topology.reconciled', { payload: { reason: 'commit-completed' } }); }
export function assertSafeGitMutation(m, command) {
    ...
    if (isGitTopologyMutation(command)) {
        ...
        const proof = m.vcs.git_topology_safety;
    ...
    if (!isGitCommit(command))
        return;
    const proof = m.vcs.staging_safety;
    if (!proof || Date.now() - proof.verified_at > 120000) {
    ...
    const owned = new Set([...(m.vcs.changed_files ?? []), ...(m.vcs.git_topology_owned_files ?? [])].map(normFile));
...
export function invalidateStagingProof(m) { m.vcs.staging_safety = undefined; }
