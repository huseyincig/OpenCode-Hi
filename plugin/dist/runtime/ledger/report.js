...
export function compactLedgerReport(m, limit = 40) {
    ...
    return {
        ...
        evidence: { fresh: hasFreshPassedEvidence(m.execution.evidence.items), items: m.execution.evidence.items.filter(e => !e.invalidated_at).map(e => ({ id: e.id, kind: e.kind, summary: e.summary, outcome: e.outcome, pass: e.pass, scope: (e.scope ?? []).slice(0, 20), source: e.source })) },
