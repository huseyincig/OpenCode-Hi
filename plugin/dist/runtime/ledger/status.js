...
export function userMissionStatus(m) {
    ...
    const evidenceFresh = hasFreshPassedEvidence(m.execution.evidence.items);
    let next = 'continue';
    if (m.identity.status === 'waiting-user' || m.authority.pending_permissions > 0 || m.authority.authority?.pending || m.authority.authority?.executing)
        ...
        next = 'wait';
    else if (open === 0 && evidenceFresh && !m.execution.blockers.length)
        next = 'complete';
    else if (!evidenceFresh && m.execution.verification_policy.requiredKinds.length)
        next = 'verify';
    ...
    return { status: m.identity.status, active_workers: active, open_obligations: open, evidence: evidenceFresh ? 'fresh' : 'stale', blockers: m.execution.blockers.length, next_action: next, human_decision: human };
}
