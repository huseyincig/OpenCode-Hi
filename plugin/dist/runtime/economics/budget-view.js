...
function axis(input) { return { ...input, status: input.measurement === 'unavailable' || input.enforcement === 'unavailable' ? 'unavailable' : input.enforcement === 'observed-only' ? 'observed' : input.limit !== undefined && input.used >= input.limit ? 'exhausted' : 'within' }; }
