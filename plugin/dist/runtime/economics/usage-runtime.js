...
export function bindWorkerUsageObservation(m, worker, usage, at = Date.now()) {
    if (!worker.session_id || worker.attempt < 1)
        return undefined;
...
export function workerExactTokenUsage(worker) {
...
export function workerDerivedOpenCodeCost(worker) { return (worker.usage_observations ?? []).reduce((sum, item) => sum + (item.monetary?.source === 'opencode-calculated' ? item.monetary.usd : 0), 0); }
