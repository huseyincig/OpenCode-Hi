...
export class TeamRuntime {
    ...
    async startMember(m, t, role, overrideModel, overrideVariant) {
        const readOnly = isHiReadOnlyChildRole(role), reviewScope = m.changed_files.length ? m.changed_files : (m.intent.likelyTargets ?? []), started = await this.tasks.start(m, { objective: `${t.objective}
Team perspective: ${role}. Report findings to parent; do not create another team.`, role, category: m.risk === 'high' ? 'critical' : 'deep', scope: readOnly ? reviewScope : (m.intent.likelyTargets ?? []), constraints: ['parent-mediated team', 'no nested team', 'bounded scope', 'compact evidence'], model: overrideModel, modelVariant: overrideVariant });
    ...
    async create(m, objective, members, memberModels) { if (!this.enabled())
        ...
        throw new Error('Nested or second active team is not allowed'); const l = this.limits(), requested = [...new Set(members.map(x => x.trim()).filter(Boolean))], invalid = requested.filter(x => !validTeamRole(x)); if (invalid.length)
        throw new Error(`Unknown Team Mode role(s): ${invalid.join(', ')}`); const unique = requested.slice(0, l.maxMembers); if (unique.length < 2)
        throw new Error('Team Mode requires at least two distinct members'); const now = Date.now(), team = { id: uid('team'), mission_id: m.mission_id, mission_generation: m.generation, objective, status: 'active', members: unique, worker_ids: [], member_workers: {}, created_at: now, expires_at: now + l.maxWallMs }; this.#teams.set(team.id, team); m.execution_mode = 'team'; appendLedger(m, 'team.created', { payload: { team_id: team.id, members: unique, expires_at: team.expires_at, member_models: memberModels ? Object.keys(memberModels).length : 0 } }); try {
