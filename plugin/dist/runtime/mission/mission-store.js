...
function explicitTestAuthoringRequested(text) {
    const normalized = text.toLowerCase().replace(/[\u2018\u2019]/g, "'").replace(/\s+/g, ' ').trim();
    if (!normalized || explicitTestMutationForbidden(normalized))
        return false;
    if (explicitTestFirstRequested(normalized))
        return true;
    return [
...
function testLikePath(path) { return /(^|\/)(?:test|tests|__tests__)(?:\/|$)|\.(?:test|spec)\.[^/]+$/i.test(path); }
...
function repositoryBoundAssessment(root, userText, assessment) {
    ...
    const retain = (target) => { if (/^https?:\/\//i.test(target))
...
function reconciledIntentMethodologySignals(assessment, userText = '') {
    ...
    if (diagnosisOwnsRootCause) {
        suppressed.add('intent.debugging');
        runtimeSuppressed.push('intent.debugging');
    }
    else if (boundedDirectBugFix && assessment.intent_signals.includes('intent.debugging') && (assessment.ambiguity === 'none' || !debuggingBackedByDiagnosis)) {
        suppressed.add('intent.debugging');
        runtimeSuppressed.push('intent.debugging');
    }
...
function groundedIntentAuthority(assessment, userText) {
    ...
    let requiredCapabilities = assessment.required_capabilities.filter(cap => cap !== 'test-authoring');
    if (testAuthoringRequested || signals.active.includes('intent.tdd'))
        requiredCapabilities = [...requiredCapabilities, 'test-authoring'];
    const capabilityRequestUnits = { ...(assessment.capability_request_units ?? {}) };
    if (!testAuthoringRequested && !signals.active.includes('intent.tdd'))
        delete capabilityRequestUnits['test-authoring'];
    ...
    let grounded = changed ? { ...assessment, required_capabilities: requiredCapabilities, capability_request_units: capabilityRequestUnits, ...(assessment.mutation_targets === undefined ? {} : { mutation_targets: mutationTargets ?? [] }) } : assessment;
    ...
    if (grounded.scope === 'multi-file' && grounded.ambiguity === 'none' && grounded.dependency_class === 'sequential' && materialWithoutVerifierTests.length === 1 && grounded.likely_verification.length > 0)
        grounded = { ...grounded, scope: 'local', dependency_class: 'independent' };
...
export class MissionStore {
    ...
    applyInitialSemanticAssessment(sessionID, assessment) {
        ...
        if (!assessment.material) {
            ...
            appendLedger(m, 'semantic.non-material', { payload: { revision: m.identity.semantic_assessment.revision, source: m.identity.semantic_assessment.source, taskKind: m.identity.intent.taskKind, scope: m.identity.intent.scope, risk: m.identity.intent.risk } });
            syncMissionGates(m);
        ...
        m.execution.adaptive_execution = { path: decision.executionPath, reasons: decision.executionReasons };
        m.execution.topology = { mode: decision.topology.mode, parallelism: decision.topology.parallelism, reason: decision.topology.reason };
        m.continuation.continuation_budget = decision.continuationBudget;
        appendLedger(m, 'semantic.decision', { payload: { version: decision.version, path: decision.executionPath, topology: decision.topology.mode, parallelism: decision.topology.parallelism, primary: decision.primary, child_roles: decision.childRoles, model_class: decision.modelClass, fresh_reviewer: decision.assurance.freshReviewerRequired, isolation: decision.isolation.intent, provider_surface_phase: decision.providerSurfacePhase, reasons: decision.reasons } });
        m.methodology.methodology_needs = [];
        ...
        if (['active', 'waiting-user'].includes(m.identity.status)) {
            ...
            for (const w of m.execution.workers) {
                ...
                if (legacyHostAbort) {
                    ...
                    t.status = 'waiting';
                    t.result = { status: 'NEEDS_CONTEXT', summary: 'A legacy parent-host interruption was previously persisted as terminal failure; reconcile the existing child session before continuing.', changed_files: historicalResult.changed_files ?? [], evidence: historicalResult.evidence ?? [], open_issues: historicalResult.open_issues ?? [], needs_context: ['runtime-host-interruption-reconcile'] };
                    appendLedger(m, 'worker.host-interruption.legacy-restored', { task_id: t.id, worker_id: w.id, payload: { session_id: w.session_id, attempt: w.attempt, generation: w.generation_at_spawn, policy: 'reopen-exact-pre-fix-abort-owner-for-native-reconciliation', historical_result_status: historicalResult.status, historical_result_summary: historicalResult.summary, historical_open_issues: historicalResult.open_issues } });
                }
            }
            for (const w of m.execution.workers) {
