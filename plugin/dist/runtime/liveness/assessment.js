...
export function lastDurableProgressAt(m) {
    const seen = new Set();
    let latest = Math.max(0, Number(m.identity.created_at) || 0);
    for (const event of [...m.execution.ledger].sort((a, b) => a.at - b.at)) {
        const key = durableProgressKey(event, m.continuation.generation);
        if (!key || seen.has(key))
            continue;
        seen.add(key);
...
function toolInflight(m, host) {
    ...
    for (const event of m.execution.ledger) {
        if (event.type !== 'tool.operation-started' && event.type !== 'tool.operation-result')
            continue;
        if (Number(event.payload?.generation) !== m.continuation.generation)
            continue;
        const op = String(event.payload?.operation_id ?? ''), tool = String(event.payload?.tool ?? '');
        if (!op || NON_PROGRESS_TOOL_NAMES.has(tool))
            continue;
        const current = states.get(op) ?? { started: 0, session_id: String(event.payload?.session_id ?? '') };
        if (event.type === 'tool.operation-started') {
            current.started = Math.max(current.started, event.at);
            current.session_id = String(event.payload?.session_id ?? current.session_id);
        }
        else
            current.result = Math.max(current.result ?? 0, event.at);
        states.set(op, current);
    }
    let unknown = false;
    for (const state of states.values())
        if (state.started > (state.result ?? 0)) {
            if (latestRestore && state.started < latestRestore) {
                unknown = true;
                continue;
            }
            const status = host[state.session_id] ?? 'unknown';
    ...
    for (const worker of m.execution.workers) {
        ...
        if (!worker.session_id) {
            if (['starting', 'busy'].includes(worker.status)) {
                states.push('UNKNOWN');
                reasons.push(`worker-session-unverified:${worker.id}`);
            }
            continue;
        }
        ...
        if (status === 'busy' || status === 'retry') {
            states.push('YES');
            reasons.push(`host-session-${status}:${worker.id}`);
            continue;
        }
        if (status === 'unknown') {
            states.push('UNKNOWN');
            reasons.push(`host-session-unknown:${worker.id}`);
            continue;
        }
        if (worker.restart_reconcile_pending || ['starting', 'busy'].includes(worker.status)) {
            states.push('UNKNOWN');
            reasons.push(`host-session-idle-reconcile:${worker.id}`);
        }
    }
    return { state: mergeInflight(states), reasons };
}
function processInflight(m, observed) {
    const states = [], reasons = [];
    for (const process of m.execution.processes) {
        if (process.status !== 'RUNNING')
            continue;
        const live = observed[process.process_id];
        if (!live || !live.owner_verified) {
            states.push('UNKNOWN');
            reasons.push(`process-owner-unverified:${process.process_id}`);
            continue;
        }
        if (live.status === 'running') {
            states.push('YES');
            reasons.push(`process-running:${process.process_id}`);
        }
        else if (live.status === 'unknown') {
            states.push('UNKNOWN');
            reasons.push(`process-state-unknown:${process.process_id}`);
        }
    }
    return { state: mergeInflight(states), reasons };
}
export function assessMissionLiveness(m, observation = {}) {
    ...
    if (tool === 'YES')
        ...
        reasons.push('tool-operation-reconcile');
    if (inflight === 'UNKNOWN')
        return { state: 'RECONCILE', inflight, last_durable_progress_at: last, no_progress_ms: noProgress, no_progress_window_ms: window, destructive_recovery_allowed: false, reasons };
    if (inflight === 'YES')
        return { state: 'ACTIVE', inflight, last_durable_progress_at: last, no_progress_ms: noProgress, no_progress_window_ms: window, destructive_recovery_allowed: false, reasons };
...
export function recordToolOperationProgress(m, identity, phase, at = Date.now()) {
    if (identity.generation !== m.continuation.generation || !identity.operation_id.trim() || NON_PROGRESS_TOOL_NAMES.has(identity.tool))
        return false;
    const type = phase === 'started' ? 'tool.operation-started' : 'tool.operation-result';
    if (m.execution.ledger.some(e => e.type === type && String(e.payload?.operation_id ?? '') === identity.operation_id && Number(e.payload?.generation) === identity.generation))
        return false;
    const event = appendLedger(m, type, { payload: { operation_id: identity.operation_id, session_id: identity.session_id, tool: identity.tool, generation: identity.generation } });
    event.at = at;
    return true;
}
export function recordAssistantProgress(m, input) {
    if (input.generation !== m.continuation.generation)
        return false;
    ...
    if (!input.message_id && !input.output_tokens && !input.reasoning_tokens && !input.tool_calls && !input.text_chars)
        return false;
    if (m.execution.ledger.some(e => e.type === 'assistant.progress-observed' && e.payload?.activity_key === activityKey))
        return false;
...
export function replacementExecutionAdmission(state) { return state === 'QUIESCENT' || state === 'VERIFIED_ABORTED'; }
