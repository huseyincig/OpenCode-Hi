...
function legacyRoutingDiagnostics(hostModels, hostRouting, projectFields) {
    const seen = [];
    for (const key of ['mode', 'default', 'roles'])
        if (key in hostModels)
            seen.push(`host:models.${key}`);
    for (const key of ['strategy', 'categoryModels'])
        if (key in hostRouting)
            seen.push(`host:routing.${key}`);
    for (const path of projectFields)
        seen.push(`project:${path}`);
