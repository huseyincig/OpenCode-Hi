...
import { HHC_CONFIG_SCHEMA, isRecord } from './schema.js';
...
export function resolveHhcConfigWithReport(raw, projectRoot) {
    const input = isRecord(raw) ? raw : {};
    const notes = [];
    const suppliedSchema = Number(input.schemaVersion ?? HHC_CONFIG_SCHEMA);
    if (Number.isFinite(suppliedSchema) && suppliedSchema !== HHC_CONFIG_SCHEMA)
        notes.push(`unsupported config schema ${suppliedSchema}; OHO interpreted canonical fields only`);
    const fromProject = projectRoot ? loadProjectRoutingConfig(projectRoot) : undefined;
    if (fromProject && fromProject.routing)
        notes.push(`project routing override merged from .opencode/oho-routing.json (${Object.keys(fromProject.routing.roleModels ?? {}).length} roles)`);
    ...
    const config = {
        ...
        autonomy: ['basic', 'standard', 'powerful', 'smart', 'manual'].includes(fromProject?.autonomy) ? fromProject.autonomy : (['basic', 'standard', 'powerful', 'smart', 'manual'].includes(input.autonomy) ? input.autonomy : 'smart'),
        ...
        profile: isRecord(fromProject?.profile) ? { basic: profileSettings(fromProject.profile.basic, DEFAULT_HHC_CONFIG.profile.basic), standard: profileSettings(fromProject.profile.standard, DEFAULT_HHC_CONFIG.profile.standard), powerful: profileSettings(fromProject.profile.powerful, DEFAULT_HHC_CONFIG.profile.powerful) } : (isRecord(input.profile) ? { basic: profileSettings(input.profile.basic, DEFAULT_HHC_CONFIG.profile.basic), standard: profileSettings(input.profile.standard, DEFAULT_HHC_CONFIG.profile.standard), powerful: profileSettings(input.profile.powerful, DEFAULT_HHC_CONFIG.profile.powerful) } : DEFAULT_HHC_CONFIG.profile),
    };
    return { config, report: { schema: HHC_CONFIG_SCHEMA, canonical: suppliedSchema === HHC_CONFIG_SCHEMA, notes } };
}
export function resolveHhcConfig(raw, projectRoot) { return resolveHhcConfigWithReport(raw, projectRoot).config; }
