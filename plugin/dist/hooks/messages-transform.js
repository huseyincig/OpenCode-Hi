...
export function createMessagesTransformHook(store, background) {
    return async (input, output) => {
        ...
        if (firstUser.parts.some((p) => p?.type === 'text' && p.text === contract))
            return;
        if (containsContract(firstUser.parts, marker)) {
            appendLedger(mission, 'host.composition-collision', { task_id: worker?.task_id, worker_id: worker?.id, payload: { surface: 'messages-transform', reason: 'hi-contract-marker-without-canonical-contract', kind } });
        }
