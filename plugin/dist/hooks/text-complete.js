 ...
 */
export function createTextCompleteHook(store, background, projectRoot) {
    return async (input, output) => {
        const sid = String(input?.sessionID ?? input?.sessionId ?? '');
        if (!sid || background?.list().some(w => w.session_id === sid))
            return;
        const m = store.get(sid);
        if (!m || m.identity.status !== 'active' || m.identity.semantic_assessment.status === 'pending')
            return;
        const decision = projectControlDecision(m, projectRoot);
        if (decision.action === 'DONE' || decision.action === 'USER_ACTION_REQUIRED')
            return;
