...
export function createSystemTransformHook(store, background, _projectRoot) { return async (input, output) => { if (isNativeHousekeeping(input, output))
    ...
    return; const worker = child ? m.execution.workers.find(w => w.id === child.id) : undefined, projection = buildMissionRuntimeProjection(m, worker); output.system.push(redactProviderContext(renderMissionRuntimeProjection(projection)).providerText); }; }
