...
import { lastAssistantError,lastAssistantModel,lastAssistantStructured,lastAssistantText,lastAssistantUsage,lastIncompleteAssistantTurn,lastMeaningfulAssistantActivity,listAvailableModels,listMessages,listProviders,readSessionRuntimeStatus,sendSyntheticContinuation } from './client-adapter.js'
...
export function createHostPort(ctx:OpenCodePluginContext):HostPort{
  ...
  const readAssistantResult=async(sessionID:string)=>{const messages=await listMessages(ctx.client,sessionID,20,{serverUrl:ctx.serverUrl?String(ctx.serverUrl):undefined,directory:ctx.directory}),latest=[...messages].reverse().find((msg:any)=>{const info=msg?.info??msg?.message??msg;return !info?.role||info.role==='assistant'}),current=latest?[latest]:[];return{text:lastAssistantText(current),structured:lastAssistantStructured(current),model:lastAssistantModel(current),usage:lastAssistantUsage(current),activity:lastMeaningfulAssistantActivity(messages),incomplete_turn:lastIncompleteAssistantTurn(current),error:lastAssistantError(current)}}
