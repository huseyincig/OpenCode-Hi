...
export async function abortSession(client:any,sessionID:string):Promise<void>{if(typeof client?.session?.abort==='function')await client.session.abort({path:{id:sessionID}})}
