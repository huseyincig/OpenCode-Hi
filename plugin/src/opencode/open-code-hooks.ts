...
export function createOpenCodeHooks(input:{state:PluginRuntimeState;host:HostPort;services:ReturnType<typeof createRuntimeServices>;projectRoot:string;workingDirectory?:string;packagedSkillsDir:string;projectAuthority:ProjectAuthorityStore;toolSurface:Record<string,unknown>;eventController:RuntimeEventController;instanceLease:{release:()=>void}}){
  ...
  return {
    ...
    'chat.message':async(input:any,output:any)=>{try{const messageSession=String(input?.sessionID??input?.sessionId??'');if(messageSession&&background.list().some((w:any)=>w.session_id===messageSession)){await host.log('debug','Hi child chat message ignored by parent intent hook',{session_id:messageSession});return}if(!host.getModels().length)void host.refreshRuntimeInventory('chat-message').then(()=>tasks.wakeQueued()).catch(()=>{});await createChatMessageHook(store,async(sid,text)=>{const m=store.get(sid);if(!m)return;const workersPaused=await tasks.pauseForSemanticAssessment(m);appendLedger(m,'semantic.execution-quarantined',{payload:{revision:m.identity.semantic_assessment.revision,workers:workersPaused,preview:text.slice(0,180)}})},humanDecisionTransport)(input,output);tasks.wakeQueued()}finally{for(const m of store.all())syncHumanDecisionTransport(m.authority.human_decision,humanDecisionTransport);persistence.save(store.all())}},
    ...
    'tool.execute.after':async(input:any,output:any)=>{try{await createToolAfterHook(store,background,eventSink,projectRoot,workingDirectory)(input,output)}finally{for(const m of store.all()){if(['stopped','completed'].includes(m.identity.status))eventController.clearNativePermissionsForMission(m);syncHumanDecisionTransport(m.authority.human_decision,humanDecisionTransport)}persistence.save(store.all())}},
    dispose:async()=>{try{for(const m of store.all())if(['active','waiting-user'].includes(m.identity.status)){await processRuntime.stopMission(m);appendLedger(m,'runtime.plugin-disposed',{payload:{mission_status:m.identity.status,durable_mission_preserved:true,semantic_stop:false}})}humanDecisionTransport.dispose();eventController.clearAllNativePermissions();const browserDisposable=browserExecutor as typeof browserExecutor&{dispose?:()=>Promise<void>};if(browserDisposable.dispose)await browserDisposable.dispose();await previewManager.dispose();persistence.markCleanShutdown(store.all())}finally{instanceLease.release()}},
