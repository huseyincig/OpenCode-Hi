...
export interface VerificationCoverageClaim { case_id:string; outcome:'passed'|'failed'; evidence_refs:string[]; reason?:string }
export interface ScopeExpansion { file:string; reason:string; necessary:boolean }
export interface WorkerResult { status:WorkerResultStatus; summary:string; changed_files:string[]; scope_expansions?:ScopeExpansion[]; evidence:WorkerEvidenceClaim[]; verification_coverage?:VerificationCoverageClaim[]; findings?:ReviewFinding[]; open_issues:string[]; needs_context:string[]; context_gap?:'scope'|'iterative'|'none'; failure_finding?:'ci-build'|'unknown-root-cause'|'none'; methodology_observations?:MethodologyObservation[] }
...
function boundedIssueText(v:unknown):string{if(typeof v==='string')return clip(v,1200);if(record(v)||Array.isArray(v))try{return clip(JSON.stringify(v),1200)}catch{}return clip(v,1200)}
...
function normalizeVerificationCoverage(raw:unknown):VerificationCoverageClaim[]|undefined{if(!Array.isArray(raw))return undefined;const out=raw.slice(0,16).flatMap((v:any)=>{if(!record(v))return[];const case_id=String(v.case_id??'').trim().toLowerCase();if(!/^vc_[a-z0-9][a-z0-9-]{0,47}$/.test(case_id)||!['passed','failed'].includes(String(v.outcome)))return[];const evidence_refs=normalizeEvidenceRefs(v.evidence_refs);if(!evidence_refs?.length)return[];return[{case_id,outcome:String(v.outcome) as 'passed'|'failed',evidence_refs,reason:typeof v.reason==='string'?clip(v.reason,1000):undefined}]});return out.length?out:undefined}
...
export function normalizeWorkerResult(raw:unknown):WorkerResult{
  ...
  const evidence=normalizeEvidence(x.evidence),findings=normalizeFindings(x.findings,evidence),rawFindings=x.findings,explicitFindings=rawFindings!==undefined,malformedFindings=explicitFindings&&(!Array.isArray(rawFindings)||rawFindings.length!==(findings?.length??0))
  const effectiveStatus=malformedFindings&&status==='DONE'?'FIX_REQUIRED':status
  if(malformedFindings&&!open.includes('review-finding-contract-invalid'))open.unshift('review-finding-contract-invalid')
  const needs=Array.isArray(x.needs_context)?x.needs_context.map(boundedIssueText).slice(0,30):[]
  if(malformedFindings&&!needs.some(item=>item.startsWith('review-finding-contract-retry:')))needs.unshift('review-finding-contract-retry: return each finding with canonical rf-* id, reviewer_role, bounded project scope, and evidence_refs that name evidence.kind values from the same WorkerResult; opaque ev_* ids belong on the evidence claim, not the finding')
  return{status:effectiveStatus,summary:typeof x.summary==='string'?clip(x.summary,4000):'',changed_files:Array.isArray(x.changed_files)?x.changed_files.flatMap(v=>{const p=normalizeBoundedProjectPath(v);return p?[p]:[]}).slice(0,200):[],scope_expansions:Array.isArray(x.scope_expansions)?x.scope_expansions.filter(record).slice(0,80).flatMap(v=>{const file=normalizeBoundedProjectPath(v.file);return file?[{file,reason:clip(v.reason,600),necessary:v.necessary===true}]:[]}):[],evidence,verification_coverage:normalizeVerificationCoverage(x.verification_coverage),findings,open_issues:open.slice(0,30),needs_context:needs.slice(0,30),context_gap:contextGap,failure_finding:reconcileFailureFinding(rawFinding,evidence),methodology_observations:normalizeMethodologyObservations(x.methodology_observations)}
}
