...
const PROFILE_KEYS=new Set(['role','category','task','tools','mcp_servers','browser_backend','model','model_variant','fallback_models','fallback_variants','fallback_reasons','methodologies','permission_profile','verification_policy','max_context_chars','max_handoff_chars','max_result_chars','max_artifacts','expected_turns','context_overhead'])
...
function validProfile(v:unknown):boolean{
  if(!record(v)||!Object.keys(v).every(k=>PROFILE_KEYS.has(k))||typeof v.role!=='string'||typeof v.category!=='string'||!CATEGORIES.has(v.category)||!record(v.task)||!strings(v.tools)||(v.mcp_servers!==undefined&&!strings(v.mcp_servers))||(v.browser_backend!==undefined&&!['bounded-playwright','mcp'].includes(String(v.browser_backend)))||!strings(v.fallback_models)||!strings(v.methodologies))return false
