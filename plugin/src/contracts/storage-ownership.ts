...
export const STORAGE_OWNERSHIP_CATALOG=[
  ...
  {data_class:'project-operational-tool-cache',canonical_owner:'hi-operational-tool-provisioner',scope:'project',lifecycle:'cache',path_provider:'.opencode/hi/tools/<capability>/<implementation>/**',schema_ref:'OperationalToolDefinition@1',write_owner:'OperationalToolProvisioner',readers:['OperationalToolProvisioner','runtime-service-adapters','doctor'],retention:'retain verified project-local operational tool cache until explicit cleanup/version replacement; never application dependency state',privacy:'operational-private'},
  {data_class:'project-operational-tool-receipt',canonical_owner:'hi-operational-tool-provisioner',scope:'project',lifecycle:'derived',path_provider:'.opencode/hi/tools/receipts/<capability>/<implementation>.json',schema_ref:'OperationalToolProvisioningReceipt@1',write_owner:'OperationalToolProvisioner',readers:['OperationalToolProvisioner','doctor'],retention:'latest bounded resolution/smoke receipt per code-owned implementation',privacy:'operational-private; no credentials or raw installer output'},
  {data_class:'project-operational-tool-lock',canonical_owner:'hi-operational-tool-provisioner',scope:'project',lifecycle:'ephemeral',path_provider:'.opencode/hi/tools/.locks/<capability>--<implementation>.lock',schema_ref:'OperationalToolProvisionLock@1',write_owner:'OperationalToolProvisioner',readers:['OperationalToolProvisioner'],retention:'active provisioning only; dead/stale owner reaped',privacy:'operational-private'},
  ...
  {data_class:'runtime-writer-lease',canonical_owner:'hi-runtime-instance',scope:'runtime',lifecycle:'ephemeral',path_provider:'runtimeInstanceLockPath(projectRoot) -> OS state/opencode-hi/projects/<project-hash>/runtime-instance.lock',schema_ref:'RuntimeInstanceLease@1',write_owner:'acquireHiRuntimeInstance',readers:['acquireHiRuntimeInstance'],retention:'active plugin runtime only; dead-owner lease is quarantined and reaped during recovery',privacy:'operational-private'},
