...
export const PROCESS_STATUSES=['RUNNING','EXITED','TIMED_OUT','TERMINATED','ORPHANED'] as const
export type ProcessStatus=typeof PROCESS_STATUSES[number]
export const PROCESS_CLEANUP_STATES=['ACTIVE','CLEANUP_PENDING','CLEANED','QUARANTINED'] as const
...
export function isCleanupPendingProcess(process:Pick<ProcessContract,'status'|'cleanup_state'>):boolean{return process.status!=='RUNNING'&&process.status!=='ORPHANED'&&process.cleanup_state==='CLEANUP_PENDING'}
...
export function processCommandIdentity(input:{host:string;command:string;cwd:string}):string{
  ...
  if(!host||!command||!cwd)throw new Error('Process command identity requires non-empty host, command and cwd')
  return createHash('sha256').update(`${host}\0${cwd}\0${command}`).digest('hex')
}

export function isProcessContract(value:unknown):value is ProcessContract{
  ...
  if(value.exit_code!==undefined&&!Number.isInteger(value.exit_code))return false
  if(value.termination_reason!==undefined&&(!nonempty(value.termination_reason)||value.termination_reason.length>800))return false

  if(value.status==='RUNNING'){
    if(value.ended_at!==undefined||value.exit_code!==undefined||value.termination_reason!==undefined)return false
    if(!['ACTIVE','CLEANUP_PENDING'].includes(value.cleanup_state))return false
  }else if(value.status!=='ORPHANED'){
    if(value.ended_at===undefined)return false
    if(value.cleanup_state==='ACTIVE')return false
  }
  if(value.status==='EXITED'){
    if(value.exit_code===undefined||value.termination_reason!==undefined)return false
  }else if(value.status==='TIMED_OUT'){
    if(value.timeout_at===undefined||value.termination_reason===undefined||value.exit_code!==undefined)return false
  }else if(value.status==='TERMINATED'){
    if(value.termination_reason===undefined||value.exit_code!==undefined)return false
  }else if(value.status==='ORPHANED'){
    if(value.cleanup_state!=='QUARANTINED'||value.termination_reason===undefined||value.exit_code!==undefined)return false
  }
  if(value.cleanup_state==='CLEANED'&&value.status==='RUNNING')return false
  if(value.cleanup_state==='QUARANTINED'&&value.status!=='ORPHANED')return false
  return true
}
