...
const T95:Record<number,number>={2:12.706,3:4.303,4:3.182,5:2.776,6:2.571,7:2.447,8:2.365,9:2.306,10:2.262,11:2.228,12:2.201,13:2.179,14:2.160,15:2.145,16:2.131,17:2.120,18:2.110,19:2.101,20:2.093,21:2.086,22:2.080,23:2.074,24:2.069,25:2.064,26:2.060,27:2.056,28:2.052,29:2.048,30:2.045}
const round=(n:number)=>Number(n.toFixed(12))
function finite(n:unknown):n is number{return typeof n==='number'&&Number.isFinite(n)}
export function sampleDistribution95(values:readonly number[]):EvalDistributionSummary{
  if(!values.length||values.some(v=>!finite(v)||v<0))throw new Error('distribution requires non-empty finite non-negative samples')
  const n=values.length,mean=values.reduce((a,b)=>a+b,0)/n
  if(n===1)return{sample_count:1,mean:round(mean),sample_stddev:0,confidence_level:0.95,confidence_interval_95:[round(mean),round(mean)]}
...
function band(k:number):JudgeAgreementBand{if(k<0)return'LESS_THAN_CHANCE';if(k<=.2)return'SLIGHT';if(k<=.4)return'FAIR';if(k<=.6)return'MODERATE';if(k<=.8)return'SUBSTANTIAL';return'ALMOST_PERFECT'}
export function fleissKappaBinary(matrix:readonly (readonly number[])[]):EvalJudgeAgreement{
  if(!matrix.length)return{status:'NOT_PROVIDED',item_count:0,judge_count:0}
  const judgeCount=matrix[0]?.length??0
  if(judgeCount<2||matrix.some(row=>row.length!==judgeCount||row.some(score=>score!==0&&score!==1)))return{status:'INSUFFICIENT',item_count:matrix.length,judge_count:judgeCount}
...
export function evidenceFamilyDiversity(families:readonly string[]):EvalEvidenceFamilyDiversity{
  const clean=families.map(x=>String(x).trim()).filter(Boolean)
  if(!clean.length)return{status:'NOT_PROVIDED',evidence_count:0,unique_family_count:0,largest_family_count:0,largest_family_share:0,families:{}}
  const counts:Record<string,number>={};for(const family of clean)counts[family]=(counts[family]??0)+1
...
export function buildEvalUncertaintyDiagnostics(input:{wall_times_ms:readonly number[];judge_scores?:readonly (readonly number[])[];evidence_families?:readonly string[]}):EvalUncertaintyDiagnostics{
  const judge=fleissKappaBinary(input.judge_scores??[]),diversity=evidenceFamilyDiversity(input.evidence_families??[]),flags:EvalUncertaintyDiagnostics['flags']=[]
  if(judge.status==='INSUFFICIENT')flags.push('INSUFFICIENT_JUDGE_DATA')
  if(judge.status==='MEASURED'&&(judge.fleiss_kappa??1)<.6)flags.push('JUDGE_DISAGREEMENT')
  if(diversity.status==='INSUFFICIENT')flags.push('INSUFFICIENT_EVIDENCE_DIVERSITY')
  else if(diversity.status==='MEASURED'&&diversity.unique_family_count<2)flags.push('LOW_EVIDENCE_FAMILY_DIVERSITY')
