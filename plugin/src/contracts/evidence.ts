...
export function isEvidenceItemContract(v:unknown):v is EvidenceItem{
  ...
  for(const key of ['source','source_session_id','source_state_hash','scope_state_hash','task_id','reason','browser_url','browser_origin'] as const)if(v[key]!==undefined&&typeof v[key]!=='string')return false
