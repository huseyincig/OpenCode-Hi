import { isContextReferenceContract,type ContextReferenceContract,type ContextFreshness } from './context-reference.js'
...
function strings(v:unknown):v is string[]{return Array.isArray(v)&&v.length>0&&v.every(x=>typeof x==='string'&&x.length>0)}
export function isCompressionArtifact(v:unknown):v is CompressionArtifact{
  if(!record(v)||!Object.keys(v).every(k=>KEYS.has(k))||typeof v.id!=='string'||!/^a_[a-f0-9]{24}$/.test(v.id)||!strings(v.source_context_refs)||!strings(v.source_hashes)||v.source_context_refs.length!==v.source_hashes.length)return false
  if(typeof v.summary!=='string'||!v.summary.trim()||typeof v.created_at!=='number'||!Number.isFinite(v.created_at)||!['FRESH','POTENTIALLY_STALE'].includes(String(v.freshness)))return false
  if(typeof v.consumer_scope!=='string'||!v.consumer_scope.trim()||typeof v.model_identity!=='string'||!v.model_identity.trim()||typeof v.compression_policy_version!=='string'||!v.compression_policy_version.trim())return false
  return v.source_hashes.every(x=>/^[a-f0-9]{64}$/.test(x))
}

export function buildCompressionArtifact(id:string,sources:ContextReferenceContract[],summary:string,input:{consumerScope:string;modelIdentity:string;policyVersion?:string;createdAt?:number}):CompressionArtifact{
  if(!sources.length)throw new Error('CompressionArtifact requires at least one context source')
  if(sources.some(s=>!isContextReferenceContract(s)))throw new Error('CompressionArtifact requires valid ContextReference contracts')
