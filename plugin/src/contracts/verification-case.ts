...
export interface VerificationCase { id:string; subject:string; required_browser_actions:BrowserObservationAction[]; source_units?:string[] }
...
export function verificationCaseValidationError(v:unknown):string|undefined{
  ...
  const x=v as Record<string,unknown>,keys=Object.keys(x),unknown=keys.filter(k=>!['id','subject','required_browser_actions','source_units'].includes(k));if(unknown.length)return`unsupported field(s): ${unknown.join(', ')}`
  ...
  if(x.source_units!==undefined){if(!Array.isArray(x.source_units)||x.source_units.length===0||x.source_units.length>24)return'source_units must contain 1..24 request-unit ids (ruN)';if(new Set(x.source_units).size!==x.source_units.length)return'source_units must not contain duplicates';const invalidUnit=x.source_units.find(id=>typeof id!=='string'||!/^ru[1-9][0-9]*$/.test(id));if(invalidUnit!==undefined)return`source_units contains invalid request-unit id: ${String(invalidUnit)}`}
  return undefined
}
