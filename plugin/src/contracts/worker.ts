...
export interface WorkerContract {
  ...
  fallback_history?:Array<{from?:string;to:string;variant?:string;reason:string;phase:'dispatch'|'runtime';at:number}>
...
function stringRecord(v:unknown):boolean{return record(v)&&Object.values(v).every(x=>typeof x==='string')}
...
function pendingAssistantResult(v:unknown):boolean{
  ...
  try{JSON.parse(v.structured_json)}catch{return false}
  for(const k of ['message_id','parent_id','model','variant'])if(v[k]!==undefined&&(typeof v[k]!=='string'||String(v[k]).length>512))return false
  if(v.created_at!==undefined&&!finite(v.created_at))return false
  return v.usage===undefined||isHostUsageObservation(v.usage)
}
export function isWorkerContract(v:unknown):v is WorkerContract{
  if(!record(v)||!Object.keys(v).every(k=>WORKER_KEYS.has(k))||typeof v.id!=='string'||typeof v.task_id!=='string'||typeof v.role!=='string'||typeof v.category!=='string'||!CATEGORIES.has(v.category)||typeof v.parent_session_id!=='string'||typeof v.parent_mission_id!=='string'||typeof v.fingerprint!=='string'||typeof v.status!=='string'||!STATUS.has(v.status))return false
  if(!strings(v.fallbacks)||(v.recovery_candidates!==undefined&&!strings(v.recovery_candidates))||!strings(v.selected_methodologies)||!strings(v.loaded_methodologies)||!Array.isArray(v.methodologies)||!v.methodologies.every(methodology)||!Number.isInteger(v.attempt)||Number(v.attempt)<0||!Number.isInteger(v.generation_at_spawn)||Number(v.generation_at_spawn)<1||!finite(v.updated_at))return false
  for(const k of ['session_id','forked_from_session_id','requested_model','requested_model_variant','model','model_variant','projected_model','projected_model_variant','last_result_digest','attempt_prompt_message_id','native_state_hash','last_runtime_failure_kind','effective_model','effective_model_variant','effective_model_source'])if(v[k]!==undefined&&typeof v[k]!=='string')return false
  for(const k of ['started_at','completed_at','last_result_at','runtime_recovery_attempt','effective_model_observed_at','semantic_pause_revision'])if(v[k]!==undefined&&!finite(v[k]))return false
  for(const k of ['restart_reconcile_pending','runtime_recovery_pending','runtime_fallback_exhausted','effective_model_verified','effective_model_variant_verified'])if(v[k]!==undefined&&typeof v[k]!=='boolean')return false
  if(v.attempt_prompt_message_id!==undefined&&(typeof v.attempt_prompt_message_id!=='string'||!/^msg_[0-9a-f]{26}$/i.test(v.attempt_prompt_message_id)))return false
  if(v.write_set!==undefined&&!strings(v.write_set))return false
  if(v.text_transport_fallback_models!==undefined&&!strings(v.text_transport_fallback_models))return false
  if(v.model_selection_reason!==undefined&&!strings(v.model_selection_reason))return false
