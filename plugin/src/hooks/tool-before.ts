...
export function createToolBeforeHook(store:MissionStore,background?:BackgroundRegistry,projectRoot?:string){return async(input:any,output:any)=>{const sid=input?.sessionID??input?.sessionId,child=sid&&background?background.list().find(w=>w.session_id===sid):undefined,m=child?store.get(child.parent_session_id):store.get(sid);if(!m)return;if(child&&((child.parent_mission_id!==undefined&&child.parent_mission_id!==m.mission_id)||(child.generation_at_spawn!==undefined&&child.generation_at_spawn!==m.generation)))return;const tool=String(input?.tool??''),args=output?.args??input?.args??{}
  ...
  if(m.semantic_assessment.status==='pending'){const allowed=new Set(['hi_intent_assess','hi_status','hi_ledger','hi_readiness']);if(!allowed.has(tool))throw new Error(`Hi semantic gate: '${tool}' is blocked until the host primary submits the structured semantic assessment.`)}
  if(tool==='skill'){const name=requestedMethodologyName(args);if(name&&child)assertChildMethodologyLoad(m.workers.find(worker=>worker.id===child.id),name);else if(name)assertParentMethodologyLoad(m,name,projectRoot)}
