...
export function createChatMessageHook(store:MissionStore,onFollowupPending?: (sessionID:string,text:string)=>Promise<void>){return async(input:any,output:any)=>{
  ...
  if(!existing||existing.status==='completed'||existing.status==='failed'){store.start(sid,userText,observedPrimary);return}
  ...
  if(existing.status==='stopped'||existing.user_interrupted){store.start(sid,userText,observedPrimary);return}
  if(existing.semantic_assessment.status==='pending')return
