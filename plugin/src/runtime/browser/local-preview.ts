...
function admittedPath(target:string,scope:string[]):boolean{const admitted=scope.map(scopePath).filter((x):x is string=>Boolean(x));return !admitted.length||admitted.some(item=>target===item||target.startsWith(`${item}/`))}
...
export class LocalPreviewManager{
  ...
  async start(taskID:string,targetRaw:string,scope:string[]):Promise<LocalPreviewResult>{
    ...
    const current=this.#leases.get(taskID);if(current&&current.root===root)return{task_id:taskID,origin:current.origin,url:current.origin+encodedPath(target),root,target,reused:true}
    ...
    const server=createServer((req,res)=>{
      ...
      const bounded=normalizeBoundedProjectPath(raw.replace(/^\/+/,''));if(!bounded||bounded.split('/').some(part=>part.startsWith('.'))||!admittedPath(bounded,scope)){res.writeHead(403);res.end('Forbidden');return}
    ...
    server.unref();const origin=`http://127.0.0.1:${address.port}`;this.#leases.set(taskID,{taskID,root,server,origin,scope:[...scope]})
