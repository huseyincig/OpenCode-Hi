...
export async function dispatchContinuation(client:OpenCodeClient,mission:MissionState,prompt:string,reason:string):Promise<boolean>{
  ...
  try{
    const sent=await sendSyntheticContinuation(client,mission.identity.session_id,prompt,{hiInternalContinuation:true,reason,generation,actionID});if(!sent){appendLedger(mission,'continuation.unavailable',{payload:{reason:'host-continuation-api-missing'}});return false}
