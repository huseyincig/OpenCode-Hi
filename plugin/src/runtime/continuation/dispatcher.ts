...
export async function dispatchContinuation(client:any,mission:MissionState,prompt:string,reason:string):Promise<boolean>{
  const now=Date.now(),generation=mission.generation;if(mission.user_interrupted||mission.status==='stopped'){appendLedger(mission,'continuation.rejected',{payload:{reason:'user-interrupted',generation}});return false}
  if(mission.continuation_active||mission.active_action_id||(mission.suppress_until??0)>now||(mission.continuation_lock_until??0)>now)return false
  ...
  if(!fn){appendLedger(mission,'continuation.unavailable',{payload:{reason:'host-continuation-api-missing'}});return false}
  ...
  }catch(error){if(mission.generation===generation&&mission.active_action_id===actionID){mission.continuation_failure_count=(mission.continuation_failure_count??0)+1;mission.last_continuation_failure_at=Date.now();if(mission.iteration===iteration)mission.iteration=Math.max(0,mission.iteration-1)}appendLedger(mission,'continuation.failed',{payload:{error:String(error),generation,action_id:actionID,runtime_failures:mission.continuation_failure_count??0}});return false}
  finally{if(mission.generation===generation&&mission.active_action_id===actionID){mission.continuation_active=false;mission.active_action_id=undefined}}
}
