...
export function evaluateIdle(m:MissionState|undefined,now=Date.now(),projectRoot?:string):DecisionResult{
  ...
  const operationalPrefixes=['dependency-unavailable:','workspace-orphan:','workspace-reintegration-failed:','workspace-provision-failed:','queue-overflow-cleanup-failed:','browser-cleanup-failed:','process-cleanup:','process-termination-unverified:','process-wait-failed:','scheduler-restart-reconcile-failed:'] as const
  ...
  const pre=evaluatePreconditions(m,projectRoot);const contractOnly=pre.items.filter(x=>x.status==='blocked').every(x=>x.id==='gate-contract-ambiguity')&&pre.items.some(x=>x.id==='gate-contract-ambiguity'&&x.status==='blocked')
...
export function shouldCountStagnation(decision:DecisionResult):boolean{return ['open-obligation','contract-ambiguity-repo-first','stagnation-recovery','verification-pending','verification-failed','worker-result-unreconciled'].includes(decision.reason_code)}
