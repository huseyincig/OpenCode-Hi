...
export function observeToolAfter(mission:MissionState,tool:string,args:any,output:any,projectRoot?:string,owner?:ToolEvidenceOwner):void{
  ...
  if(tool==='bash'){
    ...
    if(kind){
      ...
      for(const obligation of mission.execution.obligations.filter(o=>o.kind==='verification'&&o.status==='open'))if(verificationSatisfied(mission,obligation.id,projectRoot).ok){obligation.status='closed';obligation.closedAt=Date.now();appendLedger(mission,'obligation.closed',{payload:{obligation:obligation.id,owner:'verification-evidence'}})}
    }
  }
}
