...
function outcomeOf(output:any,text:string):{outcome:EvidenceOutcome;reason?:string}{const exit=numericExit(output);if(exit===0)return{outcome:'passed'};if(ENVIRONMENT_FAILURE.test(text))return{outcome:'environment-issue',reason:'verification-environment-unavailable'};if(exit!==undefined)return{outcome:'failed',reason:`verification-exit-${exit}`};if(/(^|\n)\s*(fail|failed|error)|exit\s*code\s*[1-9]/i.test(text))return{outcome:'failed',reason:'verification-reported-failure'};return{outcome:'pending',reason:'verification-exit-unknown'}}
