...
function verificationExitMayBeMasked(command:string):boolean{
  const text=command.trim()
  if(/\|\|/.test(text))return true
  if(/;\s*(?:true|:|exit\s+0)\s*$/.test(text))return true
  const hasPipeline=/(^|[^|])\|([^|]|$)/.test(text),hasPipefail=/(?:set\s+-o\s+pipefail|set\s+-[^;\n]*o\s+pipefail)/i.test(text)
  return hasPipeline&&!hasPipefail
}
function outcomeOf(output:any,text:string,command=''):{outcome:EvidenceOutcome;reason?:string}{if(verificationExitMayBeMasked(command))return{outcome:'pending',reason:'verification-exit-status-masked'};const exit=numericExit(output);if(exit===0)return{outcome:'passed'};if(ENVIRONMENT_FAILURE.test(text))return{outcome:'environment-issue',reason:'verification-environment-unavailable'};if(exit!==undefined)return{outcome:'failed',reason:`verification-exit-${exit}`};if(/(^|\n)\s*(fail|failed|error)|exit\s*code\s*[1-9]/i.test(text))return{outcome:'failed',reason:'verification-reported-failure'};return{outcome:'pending',reason:'verification-exit-unknown'}}
...
export function markMutation(mission:MissionState,files:string[]=[],source='tool'):void{
  ...
  const invalidated:string[]=[],claimIDs=new Set<string>()
  for(const item of mission.execution.evidence.items)if(!item.invalidated_at&&mutationAffectsEvidence(item.scope??[],changed)){item.invalidated_at=now;invalidated.push(item.id);for(const id of item.obligation_ids??[])claimIDs.add(id)}
  const reopened:string[]=[]
  for(const obligation of mission.execution.obligations)if(obligation.kind==='verification'&&obligation.status==='closed'&&claimIDs.has(obligation.id)){obligation.status='open';obligation.closedAt=undefined;reopened.push(obligation.id);appendLedger(mission,'obligation.reopened',{payload:{obligation:obligation.id,owner:'evidence-freshness',reason:'claim-linked-evidence-invalidated',evidence_invalidated:invalidated.slice(0,100)}})}
  refreshCompatibilityFreshness(mission);mission.vcs.changed_files=[...new Set([...mission.vcs.changed_files,...changed])];appendLedger(mission,'file.changed',{payload:{source,files:changed,evidence_invalidated:invalidated.slice(0,100),verification_obligations_reopened:reopened,invalidation_mode:changed.length?'scope-overlap':'unknown-surface-fail-closed'}})
}

export function addEvidence(mission:MissionState,input:Omit<EvidenceItem,'id'|'observed_at'> & {observed_at?:number}):EvidenceItem{if(!evidenceVerdictConsistent(input.pass,input.outcome))throw new Error(`Contradictory evidence verdict: pass=${String(input.pass)} outcome=${String(input.outcome)}`);const outcome=resolvedEvidenceOutcome(input.pass,input.outcome),item:EvidenceItem={id:id(),observed_at:input.observed_at??Date.now(),kind:input.kind,summary:input.summary,scope:input.scope,source:input.source,trusted_source_class:input.trusted_source_class,source_session_id:input.source_session_id,source_state_hash:input.source_state_hash,scope_state_hash:input.scope_state_hash,task_id:input.task_id,obligation_ids:input.obligation_ids,evidence_refs:input.evidence_refs,browser_url:input.browser_url,browser_origin:input.browser_origin,producer_attempt:input.producer_attempt,pass:evidenceVerdictPassValue(input.pass,outcome),outcome,reason:input.reason,invalidated_at:input.invalidated_at};mission.execution.evidence.items.push(item);if(mission.execution.evidence.items.length>100)mission.execution.evidence.items.splice(0,mission.execution.evidence.items.length-100);refreshCompatibilityFreshness(mission);appendLedger(mission,item.outcome==='failed'?'verification.fail':item.outcome==='environment-issue'?'verification.environment-issue':item.outcome==='passed'?'verification.pass':'evidence.observed',{payload:{kind:item.kind,summary:item.summary,reason:item.reason,trusted_source_class:item.trusted_source_class,source_session_id:item.source_session_id,source_state_hash:item.source_state_hash,task_id:item.task_id,obligation_ids:item.obligation_ids,evidence_refs:item.evidence_refs,browser_url:item.browser_url,browser_origin:item.browser_origin}});return item}
export function reconcileEvidenceOwnedVerificationObligations(mission:MissionState,projectRoot?:string,context?:{task_id?:string;worker_id?:string;owner?:string}):string[]{
  const closed:string[]=[],now=Date.now()
  for(const obligation of mission.execution.obligations.filter(o=>o.kind==='verification'&&o.status==='open')){
    if(!verificationSatisfied(mission,obligation.id,projectRoot).ok)continue
    obligation.status='closed';obligation.closedAt=now;closed.push(obligation.id)
  ...
  if(closed.length)reconcileSatisfiedTaskArtifacts(mission,'verification-evidence-obligation-closed')
  return closed
}
...
export function observeToolAfter(mission:MissionState,tool:string,args:any,output:any,projectRoot?:string,owner?:ToolEvidenceOwner):void{
  ...
  if(tool==='read'&&mission.identity.intent.taskKind==='review'){
    ...
    if(text.trim()&&!/(error|failed)/i.test(text))addEvidence(mission,{kind:'review-input',summary:path?('Read '+path):'Read-only review input',scope:path?[path]:[],source:'read',trusted_source_class:'runtime-observation',pass:true,outcome:'passed'})
  }
  if(tool==='skill'&&mission.identity.intent.taskKind==='review'){
    const text=typeof output==='string'?output:JSON.stringify(output??'')
    if(text.trim()&&!/(error|failed)/i.test(text))addEvidence(mission,{kind:'review-input',summary:'Native skill content loaded',scope:[],source:'skill',trusted_source_class:'runtime-observation',pass:true,outcome:'passed'})
  }
  if(tool==='bash'&&mission.identity.intent.taskKind==='review'&&!shellMayMutate(command)){
    ...
    if(text.trim()&&(exit===undefined||exit===0)&&!/(error|failed)/i.test(text))addEvidence(mission,{kind:'review-input',summary:'Read-only command: '+command.slice(0,180),scope:[],source:'bash',trusted_source_class:'host-tool-observation',pass:true,outcome:'passed'})
  }
  if(tool==='bash'){
