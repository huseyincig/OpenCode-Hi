...
function reviewerObservation(e:EvidenceItem):boolean{return e.trusted_source_class==='reviewer-observation'||String(e.source??'').startsWith('reviewer:')}
function exactProducer(m:MissionState,e:EvidenceItem):string[]{
  const reasons:string[]=[],p=e.producer_attempt,isWorker=workerSource(e),isReviewer=reviewerObservation(e),requiresProducer=isWorker||isReviewer
  ...
  if(isReviewer&&(!e.source_session_id||!e.source_state_hash||!/^[a-f0-9]{64}$/i.test(e.source_state_hash)))reasons.push("reviewer-source-state-unbound")
  if(!p)return requiresProducer?[...reasons,isReviewer?"reviewer-attempt-unbound":"worker-attempt-unbound"]:reasons
  ...
  if(isReviewer){
    if(!isHiReviewerRole(worker.role))reasons.push("reviewer-role-mismatch")
    if(e.source!==`reviewer:${worker.id}`)reasons.push("reviewer-source-mismatch")
    if(!worker.native_state_hash||e.source_state_hash!==worker.native_state_hash)reasons.push("reviewer-state-mismatch")
  }
