...
function validVerificationCases(value:unknown):boolean{return Array.isArray(value)&&value.every(isVerificationCase)&&new Set(value.map((x:any)=>x.id)).size===value.length}
...
const INTENT_KEYS=['objective','likelyTargets','mutationTargets','taskKind','scope','risk','ambiguity','dependencyClass','requiredCapabilities','requestedExternalActions','likelyVerification','verificationCases','avoid'] as const
...
function validObligation(value:unknown):boolean{
  ...
  if(value.verificationCases!==undefined&&(String(value.kind)!=='verification'||!validVerificationCases(value.verificationCases)))return false
...
function validIntent(value:unknown):value is NormalizedMissionIntent{
  ...
  return typeof value.objective==='string'
    ...
    &&semanticTaskCapabilityConsistencyError(String(value.taskKind),value.requiredCapabilities,String(value.scope),stringArray(value.mutationTargets)?value.mutationTargets:[])===undefined
...
export function validateTaskDAG(identity:Record<string,unknown>,execution:Record<string,unknown>):boolean{
  ...
  const knownTasks=new Set(taskIDs),knownWorkers=new Set(workerIDs),obligationIDs=new Set((Array.isArray(execution.obligations)?execution.obligations:[]).map((o:any)=>String(o.id??'')))
  for(const task of tasks){
    if(task.mission_id!==missionID)return false
    const id=String(task.id),dependencies=task.dependencies as unknown[],ownedObligations=task.obligation_ids as unknown[]
    if(!Array.isArray(dependencies)||dependencies.some(dep=>typeof dep!=='string'||dep===id||!knownTasks.has(dep)))return false
    if(!Array.isArray(ownedObligations)||new Set(ownedObligations).size!==ownedObligations.length||ownedObligations.some(obligation=>typeof obligation!=='string'||!obligationIDs.has(obligation)))return false
...
export function validateMissionEnvelope(value:unknown):value is MissionState{
  ...
  if(!isRecord(identity)||!isRecord(execution)||!validateTaskDAG(identity,execution))return false
  ...
  return validateWorkGraph(projectMissionToWorkGraph(value as unknown as MissionState,Number(identity.updated_at))).ok
}
