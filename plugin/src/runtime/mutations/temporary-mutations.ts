...
export function registerTemporaryMutation(m:MissionState,input:{kind:string;description:string;rollback_command?:string;rollback_mode?:'command'|'native-revert';session_id?:string;message_id?:string}):TemporaryMutation{
  ...
  if(mode==='native-revert'&&!nativeSession)throw new Error('native-revert requires exact session_id')
  if(mode==='native-revert'&&!nativeMessage)throw new Error('native-revert requires exact message_id')
  ...
  const item:TemporaryMutation={id:`tm_${Date.now().toString(36)}_${Math.random().toString(36).slice(2,7)}`,kind:input.kind,description:redactDurableText(input.description),rollback_command:rollbackCommand,rollback_hash:payloadHash(rollbackCommand),rollback_mode:mode,session_id:mode==='native-revert'?nativeSession:input.session_id,message_id:mode==='native-revert'?nativeMessage:input.message_id,status:'active',created_at:Date.now()};m.vcs.temporary_mutations??=[];m.vcs.temporary_mutations.push(item);appendLedger(m,'temporary-mutation.registered',{payload:{id:item.id,kind:item.kind,rollback_hash:item.rollback_hash,rollback_mode:mode,session_id:item.session_id,message_id:item.message_id}});return item
}
