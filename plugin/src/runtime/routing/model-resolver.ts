...
export function resolveModel(category:Category,availableInput:AvailableModel[],config:HiConfig,explicit?:string,role?:string,hostConfig?:Record<string,unknown>,feedback:MissionModelFeedback={}):ModelResolution{
  ...
  const explicitRoleConfigured=role?config.routing.roleModels[role]??[]:[],roleConfigured=explicitRoleConfigured.length?explicitRoleConfigured:(role?DEFAULT_ROLE_MODELS_OPENCODE_GO[role]??[]:[]);if(roleConfigured.length){preferred.push(...roleConfigured);reason.push(explicitRoleConfigured.length?`role override:${role}`:`default role prior:${role}`)}const categoryConfigured=config.routing.categoryModels[category]??[];if(categoryConfigured.length){preferred.push(...categoryConfigured);reason.push(`category override:${category}`)}
