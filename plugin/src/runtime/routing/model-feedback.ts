...
function activeFeedbackGenerationFloor(m:MissionState):number{
  const generationByRevision=new Map<number,number>()
  for(const event of m.execution.ledger){
    if(event.type!=='semantic.followup-pending')continue
    const revision=Number(event.payload?.revision),generation=Number(event.payload?.generation)
    if(Number.isInteger(revision)&&revision>=2&&Number.isInteger(generation)&&generation>=1)generationByRevision.set(revision,generation)
  }
  let floor=1
  for(const event of m.execution.ledger){
    if(event.type!=='semantic.followup-assessed'||!MATERIAL_FEEDBACK_BOUNDARIES.has(String(event.payload?.message_kind??'')))continue
    const revision=Number(event.payload?.revision),generation=generationByRevision.get(revision)
    if(generation!==undefined)floor=Math.max(floor,generation)
  }
  return floor
}
function eligibleFeedbackWorkers(m:MissionState,role?:string,category?:Category):WorkerState[]{
