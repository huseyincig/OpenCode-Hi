...
export const HI_PROCESS_EXECUTION_TOOL_IDS=['hi_process_spawn','hi_process_read','hi_process_write','hi_process_wait','hi_process_kill','hi_process_cleanup','hi_process_list'] as const
export const HI_CONTROL_TOOL_IDS=['hi_doctor','hi_status','hi_settings','hi_role_models','hi_metrics','hi_super_product','hi_ledger','hi_readiness','hi_context_artifact_add','hi_context_artifacts','hi_temporary_mutation_register','hi_temporary_mutation_revert','hi_direct_progress','hi_task_start','hi_task_await','hi_task_peek','hi_task_list','hi_task_cancel'] as const
export const HI_RUNTIME_TOOL_IDS=[...HI_CONTROL_TOOL_IDS,'hi_intent_assess',...HI_PROCESS_EXECUTION_TOOL_IDS,...HI_BROWSER_EXECUTION_TOOL_IDS] as const
...
export interface McpServerExposure{configured:string[];selected:string[];disabledPatterns:string[]}
function mcpServerPattern(name:string):string{return name.replace(/[^a-zA-Z0-9_-]/g,'_')+'_*'}
export function resolveMcpServerExposure(hostConfig:Record<string,unknown>,selected:string[]=[]):McpServerExposure{const mcp=isRecord(hostConfig.mcp)?hostConfig.mcp:{},active=Object.entries(mcp).filter(([,value])=>!isRecord(value)||value.enabled!==false).map(([name])=>name).sort(),requested=[...new Set(selected.map(x=>String(x).trim()).filter(Boolean))].sort(),missing=requested.filter(name=>!active.includes(name));if(missing.length)throw new Error(`Requested MCP server(s) unavailable: ${missing.join(', ')}`);const byPattern=new Map<string,string[]>();for(const name of active){const pattern=mcpServerPattern(name);byPattern.set(pattern,[...(byPattern.get(pattern)??[]),name])}const collisions=[...byPattern.entries()].filter(([,names])=>names.length>1);if(collisions.length)throw new Error(`MCP server tool namespace collision: ${collisions.map(([pattern,names])=>`${pattern}<=${names.join('|')}`).join(', ')}`);const keep=new Set(requested);return{configured:active,selected:requested,disabledPatterns:active.filter(name=>!keep.has(name)).map(mcpServerPattern)}}
export function taskPromptToolOverrides(allowed:string[],hostConfig:Record<string,unknown>,selectedMcpServers:string[]=[]):Record<string,boolean>{const out=promptToolOverrides(allowed),mcp=resolveMcpServerExposure(hostConfig,selectedMcpServers);for(const pattern of mcp.disabledPatterns)out[pattern]=false;return out}
export function promptToolOverrides(allowed:string[],hiToolNames:string[]=[...HI_CONTROL_TOOL_IDS]):Record<string,boolean>{const keep=new Set(allowed);const out:Record<string,boolean>={};for(const id of KNOWN_BUILTIN_TOOL_IDS)if(!keep.has(id))out[id]=false;for(const id of hiToolNames)out[id]=false;for(const id of HI_PROCESS_EXECUTION_TOOL_IDS)if(!keep.has(id))out[id]=false;for(const id of HI_BROWSER_EXECUTION_TOOL_IDS)if(!keep.has(id))out[id]=false;return out}
