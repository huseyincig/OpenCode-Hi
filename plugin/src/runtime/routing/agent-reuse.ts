...
const HHC_CUSTOM_ROLE_REQUIREMENTS:Record<string,string[]>= {
...
export function auditNativeAgentReuse(role:string):AgentReuseAudit{
  ...
  if(reasons?.length)return{role,decision:'hhc-custom-required',reasons}
...
export function knownHhcCustomRoles():string[]{return Object.keys(HHC_CUSTOM_ROLE_REQUIREMENTS).sort()}
