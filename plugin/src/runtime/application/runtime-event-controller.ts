...
export class RuntimeEventController{
  ...
  async handle(ev:HostEvent){
    ...
    const progressed=store.updateProgress(m,false);void eventSink(runtimeSignal('mission.idle',m.identity.mission_id));let decision=evaluateIdle(m,Date.now(),projectRoot);if(!progressed&&shouldCountStagnation(decision)){store.updateProgress(m,true);decision=evaluateIdle(m,Date.now(),projectRoot)}appendLedger(m,'runtime.decision',{payload:{decision:decision.decision,reason:decision.reason,reason_code:decision.reason_code,progressed,stagnation_count:m.continuation.stagnation_count}})
