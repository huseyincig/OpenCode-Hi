...
export class RuntimeEventController{
  ...
  async handle(ev:HostEvent){
    ...
    if(mission&&child?.status==='cancelled'){appendLedger(mission,'worker.callback.after-cancel-ignored',{worker_id:child.id,payload:{session_id:sid,event:ev.rawType}});persistence.save(store.all());return}
    ...
    if(mission?.identity.status==='completed'&&ev.kind!=='session-deleted')return
