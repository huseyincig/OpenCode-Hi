...
export class RuntimeEventController{
  ...
  async handle({event}:any){
    ...
    if(child&&mission&&tasks.childCallbackDisposition(mission,child)==='stale-mission'){appendLedger(mission,'worker.callback.stale-mission-ignored',{worker_id:child?.id,payload:{worker_mission_id:child?.parent_mission_id,mission_id:mission.mission_id,worker_generation:child?.generation_at_spawn,mission_generation:mission.generation,event:ev.rawType}});persistence.save(store.all());return}
