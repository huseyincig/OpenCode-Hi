  ...
  async handle(ev:HostEvent){
    ...
    if(child){
      ...
      if(ev.kind==='session-error'||ev.kind==='session-deleted'){
        ...
        tasks.fail(m,child.id,detail);await tasks.cleanupWorkspaceForTask(m,child.task_id);store.updateProgress(m);appendLedger(m,'parent.wake',{worker_id:child.id,payload:{result:'FAILED',event:ev.rawType}})
