...
export function createRuntimeServices(input:{ports:RuntimeServicePorts;projectRoot:string;packageRoot:string;getConfig:()=>HiConfig;getModels:()=>AvailableModel[];getHostConfig:()=>Record<string,unknown>}){
  ...
  const ensureBrowserAvailable=async()=>{
    if(ports.browserTool)try{browserToolReceipt=await operationalTools.ensure('browser-execution',{authority:{source:'task-requirement',ref:'semantic:browser-execution'}});const health=await browserExecutor.health();setBrowserAvailable(health.available);return{available:health.available,attempted:browserToolReceipt.status==='provisioned',reason:health.available?undefined:health.reason,implementationId:browserToolReceipt.implementation_id,status:browserToolReceipt.status,scope:browserToolReceipt.scope,receiptPath:browserToolReceipt.receipt_path}}catch(error){setBrowserAvailable(false);return{available:false,attempted:browserBootstrapStatus?.attempted,reason:String(error),implementationId:ports.browserTool.implementationId,receiptPath:browserToolReceipt?.receipt_path}}
