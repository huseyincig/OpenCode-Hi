...
export interface ProcessSpawnAuthorityResult{decision:'ALLOW'|'ASK'|'DENY';reason:string;command_line:string;external_cwd:boolean}
...
export function evaluateProcessSpawnAuthority(request:ProcessSpawnRequest,projectRoot:string,hostConfig:Record<string,unknown>):ProcessSpawnAuthorityResult{
  ...
  if(bash!=='allow')return{decision:bash==='ask'?'ASK':'DENY',reason:`bash-permission:${bash}`,command_line:commandLine,external_cwd:externalCwd}
  if(externalCwd){const pattern=canonical(request.cwd).replace(/[\\/]$/,'')+sep+'*',decision=permissionDecision(hostConfig,request.role,'external_directory',pattern);if(decision!=='allow')return{decision:decision==='ask'?'ASK':'DENY',reason:`external-directory-permission:${decision}`,command_line:commandLine,external_cwd:true}}
