...
export interface ProcessHandle{contract:ProcessContract;host_process_id:string}
export interface ProcessOutputWindow{cursor?:number;max_chars?:number}
export interface ProcessOutput{text:string;start_cursor:number;end_cursor:number;available_start_cursor:number;available_end_cursor:number;truncated:boolean;status:ProcessContract['status']}
export interface ProcessExit{contract:ProcessContract}
export interface ProcessReconcileResult{disposition:'ADOPTED'|'TERMINAL'|'ORPHANED';contract:ProcessContract}
