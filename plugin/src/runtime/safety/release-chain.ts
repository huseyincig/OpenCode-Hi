...
function dependencyGraph(root:string):{digest:string;count:number;direct:Array<{name:string;license:string}>}|undefined{try{const lock=JSON.parse(readFileSync(join(root,'plugin','package-lock.json'),'utf8')),packages=lock?.packages??{},rm=packages['']??{},dev=new Set(Object.keys(rm.devDependencies??{})),peer=new Set(Object.keys(rm.peerDependencies??{})),rows:Array<{path:string;name:string;version:string;license:string;relation:string}>=[];for(const [path0,meta0] of Object.entries(packages)){if(!path0)continue;const meta:any=meta0??{},path=String(path0).replace(/\\/g,'/'),name=String(meta.name??(path.includes('node_modules/')?path.split('node_modules/').pop():path)??path),relation=dev.has(name)?'direct-dev':peer.has(name)?'direct-peer':'transitive';rows.push({path,name,version:String(meta.version??''),license:String(meta.license??'UNKNOWN'),relation})}rows.sort((a,b)=>a.path.localeCompare(b.path));const h=createHash('sha256');for(const r of rows){for(const k of ['path','name','version','license','relation'] as const){h.update(r[k]);h.update('\0')}h.update('\n')}return{digest:h.digest('hex'),count:rows.length,direct:rows.filter(r=>r.relation!=='transitive').map(r=>({name:r.name,license:r.license}))}}catch{return undefined}}
function validateSupplyChain(root:string,manifest:any):string[]{const issues:string[]=[],sc=manifest?.supply_chain,g=dependencyGraph(root);if(manifest?.schema<5||sc?.schema!==1||typeof sc?.dependency_graph_sha256!=='string'||typeof sc?.sbom!=='string'||typeof sc?.third_party_notices_sha256!=='string')return['release-supply-chain-missing'];if(!g||g.digest!==sc.dependency_graph_sha256||g.count!==sc.component_count)issues.push('dependency-graph-drift');const sbomPath=join(root,'dist',sc.sbom);try{const sb=JSON.parse(readFileSync(sbomPath,'utf8'));if(sb?.dependency_graph_sha256!==g?.digest||sb?.component_count!==g?.count||sha256File(sbomPath)!==sc.sbom_sha256)issues.push('sbom-drift')}catch{issues.push('sbom-missing-or-invalid')}const notices=join(root,'THIRD_PARTY_NOTICES.md'),text=existsSync(notices)?readFileSync(notices,'utf8'):'';if(sha256File(notices)!==sc.third_party_notices_sha256)issues.push('third-party-notices-drift');for(const d of g?.direct??[]){if(!text.includes('`'+d.name+'`')||(d.license!=='UNKNOWN'&&!text.includes(d.license)))issues.push('third-party-notices-incomplete:'+d.name)}return issues}
...
export function missionRequiresPackagePublish(objective:string):boolean{return /(?:\bnpm\s+publish\b|\bpnpm\s+publish\b|\byarn\s+npm\s+publish\b|\bbun\s+publish\b|\bpublish\s+(?:the\s+)?package\b|\bpaketi\s+yayınla\b)/i.test(objective)}
export function missionRequiresReleaseCreate(objective:string):boolean{return /(?:\bgh\s+release\s+create\b|\bcreate\s+(?:a\s+)?release\b|\brelease\s+it\b|\brelease\s+oluştur\b|\bsürüm\s+yayınla\b)/i.test(objective)}
...
function reevaluatePushRemote(m:MissionState):void{
  const p=m.release_chain?.push;if(!p||p.outcome!=='success'||!p.expected_remote||!p.expected_ref||!p.local_head||!p.remote_hash)return
...
function reevaluateTagPushRemote(m:MissionState):void{
  const t=m.release_chain?.tag_push;if(!t||t.outcome!=='success'||!t.expected_tag||!t.expected_commit)return
...
function reevaluateReleaseRemote(m:MissionState):void{
  ...
  const expectedAssets=m.release_chain?.quality?.assets?.map(a=>a.path)??[];const observed=new Set(r.observed_assets?.map(a=>a.name)??[]);const assetsOk=expectedAssets.length===0||expectedAssets.every(x=>observed.has(x));r.assets_verified=assetsOk
...
export function noteLocalReleaseMutation(m:MissionState,command:string,success:boolean):void{
  ...
  const at=Date.now();m.release_chain={...(m.release_chain??{}),local_revision_at:at,last_local_command:norm(command).slice(0,240),push:undefined,release:undefined,package:m.release_chain?.package?{...m.release_chain.package,remote_verified:false}:undefined,blocked_reason:undefined}
...
export function recordRemoteReleaseVerification(m:MissionState,command:string,output:any,projectRoot?:string):void{
  ...
  const pack=parsePackDryRun(command,output,projectRoot);if(pack){const current=projectRoot?packageJson(projectRoot):undefined;m.release_chain={...(m.release_chain??{}),package:{...(m.release_chain?.package??{}),name:pack.name??current?.name,version:pack.version??current?.version,pack_integrity:pack.integrity,pack_shasum:pack.shasum,pack_filename:pack.filename,pack_files:pack.files,pack_state_hash:pack.state_hash,pack_verified_at:Date.now(),remote_verified:false}};clearReleaseBlockers(m,'release-chain:package-pack-');clearReleaseBlockers(m,'release-chain:package-surface-');appendLedger(m,'release-chain.package.pack-verified',{payload:{name:pack.name,version:pack.version,integrity:pack.integrity,shasum:pack.shasum,filename:pack.filename,file_count:pack.files.length,state_hash:pack.state_hash}});return}
  const registry=parseRegistryView(command,output);if(registry&&m.release_chain?.package){const p=m.release_chain.package;p.registry_version=registry.version;p.registry_integrity=registry.integrity;p.registry_shasum=registry.shasum;appendLedger(m,'release-chain.package.registry-probe',{payload:registry});reevaluatePackageRemote(m);return}
  const upstream=parseUpstream(command,output);if(upstream&&m.release_chain?.push){m.release_chain.push.expected_remote=upstream.remote;m.release_chain.push.expected_ref=upstream.ref;appendLedger(m,'release-chain.push.upstream',{payload:upstream});reevaluatePushRemote(m);return}
  const head=parseHead(command,output);if(head&&m.release_chain?.push){m.release_chain.push.local_head=head;appendLedger(m,'release-chain.push.local-head',{payload:{hash:head}});reevaluatePushRemote(m);return}
  const tagProbe=parseRemoteTagProbe(command,output);if(tagProbe){let handled=false;const tp=m.release_chain?.tag_push;if(tp&&(!tp.expected_tag||tp.expected_tag===tagProbe.tag)){tp.observed_remote=tagProbe.remote;tp.direct_tag_hash=tagProbe.direct_hash;tp.peeled_tag_hash=tagProbe.peeled_hash;appendLedger(m,'release-chain.tag-push.tag-probe',{payload:{remote:tagProbe.remote,tag:tagProbe.tag,direct_hash:tagProbe.direct_hash,peeled_hash:tagProbe.peeled_hash}});reevaluateTagPushRemote(m);handled=true}if(m.release_chain?.release){const r=m.release_chain.release;r.observed_remote=tagProbe.remote;r.direct_tag_hash=tagProbe.direct_hash;r.peeled_tag_hash=tagProbe.peeled_hash;appendLedger(m,'release-chain.release.tag-probe',{payload:{remote:tagProbe.remote,tag:tagProbe.tag,direct_hash:tagProbe.direct_hash,peeled_hash:tagProbe.peeled_hash}});reevaluateReleaseRemote(m);handled=true}if(handled)return}
  const remote=parseLsRemote(command,output);if(remote&&m.release_chain?.push){const p=m.release_chain.push;p.observed_remote=remote.remote;p.observed_ref=remote.ref;p.remote_hash=remote.hash;appendLedger(m,'release-chain.push.remote-probe',{payload:{remote:remote.remote,ref:remote.ref,hash:remote.hash}});reevaluatePushRemote(m);return}
  const rv=parseReleaseView(command,output);if(rv&&m.release_chain?.release){const r=m.release_chain.release;r.observed_tag=rv.tag;r.observed_target=rv.target;r.observed_assets=rv.assets;r.view_verified=!!r.expected_tag&&rv.tag===r.expected_tag&&(!r.expected_target||!rv.target||(hex40(r.expected_target)?rv.target.toLowerCase()===r.expected_target.toLowerCase():rv.target===r.expected_target));appendLedger(m,r.view_verified?'release-chain.release.view-verified':'release-chain.release.view-drift',{payload:{expected_tag:r.expected_tag,expected_target:r.expected_target,observed_tag:rv.tag,target:rv.target,assets:rv.assets?.map(a=>a.name)}});reevaluateReleaseRemote(m)}
}
