...
export function assertSafeGitMutation(m:MissionState,command:string):void{
  ...
  const proof=m.staging_safety;if(!proof||Date.now()-proof.verified_at>120000){appendLedger(m,'git.commit.blocked',{payload:{reason:'staging-not-verified'}});throw new Error('Hi staging safety: inspect the exact staged set with `git diff --cached --name-only` immediately before commit. Commit is blocked until staged ownership is verified.')}
