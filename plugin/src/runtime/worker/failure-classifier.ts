...
function structured(error:unknown):{name:string;text:string;isRetryable?:boolean;statusCode?:number}{
  const value=error&&typeof error==='object'?error as any:undefined,name=String(value?.name??'').trim(),message=String(value?.message??error??'').trim(),isRetryable=typeof value?.isRetryable==='boolean'?value.isRetryable:undefined,statusCode=Number.isInteger(value?.statusCode)&&value.statusCode>=0?value.statusCode:undefined
...
export function classifyWorkerFailure(error:unknown):ClassifiedWorkerFailure{
  ...
  if(/rate.?limit|429|quota|provider|upstream|gateway|503|502|network[-_\s]?error|connection|timeout|timed out|transport|socket|econn|temporar/.test(text))return{kind:'provider-transport',stagnation:false,retryable:true,reason:'legacy-provider-or-transport-failure'}
