...
export function evaluateIdle(m:MissionState|undefined,now=Date.now()):DecisionResult{
  ...
  const completion=adjudicateCompletion(m);if(completion.complete)return{decision:'STOP',reason:'complete',reason_code:'complete'}
  ...
  const recovery=recoveryPlan(m);if(m.iteration>=m.continuation_budget||recovery.action==='user-action')return{decision:'USER_ACTION_REQUIRED',reason:'bounded-autonomy-exhausted',reason_code:'bounded-autonomy-exhausted'}
...
export function continuationPrompt(m:MissionState,action:string):string{const open=m.obligations.filter(o=>o.status==='open').map(o=>o.summary).slice(0,3);return['HHC runtime: mission is still active.',`Open obligation: ${open.join(' | ')||'none'}.`,action,'Resume from current state. Do not restart planning. Do not create duplicate tasks.'].join('\n')}
