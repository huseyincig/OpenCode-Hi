...
export function methodologyProvenance(candidates:SkillCandidate[]):MethodologyProvenance[]{
  const now=Date.now()
  return candidates.map(candidate=>{
    let digest:string|undefined
    try{digest=sha256(readFileSync(candidate.path,'utf8'))}catch{}
...
export function ownershipContract(kind:'parent'|'child',methodologies:string[]=[]):string{
