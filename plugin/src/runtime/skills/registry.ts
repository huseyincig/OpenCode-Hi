...
export function discoverSkills(projectRoot:string,hiRoot?:string,extraPaths:string[]=[]):SkillCandidate[]{const home=process.env.HOME??process.env.USERPROFILE??'',opencodeConfigDir=process.env.OPENCODE_CONFIG_DIR?resolve(process.env.OPENCODE_CONFIG_DIR):join(home,'.config','opencode'),roots:[string,SkillProvider][]=[[join(projectRoot,'.opencode','skills'),'project'],[join(projectRoot,'.claude','skills'),'project'],[join(projectRoot,'.agents','skills'),'project'],...(hiRoot?[[join(hiRoot,'skills'),'hi'] as [string,SkillProvider]]:[]),[join(opencodeConfigDir,'skills'),'personal'],[join(home,'.claude','skills'),'personal'],[join(home,'.agents','skills'),'personal'],...extraPaths.map(x=>[x,'personal'] as [string,SkillProvider])];const out:SkillCandidate[]=[];for(const [root,provider] of roots)out.push(...inspectDir(root,provider));return out}
...
export function resolveSkillPlan(capabilities:string[],candidates:SkillCandidate[],permissionMap?:Record<string,SkillPermission>,skillToolEnabled=true,role='coder'):SkillPlan{
  ...
  for(const name of requested){const permission=resolveSkillPermission(name,permissionMap),all=candidates.filter(c=>c.name===name),ordered=[...all.filter(c=>c.provider==='hi'),...all.filter(c=>c.provider==='project'),...all.filter(c=>c.provider==='personal')],candidate=ordered.find(c=>c.valid&&c.enabled);let outcome:SkillPreflightOutcome;if(!all.length)outcome='missing';else if(!skillToolEnabled)outcome='disabled';else if(!all.some(x=>x.valid))outcome='invalid';else if(permission==='deny')outcome='deny';else if(candidate)outcome=permission==='ask'?'ask':'allow';else outcome='missing';outcomes.push({name,outcome,provider:candidate?.provider,path:candidate?.path});if(candidate&&skillToolEnabled&&permission!=='deny')selected.push({...candidate,permission});else missing.push(name)}
