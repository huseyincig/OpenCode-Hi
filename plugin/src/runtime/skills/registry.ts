...
const ROLE_CAPABILITY_SKILLS:Record<string,Record<string,string[]>>={
  coder:{
...
function validSkillFrontmatter(text:string,name:string):boolean{const m=text.match(/^---\s*\n([\s\S]*?)\n---\s*\n/);if(!m)return false;const fm=m[1],n=fm.match(/^name:\s*["']?([^"'\n]+)["']?\s*$/m)?.[1]?.trim(),d=fm.match(/^description:\s*(.+)$/m)?.[1]?.trim();return n===name&&Boolean(d)}
...
export function discoverSkills(projectRoot:string,hhcRoot?:string,extraPaths:string[]=[]):SkillCandidate[]{const home=process.env.HOME??process.env.USERPROFILE??'',opencodeConfigDir=process.env.OPENCODE_CONFIG_DIR?resolve(process.env.OPENCODE_CONFIG_DIR):join(home,'.config','opencode'),roots:[string,SkillProvider][]=[[join(projectRoot,'.opencode','skills'),'project'],[join(projectRoot,'.claude','skills'),'project'],[join(projectRoot,'.agents','skills'),'project'],...(hhcRoot?[[join(hhcRoot,'skills'),'hhc'] as [string,SkillProvider]]:[]),[join(opencodeConfigDir,'skills'),'personal'],[join(home,'.claude','skills'),'personal'],[join(home,'.agents','skills'),'personal'],...extraPaths.map(x=>[x,'personal'] as [string,SkillProvider])];const out:SkillCandidate[]=[];for(const [root,provider] of roots)out.push(...inspectDir(root,provider));return out}
function desiredFor(role:string,capabilities:string[]):string[]{const map=ROLE_CAPABILITY_SKILLS[role]??GENERIC;return[...new Set(capabilities.flatMap(c=>map[c]??GENERIC[c]??[]))]}
export function resolveSkillPlan(capabilities:string[],candidates:SkillCandidate[],permissionMap?:Record<string,SkillPermission>,skillToolEnabled=true,role='coder'):SkillPlan{
  const requested=desiredFor(role,capabilities).slice(0,3),selected:SkillCandidate[]=[],missing:string[]=[],outcomes:SkillPreflightResult[]=[]
  for(const name of requested){const permission=resolveSkillPermission(name,permissionMap),all=candidates.filter(c=>c.name===name),ordered=[...all.filter(c=>c.provider==='hhc'),...all.filter(c=>c.provider==='project'),...all.filter(c=>c.provider==='personal')],candidate=ordered.find(c=>c.valid&&c.enabled);let outcome:SkillPreflightOutcome;if(!all.length)outcome='missing';else if(!skillToolEnabled)outcome='disabled';else if(!all.some(x=>x.valid))outcome='invalid';else if(permission==='deny')outcome='deny';else if(candidate)outcome=permission==='ask'?'ask':'allow';else outcome='missing';outcomes.push({name,outcome,provider:candidate?.provider,path:candidate?.path});if(candidate&&skillToolEnabled&&permission!=='deny')selected.push({...candidate,permission});else missing.push(name)}
...
export function selectSkills(capabilities:string[],candidates:SkillCandidate[]):SkillCandidate[]{return resolveSkillPlan(capabilities,candidates).selected}
