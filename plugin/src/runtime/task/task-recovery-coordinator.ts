  ...
  async recoverHostTerminalFailure(m:MissionState,workerID:string,error:unknown):Promise<HostTerminalRecoveryDisposition>{
    ...
    for(const model of candidates){
      ...
      try{
        ...
        beginWorkerAttempt(task,worker);await this.child.sendProviderPrompt(recoverySessionID,prompt,worker.role,model==='host-default'?undefined:model,variant,taskPromptToolOverrides(task.execution_profile?.tools??[],this.getHostConfig(),task.execution_profile?.mcp_servers??[]))
