...
export class TaskRecoveryCoordinator{
  ...
  async recoverCanonicalStall(m:MissionState,assessment:MissionLivenessAssessment):Promise<{disposition:'NOOP'|'RECOVERED';reason:string;worker_id?:string;task_id?:string}>{
    ...
    if(status!=='idle'){appendLedger(m,'runtime.liveness-recovery.deferred',{task_id:task.id,worker_id:worker.id,payload:{reason:'exact-old-execution-not-quiescent',host_status:status}});return{disposition:'NOOP',reason:status==='unknown'?'old-execution-reconcile-required':'old-execution-still-active',worker_id:worker.id,task_id:task.id}}
  ...
  async recoverStagnation(m:MissionState,level:number,action:'same-worker-resume'|'model-escalation'='same-worker-resume'):Promise<boolean>{
    ...
    if(action==='same-worker-resume'){const eligibility=recoveryStrategyEligibility(m,{level:level as 1|2,action});if(!eligibility.allowed){appendLedger(m,'worker.stagnation-recovery.rejected',{task_id:task.id,worker_id:worker.id,payload:{level,action,reason:eligibility.reason,progress_signature:eligibility.progress_signature}});return false}}
    ...
    if(action==='model-escalation'){
      ...
      try{
        ...
        worker.loaded_methodologies=[];worker.model=next;worker.model_variant=nextVariant;worker.fallback_history=[...(worker.fallback_history??[]),{from:model,to:next,variant:nextVariant,reason:'recovery-only model escalation after one repeated same-failure correction without semantic gain',phase:'runtime',at:Date.now()}];worker.status='busy';worker.generation_at_spawn=m.continuation.generation;worker.parent_mission_id=m.identity.mission_id;worker.started_at=Date.now();task.status='running';this.registry.set(worker)
    ...
    try{
      ...
      beginWorkerAttempt(task,worker);this.child.recordModelProjection(worker,model,variant);await this.child.sendProviderPrompt(worker.session_id,clipText([instruction,...taskSpecificResultContractInstructions(task,worker.role),'Return the normal structured WorkerResult.'].join('\n'),DEFAULT_CONTEXT_BUDGET.max_handoff_chars),worker.role,model==='host-default'?undefined:model,variant,taskPromptToolOverrides(task.execution_profile?.tools??[],this.getHostConfig(),task.execution_profile?.mcp_servers??[]),worker.attempt_prompt_message_id)
      recordRecoveryStrategy(m,{level:level as 1|2,action},'started',Date.now(),{task_id:task.id,worker_id:worker.id,model,failure_signature:sameModelFailureSignature})
  ...
  private providerRecoveryCandidates(worker:MissionState['execution']['workers'][number]):string[]{
  ...
  private async launchProviderRecoveryCandidate(m:MissionState,worker:MissionState['execution']['workers'][number],task:MissionState['execution']['tasks'][number],model:string,failedSession:string|undefined,failureKind:string,reason:string):Promise<'RECOVERED'|'QUARANTINED'|'FAILED'>{
    const runtimeCandidate=runtimeModelCandidateStatus(model,this.getModels(),this.getConfig(),this.getHostConfig(),worker.role);if(!runtimeCandidate.ok){appendLedger(m,'worker.runtime-fallback.skipped',{task_id:task.id,worker_id:worker.id,payload:{model,reason:runtimeCandidate.reason,failure_class:failureKind,phase:'runtime-policy-revalidation'}});return'FAILED'}
    const variant=task.execution_profile?.fallback_variants?.[model],previous=worker.model,reservation=reserveTaskRuntimeDispatch(m,worker,model,this.scheduler,Date.now(),this.getProjectPeerView(m));if(!reservation.accepted){appendLedger(m,'worker.runtime-fallback.skipped',{task_id:task.id,worker_id:worker.id,payload:{model,reason:reservation.reason,failure_class:failureKind,source:'scheduler'}});return'FAILED'}
    try{
      ...
      worker.loaded_methodologies=[];worker.model=model;worker.model_variant=variant;worker.fallback_history=[...(worker.fallback_history??[]),{from:previous,to:model,variant,reason:`${reason}; failure=${failureKind}`,phase:'runtime',at:Date.now()}];worker.status='busy';worker.runtime_fallback_exhausted=false;worker.runtime_recovery_pending=true;worker.runtime_recovery_attempt=(worker.runtime_recovery_attempt??0)+1;worker.generation_at_spawn=m.continuation.generation;worker.parent_mission_id=m.identity.mission_id;worker.started_at=Date.now();task.status='running';this.registry.set(worker)
      const resolvedProviderBlockers=new Set((task.result?.open_issues??[]).filter(issue=>issue.startsWith('provider-failure:provider-transport:')));m.execution.blockers=m.execution.blockers.filter(blocker=>!resolvedProviderBlockers.has(blocker))
      recordPreexistingUserBaseline(m,await this.child.captureNativeDiff(worker,'baseline'))
      const exitRequirements=worker.selected_methodologies.flatMap(name=>{const item=methodologyCatalog(this.projectRoot).find(x=>x.name===name);return item?[`${name}: ${item.exitRequirements.join(', ')}`]:[]})
      const prompt=clipText([ownershipContract('child',worker.selected_methodologies),`Hi terminal runtime recovery for existing task ${task.id}.`,`Failure class: ${failureKind}.`,failedSession?`Previous failed session: ${failedSession}.`:'Previous provider-terminal session is already quiescent and detached.',`Fallback model: ${model}.`,`Recovery authority: ${reason}.`,`OBJECTIVE: ${task.objective}`,`SCOPE: ${task.scope.join(', ')||'bounded by objective'}`,`CURRENT USER CONSTRAINTS: ${(task.constraints??[]).join(' | ')||'none'}.`,`OBSERVED CHANGED FILES SO FAR: ${m.vcs.changed_files.slice(-30).join(', ')||'none'}`,`METHODOLOGY EXIT REQUIREMENTS: ${exitRequirements.join(' | ')||'none'}`,...taskSpecificResultContractInstructions(task,worker.role),worker.selected_methodologies.length?'This is a fresh child session. Reload every still-selected methodology through the native skill tool before applying it.':'No methodology is selected for this recovery.','Preserve already-observed repository changes and bounded evidence, but do not assume the failed session context is present. Inspect only the minimum current state needed to continue the SAME task. Do not restart top-level planning. Return the normal structured WorkerResult.'].join('\n'),DEFAULT_CONTEXT_BUDGET.max_handoff_chars)
      beginWorkerAttempt(task,worker);await this.child.sendProviderPrompt(recoverySessionID,prompt,worker.role,model==='host-default'?undefined:model,variant,taskPromptToolOverrides(task.execution_profile?.tools??[],this.getHostConfig(),task.execution_profile?.mcp_servers??[]),worker.attempt_prompt_message_id)
      ...
      appendLedger(m,'worker.runtime-fallback.failed',{task_id:task.id,worker_id:worker.id,payload:{model,error:String(nextError),failure_class:failureKind,from_session:failedSession,host_stopped:recoveryStopped}});return recoveryStopped?'FAILED':'QUARANTINED'
    }
  }
  ...
  async dispatchPendingTextTransportRecovery(m:MissionState,workerID:string):Promise<'DISPATCHED'|'QUARANTINED'|'NOOP'>{
    ...
    if(!worker||!task||!pending)return'NOOP'
    ...
    if(!current){delete worker.pending_text_transport_recovery;this.registry.set(worker);appendLedger(m,'worker.output-transport-fallback.pending-stale',{task_id:task.id,worker_id:worker.id,payload:{receipt_session_id:pending.session_id,worker_session_id:worker.session_id,receipt_model:pending.model,worker_model:worker.model,receipt_attempt:pending.source_attempt,worker_attempt:worker.attempt,receipt_generation:pending.generation,worker_generation:worker.generation_at_spawn}});return'NOOP'}
    ...
    delete worker.pending_text_transport_recovery;worker.text_transport_fallback_models=[...new Set([...(worker.text_transport_fallback_models??[]),model])];worker.runtime_recovery_pending=true;worker.runtime_recovery_attempt=(worker.runtime_recovery_attempt??0)+1;worker.status='busy';task.status='running';beginWorkerAttempt(task,worker);this.child.recordModelProjection(worker,worker.model,worker.model_variant);this.registry.set(worker)
    try{await this.child.sendProviderPrompt(sessionID,prompt,worker.role,worker.model==='host-default'?undefined:worker.model,worker.model_variant,taskPromptToolOverrides(task.execution_profile?.tools??[],this.getHostConfig(),task.execution_profile?.mcp_servers??[]),worker.attempt_prompt_message_id,'text');appendLedger(m,'worker.output-transport-fallback',{task_id:task.id,worker_id:worker.id,payload:{model,session_id:sessionID,from:'opencode-json-schema',to:'text-compatibility',reason:pending.reason,source_attempt:pending.source_attempt,attempt:worker.attempt,generation:worker.generation_at_spawn,trigger:'admitted-session-idle'}});return'DISPATCHED'}
    catch(transportError){worker.runtime_recovery_pending=false;worker.restart_reconcile_pending=true;worker.status='ready';task.status='waiting';this.registry.set(worker);appendLedger(m,'worker.output-transport-fallback.dispatch-uncertain',{task_id:task.id,worker_id:worker.id,payload:{model,session_id:sessionID,error:String(transportError),reason:pending.reason,attempt:worker.attempt,policy:'exact-session-effect-reconcile-before-retry'}});return'QUARANTINED'}
  }
  async recoverHostTerminalFailure(m:MissionState,workerID:string,error:unknown):Promise<HostTerminalRecoveryDisposition>{
    ...
    if(failure.kind==='host-interruption'&&worker.session_id&&task&&taskOwnsUnresolvedRecoveryWork(m,worker)){
      releaseTaskRuntimeReservation(m,worker.id);worker.runtime_recovery_pending=false;worker.runtime_fallback_exhausted=false;worker.restart_reconcile_pending=true;worker.status='ready';task.status='waiting';task.updated_at=Date.now();this.registry.set(worker)
    ...
    if(failure.kind==='provider-transport'&&failure.reason==='opencode-required-tool-choice-compatibility-fallback-eligible'&&worker.session_id&&task){
      ...
      if(!alreadyTried&&!pending){
        worker.pending_text_transport_recovery={session_id:worker.session_id,model,source_attempt:worker.attempt,generation:worker.generation_at_spawn??m.continuation.generation,reason:failure.reason,requested_at:Date.now()};worker.runtime_recovery_pending=false;worker.status='busy';task.status='running';this.registry.set(worker)
      ...
      if(pending&&pending.session_id===worker.session_id&&pending.model===model){appendLedger(m,'worker.output-transport-fallback.pending-duplicate-ignored',{task_id:task.id,worker_id:worker.id,payload:{model,session_id:worker.session_id,source_attempt:pending.source_attempt,generation:pending.generation}});return'RECOVERED'}
      if(alreadyTried)appendLedger(m,'worker.output-transport-fallback.exhausted',{task_id:task.id,worker_id:worker.id,payload:{model,session_id:worker.session_id,reason:failure.reason}})
    }
    ...
    for(const model of candidates){
      ...
      const result=await this.launchProviderRecoveryCandidate(m,worker,task,model,failedSession,failure.kind,reason);if(result==='RECOVERED')return'RECOVERED';if(result==='QUARANTINED')return'QUARANTINED'
    }
    worker.runtime_fallback_exhausted=true;m.continuation.stagnation_count=0;const blocker=`provider-failure:${failure.kind}:${worker.model??'unknown'}`;m.execution.blockers=[...new Set([...m.execution.blockers,blocker])];task.status='blocked';task.updated_at=Date.now();task.result={status:'BLOCKED',summary:'Runtime provider/model fallback chain exhausted.',changed_files:[],evidence:[],open_issues:[blocker],needs_context:['provider/model availability or alternate execution path']};appendLedger(m,'worker.runtime-fallback.exhausted',{task_id:task.id,worker_id:worker.id,payload:{failure_class:failure.kind,attempted:[...new Set([worker.model,...(worker.fallback_history??[]).flatMap(x=>[x.from,x.to]),...candidates].filter(Boolean))]}});return'NOT_RECOVERED'
  }
  ...
  fail(m:MissionState,workerID:string,error:string):void{
    ...
    if(worker.generation_at_spawn!==undefined&&worker.generation_at_spawn!==m.continuation.generation){appendLedger(m,'worker.failure.stale-generation-ignored',{worker_id:worker.id});return}
    const settlement=beginTaskRuntimeSettlement(m,worker);if(!settlement.accepted&&settlement.reason!=='reservation-not-found'){appendLedger(m,'worker.failure.scheduler-fence-rejected',{worker_id:worker.id,payload:{reason:settlement.reason}});return}
