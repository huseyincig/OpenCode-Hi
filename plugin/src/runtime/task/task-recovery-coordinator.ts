...
export class TaskRecoveryCoordinator{
  ...
  async recoverStalledAwaitWorker(m:MissionState):Promise<{disposition:'NOOP'|'RECOVERED'|'QUARANTINED'|'BLOCKED';reason:string;worker_id?:string;task_id?:string;from_model?:string;to_model?:string}>{
    ...
    for(const worker of candidates){
      ...
      const blockAfterAbort=(reason:string,detail?:string)=>{const marker=`host-liveness-recovery:${reason}:${task.id}`;worker.status='ready';worker.session_id=undefined;task.status='blocked';task.updated_at=Date.now();task.result={status:'BLOCKED',summary:`Bounded host-liveness recovery could not continue: ${detail??reason}`.slice(0,1200),changed_files:[],evidence:[],open_issues:[marker],needs_context:['an admissible recovery model or repaired runtime capability is required before this task can continue']};m.execution.blockers=[...new Set([...m.execution.blockers,marker])];this.registry.set(worker);syncMissionGates(m);appendLedger(m,'worker.busy-stall-recovery.blocked',{task_id:task.id,worker_id:worker.id,payload:{session_id:fromSession,attempt:worker.attempt,model:fromModel,reason,detail}});return{disposition:'BLOCKED' as const,reason,worker_id:worker.id,task_id:task.id,from_model:fromModel}}
      const stopped=await this.child.abortNativeSession(m,fromSession,'bounded-await-host-liveness-stall',worker.id,task.id)
      if(!stopped){appendLedger(m,'worker.busy-stall-recovery.quarantined',{task_id:task.id,worker_id:worker.id,payload:{session_id:fromSession,attempt:worker.attempt,model:fromModel,host_status:hostStatus,await_timeouts:timeouts.length,observed_wait_ms:total,reason:'abort-unavailable'}});return{disposition:'QUARANTINED',reason:'abort-unavailable',worker_id:worker.id,task_id:task.id,from_model:fromModel}}
      const browserClean=await this.cleanupBrowser?.(m,task.id,worker.id)??true
      const released=releaseTaskRuntimeReservation(m,worker.id);worker.session_id=undefined;worker.status='ready';task.status='waiting';this.registry.set(worker)
      if(!released.accepted){appendLedger(m,'worker.busy-stall-recovery.quarantined',{task_id:task.id,worker_id:worker.id,payload:{session_id:fromSession,attempt:worker.attempt,reason:released.reason}});return{disposition:'QUARANTINED',reason:released.reason,worker_id:worker.id,task_id:task.id,from_model:fromModel}}
      ...
      try{
        ...
        const created=await this.child.create(m.identity.session_id,`Hi · ${worker.role} · liveness recovery · ${task.objective.slice(0,40)}`,worker.role,next==='host-default'?undefined:next,nextVariant,this.workspaceBinding?.(m,task.id));if(!created.id)throw new Error('Host-liveness recovery child session id missing')
        const recoverySessionID=String(created.id);worker.session_id=recoverySessionID;worker.forked_from_session_id=fromSession
        const bound=bindTaskRuntimeHost(m,worker.id,recoverySessionID);if(!bound.accepted)throw new Error(`Scheduler host binding failed during host-liveness recovery: ${bound.reason}`)
        worker.loaded_methodologies=[];worker.model=next;worker.model_variant=nextVariant;worker.fallback_history=[...(worker.fallback_history??[]),{from:fromModel,to:next,variant:nextVariant,reason:'bounded host-liveness stall after repeated task await timeouts',phase:'runtime',at:Date.now()}];worker.runtime_recovery_pending=true;worker.runtime_recovery_attempt=(worker.runtime_recovery_attempt??0)+1;worker.status='busy';worker.generation_at_spawn=m.continuation.generation;worker.parent_mission_id=m.identity.mission_id;task.status='running';this.registry.set(worker)
        ...
        appendLedger(m,'worker.busy-stall-recovery',{task_id:task.id,worker_id:worker.id,payload:{from:fromModel,to:next,from_session:fromSession,to_session:recoverySessionID,prior_attempt:worker.attempt-1,current_attempt:worker.attempt,host_status:hostStatus,await_timeouts:timeouts.length,observed_wait_ms:total}});void this.events?.(runtimeSignal('worker.recovered',m.identity.mission_id,{task_id:task.id,worker_id:worker.id,payload:{reason:'host-liveness-stall',from:fromModel,to:next}}));return{disposition:'RECOVERED',reason:'host-liveness-stall',worker_id:worker.id,task_id:task.id,from_model:fromModel,to_model:next}
      }catch(error){let replacementStopped=true;if(worker.session_id)try{replacementStopped=await this.child.abortNativeSession(m,worker.session_id,'host-liveness-recovery-failed',worker.id,task.id)}catch{replacementStopped=false};if(replacementStopped){releaseTaskRuntimeReservation(m,worker.id);worker.forked_from_session_id=priorFork;worker.model=fromModel;worker.model_variant=priorVariant;return{...blockAfterAbort('replacement-dispatch-failed',String(error)),to_model:next}}worker.status='busy';task.status='running';this.registry.set(worker);appendLedger(m,'worker.busy-stall-recovery.failed',{task_id:task.id,worker_id:worker.id,payload:{from:fromModel,to:next,error:String(error),replacement_stopped:false}});return{disposition:'QUARANTINED',reason:String(error),worker_id:worker.id,task_id:task.id,from_model:fromModel,to_model:next}}
    }
