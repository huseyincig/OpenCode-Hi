...
function missionModelFeedback(m:MissionState){
  ...
  for(const w of m.workers){const observed=w.effective_model??w.model;if(w.status==='completed')inc(successes,observed);if(w.status==='failed')inc(failures,observed);if(w.last_runtime_failure_kind&&w.model)inc(failures,w.model);for(const h of w.fallback_history??[]){inc(retries,h.from);if(/failure=|provider|transport|tool|context/i.test(h.reason))inc(failures,h.from)}}
...
export class TaskRecoveryCoordinator{
  callbackDisposition(m:MissionState,worker:{parent_mission_id?:string;generation_at_spawn?:number;restart_reconcile_pending?:boolean}):ChildCallbackDisposition{if(worker.restart_reconcile_pending)return'restart-reconcile-pending';if((worker.parent_mission_id!==undefined&&worker.parent_mission_id!==m.mission_id)||(worker.generation_at_spawn!==undefined&&worker.generation_at_spawn!==m.generation))return'stale-mission';return'accept'}
  ...
  async recoverRuntimeFailure(m:MissionState,workerID:string,error:string):Promise<boolean>{
    ...
    for(const model of candidates){
      ...
      try{
        ...
        this.child.recordModelProjection(worker,model,variant);const child=await this.child.create(m.session_id,`Hi · ${worker.role} · runtime recovery · ${task.objective.slice(0,45)}`,worker.role,model==='host-default'?undefined:model,variant)
