...
export class TaskResultReconciler{
  ...
  applyResult(m:MissionState,workerID:string,result:WorkerResult):void{
    ...
    if(effectiveResult.findings?.length){
      const findings=effectiveResult.findings,reviewerRole=isHiReviewerRole(worker.role)
      if(!reviewerRole){
        appendLedger(m,'review.finding-authority-ignored',{task_id:task.id,worker_id:worker.id,payload:{findings:findings.map(f=>f.id),worker_role:worker.role,reason:'structured-review-findings-require-canonical-reviewer-role'}})
        effectiveResult={...effectiveResult,findings:undefined}
      }else{
        ...
        if(actionableMarkers.length&&!reportOnlyReview){effectiveResult={...effectiveResult,status:'FIX_REQUIRED',open_issues:[...new Set([...effectiveResult.open_issues,...actionableMarkers])]};appendLedger(m,'review.finding-actionable',{task_id:task.id,worker_id:worker.id,payload:{findings:actionable.map(f=>({id:f.id,severity:f.severity,causality:f.causality,blocking:f.blocking,scope:f.scope.slice(0,20)}))}})}
    ...
    if(worker.role==='visual-qa'&&requiredVerificationCases.length&&effectiveResult.status==='DONE'){
      const coverage=effectiveResult.verification_coverage??[],coverageIDs=coverage.map(c=>c.case_id),duplicateCoverageIDs=[...new Set(coverageIDs.filter((id,index)=>coverageIDs.indexOf(id)!==index))],byID=new Map(coverage.map(c=>[c.case_id,c])),actionForRef=(ref:string):string|undefined=>{const event=[...m.execution.ledger].reverse().find(event=>event.type==='browser.observation-recorded'&&event.task_id===task.id&&event.worker_id===worker.id&&event.payload?.evidence_ref===ref);return typeof event?.payload?.action==='string'?event.payload.action:undefined},failures:string[]=[],unboundDetails:Array<{case_id:string;invalid_refs:string[];unique_prefix_corrections:Array<{from:string;to:string}>}>=[]
      const currentBrowserRefs=m.execution.evidence.items.filter(item=>item.task_id===task.id&&item.trusted_source_class==='browser-observation'&&item.kind==='browser-evidence'&&!item.invalidated_at&&item.outcome!=='failed'&&item.pass!==false&&item.producer_attempt?.worker_id===worker.id&&evidenceClaimApplicability(m,item).applicable).map(item=>item.id)
      for(const required of requiredVerificationCases){const claim=byID.get(required.id);if(!claim){failures.push(`${required.id}:missing`);continue}if(claim.outcome!=='passed'){failures.push(`${required.id}:failed`);continue}const refs=[...new Set(claim.evidence_refs)],supportByRef=refs.map(ref=>m.execution.evidence.items.find(item=>item.id===ref)),support=supportByRef.filter((item):item is NonNullable<typeof item>=>Boolean(item)),actions=new Set(refs.map(actionForRef).filter((x):x is string=>Boolean(x))),missingActions=required.required_browser_actions.filter(action=>!actions.has(action)),validItem=(item:NonNullable<(typeof supportByRef)[number]>):boolean=>item.trusted_source_class==='browser-observation'&&item.kind==='browser-evidence'&&!item.invalidated_at&&item.outcome!=='failed'&&item.pass!==false&&item.task_id===task.id&&evidenceClaimApplicability(m,item).applicable,invalidRefs=refs.filter((_,index)=>{const item=supportByRef[index];return!item||!validItem(item)}),validRefs=refs.length>0&&invalidRefs.length===0&&support.length===refs.length
        if(!validRefs){const corrections=invalidRefs.flatMap(from=>{if(!/^ev_[a-z0-9_]+$/i.test(from))return[];const matches=currentBrowserRefs.filter(to=>to!==from&&to.startsWith(from));return matches.length===1?[{from,to:matches[0]}]:[]});unboundDetails.push({case_id:required.id,invalid_refs:invalidRefs.slice(0,20),unique_prefix_corrections:corrections.slice(0,20)})}
        if(!validRefs||missingActions.length)failures.push(`${required.id}:${!validRefs?'unbound-refs':`missing-actions=${missingActions.join('+')}`}`)}
      if(duplicateCoverageIDs.length)failures.push(`duplicate-cases=${duplicateCoverageIDs.join(',')}`);const unknown=coverage.filter(c=>!requiredVerificationCases.some(r=>r.id===c.case_id)).map(c=>c.case_id);if(unknown.length)failures.push(`unknown-cases=${unknown.join(',')}`)
      if(failures.length){const markers=failures.map(x=>`visual-coverage-unsatisfied:${task.id}:${x}`),invalidSummary=unboundDetails.flatMap(item=>item.invalid_refs.map(ref=>`${item.case_id}:${ref}`)).slice(0,24),correctionSummary=unboundDetails.flatMap(item=>item.unique_prefix_corrections.map(x=>`${x.from}->${x.to}`)).slice(0,24),refCorrection=unboundDetails.length?clipText(`visual-evidence-ref-correction: verification_coverage evidence_refs must be FULL opaque current-attempt evidence_ref tokens copied verbatim from Hi browser tool outputs, including every suffix segment. Invalid refs: ${invalidSummary.join(', ')||'unknown'}.${correctionSummary.length?` Rejected-attempt unique-prefix format examples: ${correctionSummary.join(', ')}.`:''} Prefixes/abbreviations are never accepted. A corrective resume opens a new attempt, so prior-attempt refs are stale: rerun each required browser action in the new attempt and copy the NEW full evidence_ref tokens exactly.`,1200):undefined;effectiveResult={...effectiveResult,status:'FIX_REQUIRED',evidence:effectiveResult.evidence.map(e=>['browser-evidence','visual-evidence','accessibility-evidence'].includes(e.kind)&&evidenceVerdictPassed(e.pass,e.outcome)?{...e,pass:undefined,outcome:'pending' as const,reason:`visual-coverage-unsatisfied: ${failures.join('; ')}`}:e),open_issues:[...new Set([...effectiveResult.open_issues,...markers])],needs_context:[...new Set([...effectiveResult.needs_context,`visual-verification-coverage: satisfy every required case with current-attempt browser evidence refs and required actions; ${failures.join('; ')}`,...(refCorrection?[refCorrection]:[])])]};appendLedger(m,'visual.coverage-rejected',{task_id:task.id,worker_id:worker.id,payload:{required_cases:requiredVerificationCases.map(c=>({id:c.id,actions:c.required_browser_actions})),reported_cases:coverage.map(c=>c.case_id),failures,unbound_refs:unboundDetails.map(item=>({case_id:item.case_id,invalid_refs:item.invalid_refs,unique_prefix_corrections:item.unique_prefix_corrections}))}})}else appendLedger(m,'visual.coverage-admitted',{task_id:task.id,worker_id:worker.id,payload:{cases:requiredVerificationCases.map(c=>c.id)}})
    }
    ...
    const browserTargetApplicable=(items:MissionState['execution']['evidence']['items']):boolean=>!requiredBrowserOrigins.length||items.some(item=>Boolean(item.browser_origin)&&requiredBrowserOrigins.includes(item.browser_origin!))
    if(worker.role!=='visual-qa'&&effectiveResult.evidence.some(e=>browserProofKinds.has(e.kind))){appendLedger(m,'browser.evidence-owner-rejected',{task_id:task.id,worker_id:worker.id,payload:{role:worker.role,reason:'browser-derived evidence requires exact visual-qa worker ownership'}});effectiveResult={...effectiveResult,evidence:effectiveResult.evidence.filter(e=>!browserProofKinds.has(e.kind))}}
    ...
    const reconciledEvidence=effectiveResult.evidence.map(e=>{
      ...
      if(valid){if(rawRequested.some((id,i)=>id!==requested[i])||supplemental.length)appendLedger(m,'browser.evidence-ref-normalized',{task_id:task.id,worker_id:worker.id,payload:{from:rawRequested.slice(0,20),to:authoritative.slice(0,20),supplemental_refs:supplemental.slice(0,20),policy:'exact-browser-evidence-authority-with-verified-hi-screenshot-provenance'}});return{...e,evidence_refs:authoritative}}
    ...
    for(const e of effectiveResult.evidence){
      ...
      if(e.kind==='review-evidence'&&isHiReviewerRole(worker.role)){
        ...
        if(!scopeStateHash){appendLedger(m,'review.evidence-unbound',{task_id:task.id,worker_id:worker.id,payload:{reason:'reviewer-observation-requires-current-bounded-scope-state',scope:claimedScope.slice(0,40),task_scope:task.scope.slice(0,40)}});continue}
        if(!claimedScopeStateHash&&claimedScope.length)appendLedger(m,'review.evidence-scope-normalized',{task_id:task.id,worker_id:worker.id,payload:{claimed_scope:claimedScope.slice(0,40),canonical_scope:scope.slice(0,40),reason:'review claim scope is semantic/non-filesystem; repository freshness is bound to canonical task scope'}})
      ...
      if(browserProofKinds.has(e.kind)&&evidenceVerdictPassed(e.pass,e.outcome)){
        ...
        if(!valid){appendLedger(m,'browser.evidence-admission-rejected',{task_id:task.id,worker_id:worker.id,payload:{kind:e.kind,requested_refs:refs.slice(0,20),reason:requiredBrowserOrigins.length?'canonical-browser-target-observation-required':'canonical-browser-observation-required',required_origins:requiredBrowserOrigins}});continue}
    ...
    if(effectiveResult.findings?.length)materializeReviewFindingRework(m,task,worker,effectiveResult.findings)
    ...
    if(effectiveResult.status==='DONE'&&resourceOnlyProcessSupport&&!terminalProcessCapabilityBlocker){
      ...
      if(!retained.length||missingOrigins.length){
        const marker=`process-resource-not-retained:${task.id}`
        effectiveResult={...effectiveResult,status:'FIX_REQUIRED',summary:`Process-support completion requires an exact retained persistent RUNNING ProcessContract${missingOrigins.length?` with required service origin(s): ${missingOrigins.join(', ')}`:''}.`,open_issues:[...new Set([...effectiveResult.open_issues,marker])],needs_context:[...new Set([...effectiveResult.needs_context,'process-resource-retention: leave the exact ready persistent service RUNNING with cleanup_state=ACTIVE when returning DONE; do not kill/cleanup it before downstream verification. Parent control-plane custody performs final teardown after dependent verification/open obligations are satisfied.'])]}
        appendLedger(m,'process.resource-retention-rejected',{task_id:task.id,worker_id:worker.id,payload:{owned_processes:m.execution.processes.filter(process=>process.task_id===task.id&&process.worker_id===worker.id).map(process=>({process_id:process.process_id,status:process.status,cleanup_state:process.cleanup_state,timeout_at:process.timeout_at,service_origins:process.service_origins??[]})).slice(0,12),required_origins:requiredOrigins,missing_origins:missingOrigins,policy:'resource-only-process-support-must-transfer-live-custody-before-DONE'}})
      }else appendLedger(m,'process.resource-retention-admitted',{task_id:task.id,worker_id:worker.id,payload:{process_ids:retained.map(process=>process.process_id),service_origins:retainedOrigins,policy:'same-mission-parent-custody-until-dependent-verification-complete'}})
    }
    ...
    for(const signal of changedSurfaceMethodologySignals(replanFiles))activateMethodologySignal(m,this.projectRoot,{signal:signal.name,producer:'changed-surface',reason:signal.reason})
    ...
    if(effectiveResult.status==='DONE'){
      const now=Date.now();if(explorationClearance.admitted&&worker.role==='repository-explorer'&&m.identity.intent.ambiguity!=='none'){if((!m.identity.intent.likelyTargets?.length||task.scope.length===0)&&explorationClearance.source_scope.length){const priorTargets=[...(m.identity.intent.likelyTargets??[])];m.identity.intent.likelyTargets=[...explorationClearance.source_scope];appendLedger(m,'intent.targets.resolved',{task_id:task.id,worker_id:worker.id,payload:{source:'repository-explorer-clearance',prior_targets:priorTargets,targets:[...explorationClearance.source_scope],source_state_hash:explorationClearance.source_state_hash,authority:'runtime-bound-current-source-scope',reason:task.scope.length===0?'unbound-discovery-promoted-current-source':'previously-unresolved-target'}})}m.identity.intent.ambiguity='none';appendLedger(m,'intent.ambiguity.resolved',{task_id:task.id,worker_id:worker.id,payload:{source:'repository-explorer-clearance',source_state_hash:explorationClearance.source_state_hash,evidence_authority:false}})}
