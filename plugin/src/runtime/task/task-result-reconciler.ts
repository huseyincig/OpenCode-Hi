...
export class TaskResultReconciler{
  ...
  applyResult(m:MissionState,workerID:string,result:WorkerResult):void{
    ...
    if(worker.role==='visual-qa'&&requiredVerificationCases.length&&effectiveResult.status==='DONE'){
      const coverage=effectiveResult.verification_coverage??[],coverageIDs=coverage.map(c=>c.case_id),duplicateCoverageIDs=[...new Set(coverageIDs.filter((id,index)=>coverageIDs.indexOf(id)!==index))],byID=new Map(coverage.map(c=>[c.case_id,c])),actionForRef=(ref:string):string|undefined=>{const event=[...m.execution.ledger].reverse().find(event=>event.type==='browser.observation-recorded'&&event.task_id===task.id&&event.worker_id===worker.id&&event.payload?.evidence_ref===ref);return typeof event?.payload?.action==='string'?event.payload.action:undefined},failures:string[]=[]
      for(const required of requiredVerificationCases){const claim=byID.get(required.id);if(!claim){failures.push(`${required.id}:missing`);continue}if(claim.outcome!=='passed'){failures.push(`${required.id}:failed`);continue}const refs=[...new Set(claim.evidence_refs)],support=refs.map(ref=>m.execution.evidence.items.find(item=>item.id===ref)).filter((item):item is NonNullable<typeof item>=>Boolean(item)),actions=new Set(refs.map(actionForRef).filter((x):x is string=>Boolean(x))),missingActions=required.required_browser_actions.filter(action=>!actions.has(action)),validRefs=refs.length>0&&support.length===refs.length&&support.every(item=>item.trusted_source_class==='browser-observation'&&item.kind==='browser-evidence'&&!item.invalidated_at&&item.outcome!=='failed'&&item.pass!==false&&item.task_id===task.id&&evidenceClaimApplicability(m,item).applicable);if(!validRefs||missingActions.length)failures.push(`${required.id}:${!validRefs?'unbound-refs':`missing-actions=${missingActions.join('+')}`}`)}
      ...
      if(failures.length){const markers=failures.map(x=>`visual-coverage-unsatisfied:${task.id}:${x}`);effectiveResult={...effectiveResult,status:'FIX_REQUIRED',evidence:effectiveResult.evidence.map(e=>['browser-evidence','visual-evidence','accessibility-evidence'].includes(e.kind)&&evidenceVerdictPassed(e.pass,e.outcome)?{...e,pass:undefined,outcome:'pending' as const,reason:`visual-coverage-unsatisfied: ${failures.join('; ')}`}:e),open_issues:[...new Set([...effectiveResult.open_issues,...markers])],needs_context:[...new Set([...effectiveResult.needs_context,`visual-verification-coverage: satisfy every required case with current-attempt browser evidence refs and required actions; ${failures.join('; ')}`])]};appendLedger(m,'visual.coverage-rejected',{task_id:task.id,worker_id:worker.id,payload:{required_cases:requiredVerificationCases.map(c=>({id:c.id,actions:c.required_browser_actions})),reported_cases:coverage.map(c=>c.case_id),failures}})}else appendLedger(m,'visual.coverage-admitted',{task_id:task.id,worker_id:worker.id,payload:{cases:requiredVerificationCases.map(c=>c.id)}})
    }
