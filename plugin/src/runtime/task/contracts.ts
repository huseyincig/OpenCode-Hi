...
export function workerHandoffText(h:WorkerHandoff,maxChars:number=DEFAULT_CONTEXT_BUDGET.max_handoff_chars):string {
  ...
  const lines=[
    ...
    visualProof?'For visual-check / hi-visual-qa, emit evidence.kind="visual-evidence" with evidence.outcome="passed" only after the claimed browser-visible state actually passes; include the actual Hi browser evidence_ref values in evidence_refs. hi_browser_screenshot provides the pixels as a native image attachment; screenshot_artifact_ref=hi-artifact:... is opaque canonical provenance, not a filesystem path, so never read/glob/find it. Never manufacture PASS from a BrowserObservation or screenshot alone. Return the WorkerResult directly in assistant text; never write a temporary/result JSON file.':'',
