...
export function reviewFindingReworkObligationId(findingID:string):string{return`o-review-rework-${findingID}`}

function currentAttemptFindingEvidenceAdmitted(m:MissionState,task:MissionTask,worker:WorkerState,finding:ReviewFinding):boolean{
  if(!finding.evidence_refs.length)return false
 ...
 */
export function materializeReviewFindingRework(m:MissionState,task:MissionTask,worker:WorkerState,findings:ReviewFinding[]):string[]{
  ...
  for(const finding of findings){
    if(finding.reviewer_role!==worker.role||!reviewFindingNeedsCorrection(finding))continue
    ...
    const id=reviewFindingReworkObligationId(finding.id),requiredTargets=[...new Set(targets as string[])]
    ...
    if(existing){
      ...
      if(existing.status!=='open'){
        existing.status='open';existing.closedAt=undefined;existing.blocker=undefined
...
export function taskHasDelegatedReviewFindingRework(m:MissionState,task:MissionTask):boolean{
