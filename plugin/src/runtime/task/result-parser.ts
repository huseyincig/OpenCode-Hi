...
function jsonArrayValue(value:string):unknown[]{
  const raw=inlineValue(value)
  if(!raw||/^(?:none|null|n\/?a)$/i.test(raw))return[]
  try{const parsed=JSON.parse(raw);return Array.isArray(parsed)?parsed:[]}catch{return[]}
}
function stringListValue(value:string):string[]{return jsonArrayValue(value).map(String)}
function issueValue(value:string):string[]{
  const raw=inlineValue(value)
  if(!raw||/^(?:none|null|n\/?a|\[\])$/i.test(raw))return[]
  try{const parsed=JSON.parse(raw);if(Array.isArray(parsed))return parsed.map(String)}catch{}
  return[raw]
}
function markdownWorkerResult(text:string):WorkerResult|undefined{
  if(!/(?:^|\n)\s*WorkerResult\s*:/i.test(text))return undefined
  const fields=new Map<string,string>()
  for(const line of text.split(/\r?\n/)){
    const hit=line.match(/^\s*[-*]\s+(?:\*\*|__)?([a-z_]+)(?:\*\*|__)?\s*:\s*(.*)$/i)
    if(hit&&!fields.has(hit[1].toLowerCase()))fields.set(hit[1].toLowerCase(),hit[2].trim())
  }
...
export function parseWorkerResult(text:string):WorkerResult{
  ...
  if(status)return normalizeWorkerResult({status,summary:summary??bounded.slice(0,1000),open_issues:/USER_ACTION_REQUIRED/i.test(bounded)?['USER_ACTION_REQUIRED']:[]})
