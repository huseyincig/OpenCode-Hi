...
export class TaskRuntime{
  ...
  async start(m:MissionState,input:StartTaskInput={}):Promise<{task_id:string;worker_id:string;session_id?:string;model?:string;methodologies:string[];selection_reason:string[];readiness:'READY'|'WAIT';preconditions:Array<{id:string;decision:string;reason:string}>}>{
    ...
    const requestedArtifactIds=[...new Set(input.contextArtifactIds??[])].slice(0,DEFAULT_CONTEXT_BUDGET.max_artifacts),unknownArtifactIds=requestedArtifactIds.filter(id=>!m.context_artifacts.some(a=>a.id===id));if(unknownArtifactIds.length)throw new Error(`Unknown context artifact id(s): ${unknownArtifactIds.join(', ')}`);const selectedContextArtifacts=requestedArtifactIds.map(id=>m.context_artifacts.find(a=>a.id===id)!).filter(Boolean);const approvalGated=skillPlan.selected.filter(s=>s.permission==='ask').map(s=>s.name),taskTools=surface.tools.filter(t=>t!=='skill'||methodologies.length>0)
