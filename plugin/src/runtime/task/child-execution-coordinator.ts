...
export class ChildExecutionCoordinator{
  ...
  async sendProviderPrompt(sessionID:string,text:string,role?:string,model?:string,variant?:string,tools?:Record<string,boolean>,messageID?:string,transport:'structured'|'text'='structured'):Promise<unknown>{const safe=redactProviderContext(text),format=transport==='structured'&&this.host.capabilities.structuredOutput===true?workerResultOutputFormat():undefined;return this.host.prompt(sessionID,safe.providerText,role,model,variant,tools,messageID,format)}
