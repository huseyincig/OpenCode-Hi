...
export class ChildExecutionCoordinator{
  ...
  async sendProviderPrompt(sessionID:string,text:string,role?:string,model?:string,variant?:string,tools?:Record<string,boolean>,messageID?:string):Promise<unknown>{const safe=redactProviderContext(text);return this.host.prompt(sessionID,safe.providerText,role,model,variant,tools,messageID)}
  async status(sessionID:string){return this.host.status(sessionID)}
