...
import { evidenceVerdictPassed,resolvedEvidenceOutcome } from '../../contracts/evidence-kinds.js'
...
function minimumVerificationKinds(intent:NormalizedMissionIntent):string[]{
  const declared=[...new Set(intent.likelyVerification.map(canonical))]
  if(declared.length)return declared
  if(intent.taskKind==='diagnosis')return[]
  return[intent.taskKind==='review'?'review-evidence':'changed-surface-sanity']
}
export function verificationPolicyFor(intent:NormalizedMissionIntent):VerificationPolicy{const independentReview=intent.risk==='high'||intent.requiredCapabilities.includes('independent-review')||intent.requiredCapabilities.includes('security-review');return{requiredKinds:minimumVerificationKinds(intent),requireFresh:true,requireReview:independentReview,allowWorkerReportedEvidence:false}}
function effectiveVerificationKinds(m:MissionState,obligation?:MissionState['execution']['obligations'][number]):string[]{
  const obligationKinds=[...new Set((obligation?.requiredEvidence??[]).map(canonical))]
  if(obligationKinds.length)return obligationKinds
  const policyKinds=[...new Set(m.execution.verification_policy.requiredKinds.map(canonical))]
  if(policyKinds.length)return policyKinds
  if(obligation?.kind==='verification'||m.execution.obligations.some(o=>o.kind==='verification'))return minimumVerificationKinds(m.identity.intent)
  return[]
}


function normPath(p:string):string{return p.trim().replace(/\\/g,'/').replace(/^\.\//,'')}
function dependencySurface(files:string[]):boolean{return files.some(raw=>/(^|\/)(package\.json|package-lock\.json|pnpm-lock\.yaml|yarn\.lock|bun\.lockb?|requirements(?:-[^/]*)?\.txt|pyproject\.toml|poetry\.lock|cargo\.toml|cargo\.lock|go\.mod|go\.sum)(?:$|\/)/i.test(normPath(raw)))}
function sensitiveSurface(files:string[]):boolean{return files.some(raw=>{const p=normPath(raw).toLowerCase();return /(^|\/)(auth|security|permission|oauth|session|credential|secrets?)(\/|\.|$)/.test(p)||/(migration|schema|database|dockerfile|compose\.ya?ml|package\.json|lock\.ya?ml|package-lock\.json|pnpm-lock\.yaml|yarn\.lock|\.github\/workflows)/.test(p)})}
function scopeExpanded(task:MissionTask,files:string[]):boolean{const expected=task.scope.map(normPath).filter(Boolean);if(!files.length)return false;if(!expected.length)return files.length>1;return files.map(normPath).some(f=>!expected.some(e=>f===e||f.startsWith(e.endsWith('/')?e:e+'/')||e.startsWith(f.endsWith('/')?f:f+'/')))}
export interface VerificationReplan {changed:boolean;addedKinds:string[];scopeExpanded:boolean;riskEscalated:boolean;reason:string}
export function replanVerificationForChangedSurface(m:MissionState,task:MissionTask,files:string[],repo?:RepoContext):VerificationReplan{
  const actual=[...new Set(files.map(normPath).filter(Boolean))],expanded=scopeExpanded(task,actual),dependencyChanged=dependencySurface(actual),sensitive=sensitiveSurface(actual),added:string[]=[]
  if(!actual.length)return{changed:false,addedKinds:[],scopeExpanded:false,riskEscalated:false,reason:'no-changed-files'}
  ...
  if(sensitive||m.identity.risk==='high'){if(staticKind)added.push(canonical(staticKind));if(buildKind)added.push(canonical(buildKind))}
  else if(expanded&&actual.length>1&&staticKind)added.push(canonical(staticKind))
  const before=new Set(m.execution.verification_policy.requiredKinds.map(canonical));for(const k of added)before.add(k);const next=[...before],addedKinds=next.filter(k=>!m.execution.verification_policy.requiredKinds.map(canonical).includes(k))
  const riskEscalated=sensitive&&m.identity.risk!=='high'&&m.identity.risk!=='authority-boundary';if(riskEscalated){m.identity.risk='high';m.identity.intent.risk='high';m.execution.verification_policy.requireReview=true}
  if(dependencyChanged){m.identity.intent.requiredCapabilities=[...new Set([...m.identity.intent.requiredCapabilities,'dependency-change','security-review'])];m.execution.verification_policy.requireReview=true;if(!m.execution.obligations.some(o=>o.kind==='review'&&o.status==='open'&&o.summary.includes('Dependency graph changed')))m.execution.obligations.push({id:`o-dependency-review-${Date.now().toString(36)}`,kind:'review',summary:'Dependency graph changed; independent supply-chain/security review required',status:'open',requiredEvidence:['review-evidence']})}
  else if(riskEscalated&&!m.execution.obligations.some(o=>o.kind==='review'&&o.status==='open'))m.execution.obligations.push({id:`o-high-assurance-${Date.now().toString(36)}`,kind:'review',summary:'Changed surface entered a security/configuration-sensitive area; independent review required',status:'open',requiredEvidence:['review-evidence']})
  const changed=addedKinds.length>0||expanded||riskEscalated
  if(changed){m.execution.verification_policy.requiredKinds=next;for(const o of m.execution.obligations.filter(o=>o.kind==='verification'&&o.status==='open'))o.requiredEvidence=[...new Set([...(o.requiredEvidence??[]).map(canonical),...next])] }
...
export function verificationEconomyInstruction(m:MissionState,ownedKinds?:string[]):string{
  const kinds=[...new Set((ownedKinds===undefined?m.execution.verification_policy.requiredKinds:ownedKinds).map(canonical))]
  if(ownedKinds!==undefined&&!kinds.length)return 'Task evidence contract: none. Do not claim, reopen, or block on mission verification owned by other tasks.'
  const required=kinds.join(', ')||'changed-surface-sanity'
  if(m.identity.intent.taskKind==='release-readiness'||m.identity.risk==='high')return `Verification contract: ${required}. Use repo-native commands and satisfy these required kinds; prefer targeted tests first, then the required static/build checks. Do not substitute a cheap unrelated PASS for a missing required kind.`
...
export function verificationKindAdmittedForMission(m:MissionState,actual:string):boolean{const required=effectiveVerificationKinds(m);return required.length===0||required.some(kind=>verificationKindSatisfiesRequirement(kind,actual))}
...
function verificationResult(e:EvidenceItem):VerificationCheckResult{
  const outcome=resolvedEvidenceOutcome(e.pass,e.outcome)
  if(outcome==='passed')return'passed'
  if(outcome==='failed')return'failed'
  if(outcome==='environment-issue')return'environment-issue'
  return'pending'
}
export function verificationEnvelopeFor(m:MissionState,obligationID?:string,projectRoot?:string):VerificationEnvelope{
  ...
  const requiredKinds=effectiveVerificationKinds(m,obligation)
  ...
  const requiredEvidenceFresh=checks.length>0&&checks.every(check=>{
  ...
  if(freshness==='stale')limitations.push('fresh-evidence-required')
  if(p.requireReview&&!independentReview)limitations.push('independent-review-required')
...
export function verificationSatisfied(m:MissionState,obligationID?:string,projectRoot?:string):{ok:boolean;missing:string[]}{
  ...
  if(!envelope.independent_review)missing.push('review-obligation')
...
export function reviewObligationSatisfied(m:MissionState,obligationID:string,projectRoot?:string):{ok:boolean;reason?:string;evidence_id?:string}{
  ...
  const evidence=[...m.execution.evidence.items].filter(e=>canonical(e.kind)==='review-evidence'&&!e.invalidated_at&&evidenceVerdictPassed(e.pass,e.outcome)&&(!m.execution.verification_policy.requireReview||e.trusted_source_class==='reviewer-observation')&&evidenceAllowedForVerification(m,e,obligationID,projectRoot)).sort((a,b)=>b.observed_at-a.observed_at)
...
export function latestBlockingVerificationEvidence(m:MissionState,obligationID?:string):EvidenceItem|undefined{const obligation=obligationID?m.execution.obligations.find(o=>o.id===obligationID):undefined,requiredKinds=effectiveVerificationKinds(m,obligation),current=[...m.execution.evidence.items].filter(e=>!e.invalidated_at&&evidenceAllowedForVerification(m,e,obligationID)).sort((a,b)=>b.observed_at-a.observed_at);for(const e of current){if(e.outcome!=='environment-issue'&&e.outcome!=='failed')continue;const matched=requiredKinds.filter(r=>verificationKindSatisfiesRequirement(r,e.kind));if(!matched.length)continue;const superseded=matched.every(r=>current.some(candidate=>candidate.observed_at>e.observed_at&&evidenceVerdictPassed(candidate.pass,candidate.outcome)&&verificationKindSatisfiesRequirement(r,candidate.kind)));if(!superseded)return e}return undefined}
