...
export function decideTopology(intent:NormalizedMissionIntent,config:TopologyPolicyConfig=DEFAULT_TOPOLOGY_POLICY,m?:MissionState):TopologyDecision{
  if(m?.execution_mode==='team')return{mode:'multi-agent',executionMode:'team',agentCount:Math.max(2,m.topology?.parallelism??2),parallelism:Math.max(1,m.topology?.parallelism??2),reason:['existing bounded Team Mode remains authoritative']}
