...
export function decideTopology(intent:NormalizedMissionIntent,config:TopologyPolicyConfig=DEFAULT_TOPOLOGY_POLICY,m?:MissionState):TopologyDecision{
  if(m?.execution_mode==='team')return{mode:'multi-agent',executionMode:'team',agentCount:Math.max(2,m.topology?.agentCount??2),parallelism:Math.max(1,m.topology?.parallelism??2),roleReuse:false,reason:['existing bounded Team Mode remains authoritative']}
  ...
  if(config.mode==='multi-agent')return{mode:'multi-agent',executionMode:'parallel',agentCount:Math.max(2,Math.min(config.maxAgents,2)),parallelism:Math.max(1,Math.min(config.parallelism,2)),roleReuse:false,reason:['explicit user/project multi-agent override']}
  const base=resolveExecutionMode(intent)
  const benefit=base.mode==='parallel'
  if(!benefit)return{mode:'single-agent',executionMode:'single',agentCount:1,parallelism:1,roleReuse:config.allowMultiRoleAgent,reason:['adaptive policy found no material fan-out benefit',...base.reason]}
