...
export function syncMissionGates(m:MissionState):MissionGate[]{
  ...
  const reviewOpen=m.obligations.some(o=>o.kind==='review'&&o.status!=='closed');upsert(m,'gate-reviewer','reviewer','Required independent review must be completed',reviewOpen?'open':'closed',reviewOpen?'review-obligation-open':undefined)
