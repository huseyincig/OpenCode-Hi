...
export function syncMissionGates(m:MissionState):MissionGate[]{
  ...
  const verifyOpen=m.execution.obligations.some(o=>o.kind==='verification'&&o.status!=='closed');const verify=verificationSatisfied(m)
  ...
  const reviewOpen=m.execution.obligations.some(o=>o.kind==='review'&&o.status!=='closed'),independentReviewOpen=m.execution.verification_policy.requireReview&&reviewOpen;upsert(m,'gate-reviewer','reviewer','Required independent review must be completed',independentReviewOpen?'open':'closed',independentReviewOpen?'review-obligation-open':undefined)
