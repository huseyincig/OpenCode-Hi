...
export function syncMissionGates(m:MissionState):MissionGate[]{
  ...
  const reviewObligations=m.execution.obligations.filter(o=>o.kind==='review'),hasReview=reviewObligations.length>0||m.execution.verification_policy.requireReview,reviewOpen=reviewObligations.some(o=>o.status!=='closed'),review=reviewClaimsSatisfied(m);upsert(m,'gate-reviewer','reviewer','Required independent review must be completed',hasReview?(review.ok?(reviewOpen?'ready':'closed'):'open'):'closed',hasReview&&!review.ok?`review-claims:${review.missing.join(',')}`:undefined)
