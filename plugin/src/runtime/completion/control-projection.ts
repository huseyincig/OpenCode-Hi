...
export interface ControlDecisionProjection{
  ...
  wait_for:string[]
  missing_evidence:Array<{obligation_id:string;kind:string;result:string}>
  open_obligations:Array<{id:string;kind:string}>
  ineffective_actions:string[]
  verification_route_status:'available'|'none'|'unknown'
  verification_routes:Array<{required_kind:string;evidence_kind:string;command:string;source:string}>
}

function persistentServiceTarget(m:MissionState):{processes:MissionState['execution']['processes'];origins:string[]}{const processes=m.execution.processes.filter(isPersistentRunningProcess),origins=[...new Set(processes.flatMap(process=>process.service_origins??[]))];return{processes,origins}}
function activeWaits(m:MissionState):string[]{
...
function missingVerification(m:MissionState,projectRoot?:string):ControlDecisionProjection['missing_evidence']{
  const out:ControlDecisionProjection['missing_evidence']=[]
  for(const obligation of m.execution.obligations.filter(o=>o.kind==='verification')){
    ...
    for(const check of envelope.checks){
      if(check.result!=='passed'){out.push({obligation_id:obligation.id,kind:check.kind,result:check.result});continue}
      if(m.execution.verification_policy.requireFresh&&check.evidence_refs.length&&!check.evidence_refs.some(ref=>{const evidence=m.execution.evidence.items.find(item=>item.id===ref);return Boolean(evidence&&!evidence.invalidated_at)}))out.push({obligation_id:obligation.id,kind:check.kind,result:'stale'})
    }
  }
...
function verificationRoutes(m:MissionState,projectRoot:string|undefined,missing:ControlDecisionProjection['missing_evidence']):Pick<ControlDecisionProjection,'verification_route_status'|'verification_routes'> {
  if(!projectRoot)return{verification_route_status:'unknown',verification_routes:[]}
  const targets=m.vcs.changed_files.length?m.vcs.changed_files:(m.identity.intent.likelyTargets??[])
  const discovered=discoverVerificationRoutes(projectRoot,targets),verification_routes:ControlDecisionProjection['verification_routes']=[]
  for(const required of [...new Set(missing.map(x=>x.kind))])for(const route of discovered)if(verificationKindSatisfiesRequirement(required,route.evidenceKind))verification_routes.push({required_kind:required,evidence_kind:route.evidenceKind,command:route.command,source:route.source})
...
const EMPTY_ROUTES={verification_route_status:'unknown' as const,verification_routes:[] as ControlDecisionProjection['verification_routes']}
 ...
 */
export function projectControlDecision(m:MissionState,projectRoot?:string):ControlDecisionProjection{
  ...
  if(completion.complete)return{action:'DONE',completion_ready:true,wait_for:[],missing_evidence:[],open_obligations:[],ineffective_actions:[],...EMPTY_ROUTES}
  if(completion.next==='USER_ACTION_REQUIRED')return{action:'USER_ACTION_REQUIRED',completion_ready:false,wait_for:[],missing_evidence:missingVerification(m,projectRoot),open_obligations,ineffective_actions:[],...EMPTY_ROUTES}
  if(wait_for.length)return{action:'WAIT',completion_ready:false,wait_for,missing_evidence:[],open_obligations,ineffective_actions:[],...EMPTY_ROUTES}
  if(m.execution.execution_mode==='parallel'&&m.execution.tasks.length===0&&open_obligations.some(o=>o.kind!=='verification'))return{action:'CONTINUE',completion_ready:false,wait_for:[],missing_evidence:[],open_obligations,ineffective_actions:[],...EMPTY_ROUTES}
  ...
  if(completion.next==='VERIFY'){
    ...
    if(routes.verification_route_status==='none'){ineffective.push('read','hi_readiness','unclassified-bash');if(!missing.some(item=>item.kind==='visual-check'||item.kind==='visual-evidence'))ineffective.push('redundant-verifier-child')}
  ...
  if(completion.next==='RECONCILE')return{action:'RECONCILE',completion_ready:false,wait_for:[],missing_evidence:missing,open_obligations,ineffective_actions:[],...EMPTY_ROUTES}
...
export function controlDecisionInstruction(m:MissionState,decision:ControlDecisionProjection):string{
  if(decision.action==='DONE')return'stop:mission-complete; emit final answer only now'
  if(decision.action==='USER_ACTION_REQUIRED')return'user-action:canonical-gate; explain the exact required user/runtime action without claiming completion'
  if(decision.action==='WAIT')return`wait:${decision.wait_for[0]??'active-runtime'}; use the owning Hi await/status surface, do not create duplicate work`
  if(decision.action==='RECONCILE')return'reconcile:canonical-state; reconcile the existing result/session before any new verification or replacement worker'
  if(decision.action==='VERIFY'){
    ...
    if(visual&&!visualWorker&&m.identity.intent.requiredCapabilities.includes('visual-qa')){const verificationIDs=decision.open_obligations.filter(o=>o.kind==='verification').map(o=>o.id),live=persistentServiceTarget(m);if(live.processes.length&&!live.origins.length)return`verify:${missing.join(',')||'visual-check'}; live-service-target-unregistered:${live.processes.map(process=>process.process_id).join(',')}; call hi_process_read once for the exact retained live process to reconcile its observed loopback HTTP(S) origin, then retry visual task admission; static preview is forbidden while the live service is the verification surface`;if(live.origins.length>1)return`verify:${missing.join(',')||'visual-check'}; multiple-live-service-origins=${live.origins.join(',')}; call hi_task_start with one exact browser_required_origins value matching the intended live process; do not use static preview`;const requiredOrigin=live.origins[0],targetClause=requiredOrigin?`, browser_required_origins=${requiredOrigin}`:'';return`verify:${missing.join(',')||'visual-check'}; call hi_task_start with role=visual-qa, category=visual, required_evidence=${missing.join(',')||'visual-check'}${verificationIDs.length?`, obligation_ids=${verificationIDs.join(',')}`:''}${targetClause}; the visual worker must use Hi browser tools${requiredOrigin?` against ${requiredOrigin}; hi_browser_preview_open cannot substitute for this live origin`:' (hi_browser_preview_open is allowed only when no live service target exists)'}; then await/reconcile that same task; do not substitute unclassified bash or prose claims`}
    if(decision.verification_route_status==='available')return`verify:${missing.join(',')||'required-evidence'}; route=${decision.verification_routes.map(x=>x.command).join(' || ')}; evidence-owned; run only projected route(s); do not broaden without changed-surface evidence`
  ...
  if(openWork){
    ...
    if(openWork.kind==='implementation'&&primaryRoleCanDirectImplementation(m.execution.primary_mode)&&['DIRECT','EVIDENCE'].includes(m.execution.adaptive_execution?.path??''))return`continue:${openWork.id}; complete minimum owned mutation; then hi_direct_progress obligation_id=${openWork.id}; no terminal answer before RECORDED`
    if(openWork.kind==='analysis')return`continue:${openWork.id}; gather needed evidence; then hi_direct_progress obligation_id=${openWork.id}; do not verify first`
