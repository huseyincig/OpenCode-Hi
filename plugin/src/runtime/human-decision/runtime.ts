...
import { humanDecisionId,type HumanDecisionContract,type HumanDecisionResponseSchema,type HumanDecisionScope,type HumanDecisionType } from '../../contracts/human-decision.js'
...
export function openHumanDecision(m:MissionState,input:OpenHumanDecisionInput):HumanDecisionContract{
  const scope:HumanDecisionScope={mission_id:m.mission_id,task_id:input.task_id,worker_id:input.worker_id}
  ...
  const previous=m.human_decision,createdAt=previous?.status==='OPEN'&&previous.decision_id===decisionId?previous.created_at:Date.now()
  ...
  m.human_decision=decision;m.status='waiting-user'
