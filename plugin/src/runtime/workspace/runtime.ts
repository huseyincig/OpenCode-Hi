...
export class WorkspaceRuntime{
  ...
  async provision(m:MissionState,task:MissionTask,decision:IsolationDecisionContract):Promise<WorkspaceLeaseContract>{
    ...
    if(!canonical||canonical.requested_by!==`task:${task.id}`||JSON.stringify(canonical.scope)!==JSON.stringify(task.scope))throw new Error('Hi WorkspaceRuntime provision requires the canonical Mission-owned isolation decision for this exact Task scope')
    ...
    if(identityConflict){const candidate:WorkspaceLeaseContract={lease_id:leaseID(),mission_id:m.identity.mission_id,task_id:task.id,repository_root:this.projectRoot,base_ref:sourceBaseline,workspace_path:native.workspace_path,host_workspace_id:native.host_workspace_id,...(native.branch?{branch:native.branch}:{}),created_at:Date.now(),status:'RECONCILING',cleanup_state:'CLEANUP_PENDING',source_baseline:sourceBaseline};try{await this.executor.cleanup(candidate)}catch{};throw new Error(`Workspace identity is already owned by another active lease: ${identityConflict.lease_id}`)}
