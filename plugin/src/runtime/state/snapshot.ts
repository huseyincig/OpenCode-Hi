...
export function compactMissionContext(m:MissionState,worker?:WorkerState):string{
  ...
  const protectedEntries:ContextEntry[]=essential.map((text,i)=>({id:`essential-${i}`,kind:'mission-survival',text,contextClass:'PROTECTED',createdAt:m.identity.updated_at}))
