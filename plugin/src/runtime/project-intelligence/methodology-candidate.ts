...
export interface MethodologyCandidateObservation{mission_id:string;task_id:string;worker_id:string;evidence:string[];observed_at:number;outcome?:MethodologyLearningOutcome}
export interface MethodologyCandidateLearning{alpha:number;beta:number;positive:number;negative:number;last_positive_at:number;last_negative_at?:number}
export interface ProjectMethodologyCandidate{schema:1;id:string;key:string;contract_sha256:string;procedure:string;trigger:string;do_not_trigger:string;exit_condition:string;state:'CANDIDATE'|'READY'|'ARCHIVED';observations:MethodologyCandidateObservation[];learning?:MethodologyCandidateLearning;created_at:number;updated_at:number}
export interface MethodologyCandidateAssessment{eligible:boolean;reason:'archived'|'insufficient-independent-evidence'|'confidence-below-floor'|'admitted';positive:number;negative:number;independent_tasks:number;posterior_confidence:number;effective_confidence:number;age_days:number;half_life_days:number;freshness:'FRESH'|'DECAYED'}
...
function validObservation(value:unknown):value is MethodologyCandidateObservation{if(!value||typeof value!=='object'||Array.isArray(value))return false;const v=value as Record<string,unknown>;return nonempty(v.mission_id)&&nonempty(v.task_id)&&nonempty(v.worker_id)&&Array.isArray(v.evidence)&&v.evidence.length>0&&v.evidence.every(nonempty)&&finitePositive(v.observed_at)&&(v.outcome===undefined||v.outcome==='helpful'||v.outcome==='harmful')}
function derivedLearning(observations:readonly MethodologyCandidateObservation[]):MethodologyCandidateLearning{
...
function validLearning(value:unknown,observations:readonly MethodologyCandidateObservation[]):value is MethodologyCandidateLearning{
  if(!value||typeof value!=='object'||Array.isArray(value))return false;const v=value as Record<string,unknown>,expected=derivedLearning(observations)
...
export function methodologyCandidateLearning(candidate:ProjectMethodologyCandidate):MethodologyCandidateLearning{return candidate.learning?{...candidate.learning}:derivedLearning(candidate.observations)}
export function methodologyCandidateAssessment(candidate:ProjectMethodologyCandidate,now=Date.now()):MethodologyCandidateAssessment{
...
export function withDerivedMethodologyLearning(candidate:ProjectMethodologyCandidate):ProjectMethodologyCandidate{return{...candidate,observations:candidate.observations.map(o=>({...o,evidence:[...o.evidence]})),learning:derivedLearning(candidate.observations)}}
export function validProjectMethodologyCandidate(raw:unknown):raw is ProjectMethodologyCandidate{
  ...
  if(!Array.isArray(v.observations)||!v.observations.every(validObservation)||!finitePositive(v.created_at)||!finitePositive(v.updated_at)||v.updated_at<v.created_at)return false
  const observations=v.observations as MethodologyCandidateObservation[]
  if(v.learning!==undefined&&!validLearning(v.learning,observations))return false
...
export function readProjectMethodologyCandidate(projectRoot:string,id:string):ProjectMethodologyCandidate|undefined{
  ...
  try{const raw=JSON.parse(readFileSync(path,'utf8'));return validProjectMethodologyCandidate(raw)?withDerivedMethodologyLearning(raw):undefined}catch{return undefined}
}
