...
import { methodologyCandidateDigest, methodologyCandidateID, validProjectMethodologyCandidate, type ProjectMethodologyCandidate } from './methodology-candidate.js'
...
export class ProjectMethodologyLearningStore{
  ...
  #persist(item:ProjectMethodologyCandidate):void{const path=projectMethodologyCandidatePath(this.projectRoot,item.id);mkdirSync(dirname(path),{recursive:true});writeFileSync(path,JSON.stringify(item,null,2)+'\n','utf8')}
  all():ProjectMethodologyCandidate[]{return [...this.#items.values()].map(x=>({...x,observations:x.observations.map(o=>({...o,evidence:[...o.evidence]}))}))}
  observe(mission:MissionState,worker:WorkerState,observation:MethodologyObservation,resultEvidence:readonly string[]):ProjectMethodologyCandidate|undefined{
    ...
    const item:ProjectMethodologyCandidate=existing??{schema:1,id,key:observation.key,contract_sha256:contractSha,procedure:observation.procedure,trigger:observation.trigger,do_not_trigger:observation.do_not_trigger,exit_condition:observation.exit_condition,state:'CANDIDATE',observations:[],created_at:now,updated_at:now}
    if(item.state==='ARCHIVED')return item
    const taskKey=`${mission.identity.mission_id}:${worker.task_id}`
    if(!item.observations.some(o=>`${o.mission_id}:${o.task_id}`===taskKey))item.observations.push({mission_id:mission.identity.mission_id,task_id:worker.task_id,worker_id:worker.id,evidence:referenced,observed_at:now})
    const independentTasks=new Set(item.observations.map(o=>`${o.mission_id}:${o.task_id}`)).size
    if(independentTasks>=2)item.state='READY'
    item.updated_at=now;this.#items.set(id,item);this.#persist(item)
