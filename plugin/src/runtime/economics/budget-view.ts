...
export interface ExecutionBudgetView{mission:BudgetAxis[];workers:Record<string,BudgetAxis[]>;processes:Record<string,BudgetAxis[]>}
function axis(input:Omit<BudgetAxis,'status'>):BudgetAxis{return{...input,status:input.measurement==='unavailable'||input.enforcement==='unavailable'?'unavailable':input.enforcement==='observed-only'?'observed':input.limit!==undefined&&input.used>=input.limit?'exhausted':'within'}}
function activeWorkers(m:MissionState):number{return m.execution.workers.filter(w=>['created','queued','starting','busy'].includes(w.status)).length}
function currentRecoveryAttempts(m:MissionState):number{const sig=m.continuation.semantic_progress_snapshot?.state_hash??m.continuation.last_progress_signature;return(m.continuation.recovery_history??[]).filter(x=>x.progress_signature===sig&&x.generation===m.continuation.generation&&x.outcome!=='failed').length}
function workerAxes(m:MissionState,w:WorkerState):BudgetAxis[]{
  ...
  if(profile){out.push(axis({axis:'context-cap',used:0,limit:profile.max_context_chars,unit:'characters',measurement:'unavailable',enforcement:'hard',source:'execution-profile-max-context;consumption-not-observed'}),axis({axis:'handoff-cap',used:Math.max(0,...m.execution.ledger.filter(e=>e.worker_id===w.id&&e.type==='worker.handoff').map(e=>Number(e.payload?.chars??0)).filter(Number.isFinite)),limit:profile.max_handoff_chars,unit:'characters',measurement:'exact',enforcement:'hard',source:'execution-profile-max-handoff'}),axis({axis:'result-cap',used:0,limit:profile.max_result_chars,unit:'characters',measurement:'unavailable',enforcement:'hard',source:'result-parser-clipping;pre-clip-consumption-not-observed'}),axis({axis:'artifact-cap',used:task?.context_artifacts.length??0,limit:profile.max_artifacts,unit:'artifacts',measurement:'exact',enforcement:'hard',source:'execution-profile-max-artifacts'}))}
  if(providerBilled>0)out.push(axis({axis:'provider-billed-cost',used:providerBilled,unit:'USD',measurement:'exact',enforcement:'observed-only',source:'provider-billed'}))
  if(derivedCost>0)out.push(axis({axis:'opencode-derived-cost',used:derivedCost,unit:'USD',measurement:'derived',enforcement:'observed-only',source:'opencode-calculated'}))
  return out
}
export function executionBudgetView(m:MissionState,now=Date.now()):ExecutionBudgetView{
  const mission=[
