...
function axis(input:Omit<BudgetAxis,'status'>):BudgetAxis{return{...input,status:input.enforcement==='unavailable'?'unavailable':input.enforcement==='observed-only'?'observed':input.limit!==undefined&&input.used>=input.limit?'exhausted':'within'}}
