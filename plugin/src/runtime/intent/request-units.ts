...
export function assertVerificationRequestTrace(text:string,assessment:SemanticIntentAssessment):void{
  ...
  const visualRefs=new Set<string>(),visualCapabilityRefs=new Set(assessment.capability_request_units?.['visual-qa']??[]),enforceVisualCapabilityTrace=assessment.scope==='multi-stream'&&assessment.message_kind==='mission'
  if(enforceVisualCapabilityTrace&&!visualCapabilityRefs.size)throw new Error(`multi-stream visual-check requires capability_request_units.visual-qa ownership for verification case source units; ${challenge()}`)
