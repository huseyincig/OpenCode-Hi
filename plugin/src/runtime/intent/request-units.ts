...
export function assertVerificationRequestTrace(text:string,assessment:SemanticIntentAssessment):void{
  ...
  for(const c of assessment.verification_cases){const refs=c.source_units??[];if(!refs.length)throw new Error(`verification case ${c.id} requires source_units; ${challenge()}`);for(const id of refs){if(!unitMap.has(id))throw new Error(`verification case ${c.id} source_units contains unknown id ${id}; ${challenge()}`);visualRefs.add(id)}}
