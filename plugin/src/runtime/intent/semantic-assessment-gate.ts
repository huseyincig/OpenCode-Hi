...
export function renderSemanticAssessmentGate(m:MissionState):string{
  ...
  return[
    ...
    ...atomLines,
    `C=${SEMANTIC_CAPABILITIES.join('|')};X=${SEMANTIC_EXTERNAL_ACTIONS.join('|')};V=${SEMANTIC_VERIFICATION_KINDS.join('|')}. visual-check=>verification_cases{id,subject,required_browser_actions:A[],source_units:RU[]};all RU=case source_units|nonvisual_request_units;else both=[];no RU trace;caseRU in C:visual-qa;${semantic.phase==='followup'?'resume/constraint empty cases+RU keeps prior; ':''}A=open|navigate|click|type|key|inspect|viewport|screenshot;reload=navigate+inspect;intent_signals=[] by default;intent.<slug>:intent.tdd;unknown signals reject;capability-named signals reject.`,
