...
export function renderSemanticAssessmentGate(m:MissionState):string{
  ...
  return[
    ...
    `rev=${semantic.revision};phase=${semantic.phase};call hi_intent_assess once;user language`,
    `JSON(all keys required;[]=empty):material;message_kind=${messageKinds};task_kind=implementation|bug-fix|diagnosis|review|performance|release-readiness;scope=local|multi-file|repo-wide|external|multi-stream;risk=low|medium|high|authority-boundary;ambiguity=none|resolvable|contract-critical;dependency_class=independent|sequential|external-gated|unknown|independent-multi;required_capabilities=C[];requested_external_actions=X[];likely_verification=V[];likely_targets[];nonvisual_request_units[];intent_signals[]${semantic.phase==='followup'?';constraint_atoms[]':''}`,
    ...atomLines,
    `C=${SEMANTIC_CAPABILITIES.join('|')};X=${SEMANTIC_EXTERNAL_ACTIONS.join('|')};V=${SEMANTIC_VERIFICATION_KINDS.join('|')}. visual-check=>verification_cases=1..16 {id:vc_<slug>,subject,required_browser_actions:A[],source_units:RU[]};nonvisual_request_units=RU[];all RU classified;RU=ordered non-heading user clauses;${semantic.phase==='followup'?'resume/constraint empty cases+RU keeps prior; ':''}A=open|navigate|click|type|key|inspect|viewport|screenshot|wait|close;reload=navigate+inspect;intent_signals=[] by default;intent.<slug>:intent.tdd;unknown signals reject;capability-named signals reject.`,
