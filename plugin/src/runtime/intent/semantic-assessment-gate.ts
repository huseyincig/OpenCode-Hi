...
export function renderSemanticAssessmentGate(m:MissionState):string{
  ...
  return[
    ...
    `C=${SEMANTIC_CAPABILITIES.join('|')}; X=${SEMANTIC_EXTERNAL_ACTIONS.join('|')}; V=${SEMANTIC_VERIFICATION_KINDS.join('|')}. intent_signals=[] by default; intent.<slug>, e.g. intent.tdd; unknown signals reject.`,
    ...
    'intent.tdd=test-first/test-authoring only; failing test/test command=verification. Select intent.debugging only when root-cause diagnosis is materially required; if diagnosis is required include repository-analysis in required_capabilities. independent-review only for explicit user independence or risk/policy requirement. interactive-process=persistent/interactive only; bounded commands=native shell. likely_targets=paths/URLs. X nonempty=>R=authority-boundary.',
    phaseRule,
  ].join('\n')
}
