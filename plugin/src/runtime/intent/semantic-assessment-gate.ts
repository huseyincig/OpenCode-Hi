...
export function renderSemanticAssessmentGate(m:MissionState):string{
  ...
  const phaseRule=semantic.phase==='initial'?'file/repo/tool=>mission=true;chat-only=>non-material=false':'Follow-up: preserve prior semantics unless changed. resume=continue the existing unfinished contract without adding/changing requested outcomes; verification=check existing work only; amendment=add/change an implementation outcome and therefore requires C to include implementation for implementation/bug-fix/performance. Continuation/reconnect/handoff wording alone is resume, not amendment. non-material=>false; others=>true.'
  return[
    'Hi SEMANTIC ASSESSMENT GATE',
    `call hi_intent_assess once;user language`,
    `all keys required:material=true|false;message_kind=${messageKinds};task_kind=implementation|bug-fix|diagnosis|review|performance|release-readiness;scope:local|multi-file|repo-wide|external|multi-stream;risk=low|medium|high|authority-boundary;ambiguity:none|resolvable|contract-critical;dependency_class:independent|sequential|external-gated|unknown|independent-multi;continuation_required=bool(same-Mission-named-later-turn=>true;auto-resume/reconnect=>false);required_capabilities=C;requested_external_actions=X[];likely_verification=V[];verification_cases[];nonvisual_request_units[];capability_request_units={}${semantic.phase==='followup'?';constraint_atoms[]':''}`,
    ...atomLines,
    `C=${SEMANTIC_CAPABILITIES.join('|')};X=${SEMANTIC_EXTERNAL_ACTIONS.join('|')};V=${SEMANTIC_VERIFICATION_KINDS.join('|')};visual-check=>verification_cases{id,subject,required_browser_actions:A[],source_units:RU[]};RU=source_units|nonvisual_request_units;no visual=>cases/RU=[];${semantic.phase==='followup'?'resume/constraint empty cases+RU keeps prior; ':''}A=open|navigate|click|type|key|inspect|viewport|screenshot;reload=navigate+inspect;intent_signals=[];intent.<slug>:intent.tdd;signals unknown/capability-named reject`,
    'mutation_targets⊆likely_targets;write;exclude context/source-of-truth;scope/dependency=material units;unchanged tests excluded;change+verify!=sequential',
    'diagnosis/review=no-write;fix=>bug-fix+C:implementation;docs=>C:documentation;verify!=test-authoring;intent.debugging=>diagnosis+repository-analysis;independent-review=explicit/risk;interactive-process=persistent;X=>risk=authority-boundary',
    phaseRule,
  ].join('\n')
}
