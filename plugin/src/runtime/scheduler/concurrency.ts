...
export class ConcurrencyScheduler{
  ...
  canStart(id:string,provider?:string,model?:string):{ok:boolean;reason:string}{
    ...
    if(current&&current.provider===provider&&current.model===model)return{ok:true,reason:'already-acquired'}
    ...
    if(all.length+(current?1:0)>p.global)return{ok:false,reason:'global-capacity'}
    if(!current&&all.length>=p.global)return{ok:false,reason:'global-capacity'}
    if(provider){const cap=p.providers?.[provider]??p.global;if(all.filter(x=>x.provider===provider).length>=cap)return{ok:false,reason:`provider-capacity:${provider}`}}
    if(model){const cap=p.models?.[model]??p.global;if(all.filter(x=>x.model===model).length>=cap)return{ok:false,reason:`model-capacity:${model}`}}
