...
import {executionBudgetView,type BudgetAxis} from '../economics/budget-view.js'
...
export interface ObservabilityEconomicsView{
  ...
  product_summary:{
    ...
    cache:{read:number;write:number;share_of_exact_context:number|null;claim_boundary:'observed-token-volume-not-cache-savings'}
    money:{provider_billed_exact_usd:number|null;opencode_derived_usd:number|null;display_basis:'provider-billed-exact'|'opencode-calculated-derived'|'unavailable';hard_budget_enforced:false}
    repeat_compute:{exact_tokens:number;exact_context_tokens:number;share_of_exact_tokens:number|null;top_causes:Array<{cause:string;exact_tokens:number;derived_opencode_cost_usd:number;provider_billed_cost_usd:number}>}
    budgets:{hard:{within:number;exhausted:number;unavailable:number};observed_only:number;unavailable_measurements:number;note:string}
    by_model_role:Array<{model:string;role:string;workers:number;attempts:number;exact_tokens:number;cache_read:number;cache_write:number;derived_opencode_cost_usd:number;provider_billed_exact_usd:number}>
...
function totalTokens(tokens:{input:number;output:number;reasoning:number;cache_read:number;cache_write:number}):number{return tokens.input+tokens.output+tokens.reasoning+tokens.cache_read+tokens.cache_write}
function exactProviderBilled(m:MissionState):number{
...
function allBudgetAxes(budget:ReturnType<typeof executionBudgetView>):BudgetAxis[]{return[...budget.mission,...Object.values(budget.workers).flat(),...Object.values(budget.processes).flat()]}
function modelRoleSummary(m:MissionState):ObservabilityEconomicsView['product_summary']['by_model_role']{
  const rows=new Map<string,ObservabilityEconomicsView['product_summary']['by_model_role'][number]>()
  for(const worker of m.execution.workers){
    ...
    existing.workers+=1;existing.attempts+=Math.max(0,worker.attempt??0);existing.exact_tokens+=totalTokens(usage);existing.cache_read+=usage.cache_read;existing.cache_write+=usage.cache_write;existing.derived_opencode_cost_usd=Number((existing.derived_opencode_cost_usd+workerDerivedOpenCodeCost(worker)).toFixed(12));existing.provider_billed_exact_usd=Number((existing.provider_billed_exact_usd+providerBilled).toFixed(12));rows.set(key,existing)
  }
...
export function observabilityEconomicsView(m:MissionState,now=Date.now()):ObservabilityEconomicsView{
  ...
  const productSummary:ObservabilityEconomicsView['product_summary']={
