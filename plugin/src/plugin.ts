...
export const HiPlugin:Plugin=async(ctx)=>{
  ...
  const browserHealth=await services.browserRuntime.health();browserAvailable=browserHealth.available;services.setBrowserAvailable(browserAvailable);refreshOwnedCapabilities()
