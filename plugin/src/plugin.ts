...
export const HiPlugin:Plugin=async(ctx)=>{
  ...
  const browserHealth=await services.browserRuntime.health();services.setBrowserAvailable(browserHealth.available);refreshOwnedCapabilities(browserHealth.available)
