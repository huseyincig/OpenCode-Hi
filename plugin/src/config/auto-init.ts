...
function normalizeRoleModels(input:Partial<Record<ModelRoutedChildRole,string[]>>):Record<string,string[]>{
  const out:Record<string,string[]>={}
  for(const role of MODEL_ROUTED_CHILD_ROLES){const ids=[...new Set(input[role]??[])].map(String).map(x=>x.trim()).filter(Boolean);if(ids.length)out[role]=ids}
  return out
}
...
export function setProjectRoleModels(projectRoot:string,role:ModelRoutedChildRole,models:string[]):{path:string;roleModels:Record<string,string[]>}{
  ...
  if(normalized.length)automaticRoles.delete(role);else automaticRoles.add(role)
  const next={...current,schema:1,type:'hi-routing',routing:{...routing,modelPolicy:'manual',roleModels,adaptiveRoles:[...automaticRoles].map(String).filter(x=>(MODEL_ROUTED_CHILD_ROLES as readonly string[]).includes(x))},applied_at:Date.now(),applied_by:'opencode-hi'}
  mkdirSync(dirname(path),{recursive:true})
  const tmp=`${path}.tmp-${Date.now()}-${Math.random().toString(16).slice(2)}`
  writeFileSync(tmp,JSON.stringify(next,null,2)+'\n','utf8');renameSync(tmp,path)
  return{path,roleModels:normalizeRoleModels(roleModels as Partial<Record<ModelRoutedChildRole,string[]>>)}
}
