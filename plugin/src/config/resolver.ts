...
export function resolveHiConfigWithReport(raw:unknown,projectRoot?:string):{config:HiConfig;report:ConfigResolutionReport}{
  ...
  const config:HiConfig={
    ...
    execution:{topology:['single-agent','multi-agent'].includes(String(execution.topology))?execution.topology as 'single-agent'|'multi-agent':'adaptive',maxAgents:bounded(execution.maxAgents,DEFAULT_HI_CONFIG.execution.maxAgents,1,8),parallelism:bounded(execution.parallelism,DEFAULT_HI_CONFIG.execution.parallelism,1,8),allowMultiRoleAgent:execution.allowMultiRoleAgent!==false},
