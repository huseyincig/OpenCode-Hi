...
import {adjudicateCompletion} from '../dist/runtime/completion/adjudicator.js'
...
function pkgFixture(){
  const root=mkdtempSync(join(tmpdir(),'hhc-publish-'));mkdirSync(join(root,'src'),{recursive:true});mkdirSync(join(root,'plugin'),{recursive:true})
  writeFileSync(join(root,'VERSION'),'2.0.10\n')
  writeFileSync(join(root,'package.json'),JSON.stringify({name:'opencode-hhc-orchestrator',version:'2.0.10',files:['src']}))
  writeFileSync(join(root,'package-lock.json'),JSON.stringify({name:'opencode-hhc-orchestrator',version:'2.0.10',packages:{'':{name:'opencode-hhc-orchestrator',version:'2.0.10'}}}))
  writeFileSync(join(root,'src','index.js'),'export const v=1\n')
  const m=new MissionStore(root).start('pkg','publish package')
  const pack=[{name:'opencode-hhc-orchestrator',version:'2.0.10,',integrity:I,shasum:S,filename:'opencode-hhc-orchestrator-2.0.10.tgz',files:[{path:'package.json'},{path:'src/index.js'}]}]
...
test('registry verification must match packed version and integrity before publish mission can complete',()=>{
  ...
  x.m.obligations.forEach(o=>o.status='closed');x.m.tasks=[];x.m.workers=[];x.m.evidence.fresh=true
  recordRemoteReleaseVerification(x.m,'npm view opencode-hhc-orchestrator@2.0.10 --json',{stdout:JSON.stringify({name:'opencode-hhc-orchestrator',version:'2.0.10',dist:{integrity:'sha512-wrong',shasum:S}}),metadata:{exit:0}},x.root)
  assert.equal(x.m.release_chain.package.remote_verified,false);assert.ok(x.m.blockers.includes('release-chain:package-remote-drift'))
  assert.ok(adjudicateCompletion(x.m).reasons.includes('release-chain:package-remote-unverified'))
  recordRemoteReleaseVerification(x.m,'npm view opencode-hhc-orchestrator@2.0.10 --json',{stdout:JSON.stringify({name:'opencode-hhc-orchestrator',version:'2.0.10',dist:{integrity:I,shasum:S}}),metadata:{exit:0}},x.root)
  assert.equal(x.m.release_chain.package.remote_verified,true);assert.ok(!x.m.blockers.includes('release-chain:package-remote-drift'))
  assert.ok(!adjudicateCompletion(x.m).reasons.includes('release-chain:package-remote-unverified'))
})

function releaseFixture(){
 const root=mkdtempSync(join(tmpdir(),'hhc-assets-'));mkdirSync(join(root,'plugin'),{recursive:true});mkdirSync(join(root,'dist'),{recursive:true})
 ...
 const asset=join(root,'dist','OpenCode-HHC-Orchestrator-2.0.10-DISTRIBUTABLE.zip'),bytes=Buffer.from('asset');writeFileSync(asset,bytes);writeFileSync(join(root,'dist','RELEASE-MANIFEST-2.0.10.json'),JSON.stringify(provenanceManifest(root,asset)))
 const m=new MissionStore(root).start('rel','create release');m.release_chain={push:{outcome:'success',at:Date.now(),command:'git push origin main',expected_remote:'origin',expected_ref:'refs/heads/main',local_head:H,observed_remote:'origin',observed_ref:'refs/heads/main',remote_hash:H,remote_verified:true}}
 return{root,m,asset}
}
...
test('release completion requires expected uploaded asset to appear in remote release asset list',()=>{
 ...
 assert.equal(x.m.release_chain.release.assets_verified,false);assert.equal(x.m.release_chain.release.remote_verified,false)
 const name=x.asset.split('/').pop();recordRemoteReleaseVerification(x.m,'gh release view v2.0.10 --json tagName,targetCommitish,assets',{stdout:JSON.stringify({tagName:'v2.0.10',targetCommitish:'main',assets:[{name,size:5}]}),metadata:{exit:0}},x.root)
 assert.equal(x.m.release_chain.release.assets_verified,true);assert.equal(x.m.release_chain.release.remote_verified,true)
})
