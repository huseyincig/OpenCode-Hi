...
test('child roles cannot claim obligations outside their authority class',async()=>{
  ...
  const reviewObligation=review.obligations.find(o=>o.kind==='review')
  ...
  const implementation=impl.obligations.find(o=>o.kind==='implementation')
  ...
  const authorityObligation=authority.obligations.find(o=>o.kind==='authority')
...
test('review obligation auto-binding is limited to reviewer roles, not every read-only role',async()=>{
  ...
  const task=m.tasks.find(t=>t.id===out.task_id)
  assert.equal(task.obligation_ids.some(id=>m.obligations.find(o=>o.id===id)?.kind==='review'),false)
})
