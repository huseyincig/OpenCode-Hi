...
test('compact semantic entry gate preserves closed contract and is at least 30 percent smaller',()=>{
  ...
  assert.match(gate,/Select intent\.debugging only when root-cause diagnosis is materially required/)
  assert.match(gate,/if diagnosis is required include repository-analysis in required_capabilities/)
