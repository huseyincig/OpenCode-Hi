...
function harness({permission,signal}={}){
  ...
  const client={v2:{pty}}
  const adapter=new OpenCodePtyAdapter(client,new URL('http://127.0.0.1:4096'),'/repo','/repo',()=>host(permission),url=>{const ws=new FakeSocket(url);sockets.push(ws);return ws},signal??(()=>{}),32,8)
