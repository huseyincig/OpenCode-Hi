...
test('bounded repository path identity normalizes relative Windows separators and preserves UTF-8 spaces and long names',()=>{
...
test('WorkerResult cannot persist absolute or traversal changed-file ownership',()=>{
...
test('project path normalization drops absolute paths outside repository instead of claiming VCS ownership',()=>{
...
test('native diff ignores unbounded host paths while retaining bounded repository paths',async()=>{
...
test('staged-set inspection fails closed on unbounded path and normal commit remains bounded to exact owned set',()=>{
...
test('path identity preserves case, spaces, Unicode and CRLF-separated staged names without conflation',()=>{
...
test('browser executable discovery uses env/platform cache roots and contains no host-user literal dependency',()=>{
...
test('runtime state location honors explicit then XDG then LOCALAPPDATA roots without project-root or hardcoded-home dependency',()=>{
...
test('verification command that may mutate files invalidates earlier Evidence before recording the new verification result',()=>{
...
test('unusable runtime state root fails visibly rather than silently claiming persistence',()=>{
