...
test('a read is review input only; direct review completion requires explicit parent progress evidence',async()=>{
  ...
  try{
    ...
    await hooks['chat.message']({sessionID:'s',message:{role:'user',parts:[{type:'text',text:'Review src/a.ts for correctness'}]}},{parts:[]})
...
test('command rollback with unknown exit cannot close rollback gate',async()=>{
  ...
  try{
    const hooks=await HiPlugin({directory:root,worktree:root,project:{},client:client()});await hooks.config({})
    await hooks['chat.message']({sessionID:'s-rb',message:{role:'user',parts:[{type:'text',text:'Fix a local bug'}]}},{parts:[]})
...
test('resolving one task issue does not clear the same blocker still owned by another task',()=>{
  ...
  m.workers.push({id:'w1',task_id:'t1',role:'coder',category:'standard',parent_session_id:'s',parent_mission_id:m.mission_id,fallbacks:[],loaded_skills:[],methodologies:[],fingerprint:'w1',status:'ready',generation_at_spawn:m.generation})
  m.workers.push({id:'w2',task_id:'t2',role:'coder',category:'standard',parent_session_id:'s',parent_mission_id:m.mission_id,fallbacks:[],loaded_skills:[],methodologies:[],fingerprint:'w2',status:'completed',generation_at_spawn:m.generation})
...
test('noncanonical external-effect commands are rejected before native permission patterns can be bypassed',async()=>{
  ...
  try{
    const hooks=await HiPlugin({directory:root,worktree:root,project:{},client:client()});await hooks.config({})
    await hooks['chat.message']({sessionID:'s-cmd',message:{role:'user',parts:[{type:'text',text:'Prepare a release push'}]}},{parts:[]})
...
test('parent direct progress cannot close implementation from an unrelated changed file',async()=>{
  ...
  try{
    ...
    await hooks['chat.message']({sessionID:'s-owned',message:{role:'user',parts:[{type:'text',text:'Update src/a.ts to add a greeting'}]}},{parts:[]})
...
test('parent direct ownership accepts an automatically related sibling test file',async()=>{
  ...
  try{
    const hooks=await HiPlugin({directory:root,worktree:root,project:{},client:client()});await hooks.config({})
    await hooks['chat.message']({sessionID:'s-related',message:{role:'user',parts:[{type:'text',text:'Update src/a.ts to add a greeting'}]}},{parts:[]})
...
test('parent direct ownership ignores an observed file that native current diff proves was reverted',async()=>{
  ...
  try{
    const hooks=await HiPlugin({directory:root,worktree:root,project:{},client:c});await hooks.config({})
    await hooks['chat.message']({sessionID:'s-reverted',message:{role:'user',parts:[{type:'text',text:'Update src/a.ts to add a greeting'}]}},{parts:[]})
...
test('path-unknown bash mutation cannot be used as direct implementation ownership proof',async()=>{
  ...
  try{
    const hooks=await HiPlugin({directory:root,worktree:root,project:{},client:client()});await hooks.config({})
    await hooks['chat.message']({sessionID:'s-unknown',message:{role:'user',parts:[{type:'text',text:'Update src/a.ts to add a greeting'}]}},{parts:[]})
...
test('direct progress cannot close a review obligation that requires an independent reviewer',async()=>{
  ...
  try{
    const hooks=await HiPlugin({directory:root,worktree:root,project:{},client:client()});await hooks.config({})
    await hooks['chat.message']({sessionID:'s-independent',message:{role:'user',parts:[{type:'text',text:'Perform an independent review of src/a.ts for correctness'}]}},{parts:[]})
...
test('worker result normalizes real-host success alias and evidence object',()=>{
  const result=parseWorkerResult(JSON.stringify({
    status:'success',
    summary:'Reviewed opencode.json read-only.',
    changed_files:[],
    scope_expansions:[],
    evidence:{schema_value:'https://opencode.ai/config.json',read_only_confirmed:true,files_modified:[]},
    open_issues:[],
    needs_context:false
  }))
  assert.equal(result.status,'DONE')
  assert.ok(result.evidence.length>=2)
  assert.ok(result.evidence.some(e=>e.kind==='schema_value'&&e.summary==='https://opencode.ai/config.json'))
})

test('worker result normalizes real-host success alias and string evidence list',()=>{
  const result=parseWorkerResult(JSON.stringify({
    status:'success',
    summary:'Reviewed opencode.json read-only.',
    changed_files:[],
    scope_expansions:[],
    evidence:['exact $schema value: https://opencode.ai/config.json','read-only confirmation: no files modified'],
    open_issues:[],
    needs_context:false
  }))
  assert.equal(result.status,'DONE')
  assert.equal(result.evidence.length,2)
  assert.ok(result.evidence.every(e=>e.kind==='worker'))
})
...
test('non-mutating bash can provide review input for a read-only review mission',async()=>{
  ...
  try{
    const hooks=await HiPlugin({directory:root,worktree:root,project:{},client:client()}); await hooks.config({})
    await hooks['chat.message']({sessionID:'s-bash-review',message:{role:'user',parts:[{type:'text',text:'Run pwd once and report the current directory. Do not modify files.'}]}},{parts:[]})
...
test('native skill content can provide review input for a read-only review mission',async()=>{
  ...
  try{
    const hooks=await HiPlugin({directory:root,worktree:root,project:{},client:client()}); await hooks.config({})
    await hooks['chat.message']({sessionID:'s-skill-review',message:{role:'user',parts:[{type:'text',text:'Load the hi-test-strategy skill and summarize it. Do not modify files.'}]}},{parts:[]})
