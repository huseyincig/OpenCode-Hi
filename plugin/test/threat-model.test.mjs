...
test('T05 contract ambiguity is represented as blocked repo-first precondition',()=>{const s=new MissionStore();const m=s.start('s','change authentication contract');assert.equal(m.intent.ambiguity,'contract-critical');assert.ok(evaluatePreconditions(m).items.some(x=>x.id==='gate-contract-ambiguity'&&x.status==='blocked'))})
