...
test('T03 explicit stop invalidates continuation generation',async()=>{const s=new MissionStore();const m=s.start('s','fix bug');const g=m.continuation.generation;s.stop('s');assert.ok(m.continuation.generation>g);let called=false;const ok=await dispatchContinuation(continuationPort({session:{promptAsync:async()=>{called=true}}}),m,'continue','test');assert.equal(ok,false);assert.equal(called,false)})
...
test('T09 automatic recommendation never invents fallback authority',()=>{const cfg={routing:{allowedProviders:[],deniedModels:[],roleModels:{},categoryModels:{},strategy:'cost-quality',maxFallbacks:2}};const r=resolveModel('standard',[{id:'a',quality:2},{id:'b',quality:1}],cfg);assert.equal(r.fallbacks.length,0);assert.equal(r.fallbackReasons.length,0);assert.ok(r.reason.includes('cost/quality/feedback are not routing authority'))})
