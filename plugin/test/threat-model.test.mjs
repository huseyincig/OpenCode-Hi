...
test('T01 generic continuation cannot approve privileged action',()=>{const s=new MissionStore();const m=s.start('s','deploy the release');try{requireAuthority(m,'npm publish')}catch{}assert.equal(approvePendingAuthority(m,'devam et'),false);assert.equal(m.status,'waiting-user')})
test('T02 exact approval remains bound to action hash',()=>{const s=new MissionStore();const m=s.start('s','deploy the release');try{requireAuthority(m,'npm publish','/tmp/a')}catch{}assert.ok(m.authority?.pending);assert.notEqual(actionContract('npm publish','/tmp/a').hash,actionContract('npm publish','/tmp/b').hash)})
test('T03 explicit stop invalidates continuation generation',async()=>{const s=new MissionStore();const m=s.start('s','fix bug');const g=m.generation;s.stop('s');assert.ok(m.generation>g);let called=false;const ok=await dispatchContinuation({session:{promptAsync:async()=>{called=true}}},m,'continue','test');assert.equal(ok,false);assert.equal(called,false)})
...
test('T05 contract ambiguity is represented as blocked repo-first precondition',()=>{const s=new MissionStore();const m=startAssessedMission(s,'s','opaque contract task',{ambiguity:'contract-critical'});assert.equal(m.intent.ambiguity,'contract-critical');assert.ok(evaluatePreconditions(m).items.some(x=>x.id==='gate-contract-ambiguity'&&x.status==='blocked'))})
test('T06 stale verification evidence does not satisfy completion policy',()=>{const s=new MissionStore();const m=s.start('s','fix bug');m.evidence.fresh=false;assert.equal(verificationSatisfied(m).ok,false)})
...
test('T10 stopped mission cannot be complete',()=>{const s=new MissionStore();const m=s.start('s','fix bug');s.stop('s');assert.equal(m.user_interrupted,true);assert.equal(m.status,'stopped')})
