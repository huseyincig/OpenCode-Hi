...
test('seeded property fuzz: canonical IDs accept valid syntax and reject malformed forms',async()=>{
...
test('seeded property fuzz: paths remain project-bounded',async()=>{
...
test('seeded property fuzz: strict schemas reject widening unknown and malformed permission profiles',async()=>{
...
test('seeded property fuzz: permission event ordering and duplicates never leave phantom pending authority',async()=>{
...
test('seeded property fuzz: malformed host observations normalize without unbounded output or fabricated permission decisions',async()=>{
...
test('seeded property fuzz: malformed config resolves to bounded canonical executable values',async()=>{
  await cases('config',(r)=>{const weird=pick(r,[NaN,Infinity,-Infinity,-999,0,1.7,999,'7',null,{},[]]);const raw={unknown:word(r),routing:{maxFallbacks:weird,strategy:pick(r,['cost','quality','cost-quality','bogus',42]),allowedProviders:[word(r),null,word(r)]},parallel:{max:weird,enabled:pick(r,[true,false,'true',0]),providers:{x:weird}},execution:{maxAgents:weird,parallelism:weird},teamMode:{maxMembers:weird,maxWallMinutes:weird},profile:{balanced:{specialistThreshold:pick(r,['low','medium','high','evil',42]),surprise:'nope'}}};const c=resolveHiConfig(raw);assert.ok(c.routing.maxFallbacks>=0&&c.routing.maxFallbacks<=6);assert.ok(c.parallel.max>=1&&c.parallel.max<=8);assert.ok(c.execution.maxAgents>=1&&c.execution.maxAgents<=8);assert.ok(c.execution.parallelism>=1&&c.execution.parallelism<=8);assert.equal('teamMode' in c,false);assert.equal('unknown' in c,false);assert.equal('surprise' in c.profile.balanced,false);return raw})
})

test('seeded property fuzz: decision payloads cannot authorize without exact structured identity',async()=>{
...
test('seeded property fuzz: arbitrary tool/worker output parsing is bounded and canonical',async()=>{
...
test('seeded property fuzz: persistence envelopes reject malformed schema and shape while valid round-trips survive',async()=>{
