...
test('cancellation wins over a late different worker result and terminal state cannot resurrect',async()=>{
...
test('simultaneous distinct task completions reconcile without double-closing or losing shared obligation state',async()=>{
...
test('evidence/mutation race ordering is deterministic: later mutation invalidates proof and later proof can become fresh',async()=>{
...
test('bounded task queue is FIFO among runnable workers and later entries are not starved',async()=>{
