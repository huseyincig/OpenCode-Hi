...
test('explicit task restart reuses the quarantined child session and only then unlocks its callbacks',async()=>{
  ...
  assert.equal(m.execution.workers[0].status,'busy')
  assert.equal(m.execution.workers[0].restart_reconcile_pending,false)
  assert.equal(m.execution.tasks[0].status,'running')
