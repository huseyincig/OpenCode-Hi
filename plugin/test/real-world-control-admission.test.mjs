...
test('non-empty invalid child role is rejected instead of silently falling back to coder',async()=>{
  ...
  try{const hooks=await HiPlugin({directory:root,worktree:root,project:{},client});await hooks.config({});const sid='rw-invalid-role';await hooks['chat.message']({sessionID:sid},{message:{role:'user'},parts:[{type:'text',text:'Build index.html and verify it visually'}]});const {assessPluginMission}=await import('./helpers/semantic.mjs');await assessPluginMission(hooks,sid,{task_kind:'implementation',scope:'local',risk:'low',ambiguity:'none',dependency_class:'independent',required_capabilities:['implementation','visual-qa'],likely_verification:['visual-check'],likely_targets:['index.html']});const out=String(await hooks.tool.hi_task_start.execute({input:{role:'verifier',objective:'verify index.html',scope:'index.html'}},{sessionID:sid}));assert.match(out,/Unsupported Hi child role 'verifier'/);const list=JSON.parse(await hooks.tool.hi_task_list.execute({},{sessionID:sid}));assert.deepEqual(list,[]);await hooks.dispose?.()}finally{rmSync(root,{recursive:true,force:true})}
})


test('direct path admits an explicit technical verification owner instead of skipping the reviewer worker',async()=>{
  const {mkdtempSync,rmSync,mkdirSync,writeFileSync}=await import('node:fs');const {tmpdir}=await import('node:os');const {join}=await import('node:path');const HiPlugin=(await import('../dist/plugin.js')).default
  const root=mkdtempSync(join(tmpdir(),'hi-rw-direct-tech-verifier-')),prompted=[],client={app:{log:async()=>{}},provider:{list:async()=>({data:{connected:['opencode-go'],all:[{id:'opencode-go',models:{'qwen3.8-flash':{id:'qwen3.8-flash'}}}]}})},session:{create:async()=>({data:{id:'child-direct-tech'}}),promptAsync:async x=>{prompted.push(x);return {data:{}}},diff:async()=>({data:[]}),abort:async()=>({data:true})}}
  try{
    mkdirSync(join(root,'src'),{recursive:true});mkdirSync(join(root,'test'),{recursive:true});writeFileSync(join(root,'src','a.js'),'export const a=1\n');writeFileSync(join(root,'test','a.test.mjs'),"import test from 'node:test'; test('a',()=>{})\n");writeFileSync(join(root,'package.json'),'{"scripts":{"test":"node --test"}}\n')
    const hooks=await HiPlugin({directory:root,worktree:root,project:{},client});await hooks.config({});const sid='rw-direct-tech-verifier';await hooks['chat.message']({sessionID:sid},{message:{role:'user'},parts:[{type:'text',text:'Fix src/a.js and verify it with the focused test.'}]});const {assessPluginMission}=await import('./helpers/semantic.mjs');await assessPluginMission(hooks,sid,{task_kind:'implementation',scope:'local',risk:'low',ambiguity:'none',dependency_class:'independent',required_capabilities:['implementation','verification'],likely_verification:['targeted-tests'],likely_targets:['src/a.js'],mutation_targets:['src/a.js']})
    await hooks['tool.execute.before']({sessionID:sid,tool:'edit'},{args:{filePath:'src/a.js'}});await hooks['tool.execute.after']({sessionID:sid,tool:'edit',args:{filePath:'src/a.js'}},{title:'edit',output:'done',metadata:{}})
    const before=JSON.parse(await hooks.tool.hi_ledger.execute({limit:50},{sessionID:sid})),impl=before.obligations.find(o=>o.kind==='implementation'),verify=before.obligations.find(o=>o.kind==='verification');assert.ok(impl);assert.ok(verify)
    const progress=JSON.parse(await hooks.tool.hi_direct_progress.execute({summary:'Applied bounded implementation mutation.',obligation_id:impl.id},{sessionID:sid}));assert.equal(progress.status,'RECORDED')
    const started=JSON.parse(await hooks.tool.hi_task_start.execute({objective:'Run the focused test and report exact output.',role:'qa-reviewer',scope:'src/a.js,test/a.test.mjs',required_evidence:'targeted-tests',obligation_ids:verify.id},{sessionID:sid}));assert.ok(started.task_id,JSON.stringify(started));assert.notEqual(started.status,'SKIPPED');assert.equal(prompted.length,1)
    assert.equal(started.control.action,'WAIT');assert.deepEqual(started.control.open_obligations.map(x=>x.id),[verify.id])
    await hooks.dispose?.()
  }finally{rmSync(root,{recursive:true,force:true})}
})
