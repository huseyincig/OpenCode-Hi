...
test('HumanDecisionContract is strict and duplicate open requests preserve one decision identity',()=>{
  ...
  const count=m.ledger.filter(e=>e.type==='user.action.required').length
  ...
  assert.equal(second.decision_id,first.decision_id);assert.equal(second.created_at,first.created_at);assert.equal(m.ledger.filter(e=>e.type==='user.action.required').length,count)
...
test('open operational HumanDecision blocks deterministic completion and is visible in bounded user status',()=>{
  ...
  m.obligations=[];m.evidence.fresh=true
...
test('generic semantic follow-up resolves non-authority HumanDecision but never exact authority request',()=>{
  ...
  store.beginFollowupSemanticAssessment(m.session_id,'I changed the provider configuration')
  assert.equal(m.human_decision.status,'RESOLVED')
  ...
  store2.beginFollowupSemanticAssessment(a.session_id,'continue')
  assert.equal(a.human_decision.status,'OPEN')
})
...
test('exact authority approval resolves the matching HumanDecision without treating generic continuation as approval',()=>{
  ...
  assert.equal(m.human_decision.semantic_type,'authority_request');assert.equal(m.human_decision.response_schema.protocol,'approve-exact-action');assert.equal(m.human_decision.status,'OPEN')
  assert.equal(approvePendingAuthority(m,'continue'),false);assert.equal(m.human_decision.status,'OPEN')
  assert.equal(approvePendingAuthority(m,'approve'),true);assert.equal(m.human_decision.status,'RESOLVED');assert.equal(m.human_decision.resolution,'authority-approved')
})
...
test('RuntimePersistence round-trip preserves canonical HumanDecision state and rejects malformed decision',()=>{
  ...
  try{
    ...
    const loaded=new RuntimePersistence(root).load();assert.equal(loaded.length,1);assert.equal(loaded[0].human_decision.decision_id,m.human_decision.decision_id);assert.equal(loaded[0].human_decision.status,'OPEN')
    const bad=structuredClone(m);bad.human_decision={...bad.human_decision,decision_id:'hd_'+ 'f'.repeat(20)}
...
test('temporary rollback USER_ACTION_REQUIRED is operational, not authority',()=>{
  ...
  m.temporary_mutations.push({id:'tm1',kind:'test',description:'temporary',rollback_command:'echo rollback',rollback_hash:'x',status:'active',created_at:1})
