...
import {methodologyCandidateDigest,methodologyCandidateID} from '../dist/runtime/project-intelligence/methodology-candidate.js'
...
test('M14 old READY methodology candidate is inert until fresh observation; history is not wall-clock expired',()=>{
  ...
  try{
    ...
    assert.equal(learning.all()[0].state,'READY');assert.equal(m.methodology.methodology_needs.some(x=>x.signal==='project.methodology-gap'),false,'loading historical derived candidate alone must not create active methodology work')
    const out=learning.observe(m,worker,observation,['targeted-tests']);assert.equal(out.state,'READY');assert.equal(out.observations.length,3);assert.equal(m.methodology.methodology_needs.some(x=>x.signal==='project.methodology-gap'),true,'fresh evidence-backed observation makes uncovered candidate actionable again')
