...
test('M14 old READY methodology candidate is inert until fresh observation; history is not wall-clock expired',()=>{
  ...
  try{
    ...
    const out=learning.observe(m,worker,observation,['targeted-tests']),fresh=methodologyCandidateAssessment(out);assert.equal(out.state,'READY');assert.equal(out.observations.length,3);assert.equal(out.learning?.positive,3);assert.equal(out.learning?.negative,0);assert.equal(out.learning?.alpha,4);assert.equal(out.learning?.beta,1);assert.equal(fresh.eligible,true);assert.ok(fresh.effective_confidence>=.7);assert.equal(m.methodology.methodology_needs.some(x=>x.signal==='project.methodology-gap'),true,'fresh evidence-backed observation restores confidence and makes uncovered candidate actionable again')
