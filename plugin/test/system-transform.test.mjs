...
test('pending semantic gate distinguishes material implementation scope from verifier-only files and sequencing', async () => {
  ...
  const text=output.system.join('\n')
  assert.match(text,/scope\/dependency=material units/)
  assert.match(text,/unchanged tests excluded/)
  assert.match(text,/change\+verify!=sequential/)
  assert.match(text,/task_kind=implementation\|bug-fix\|diagnosis\|review/)
  assert.match(text,/diagnosis\/review=no-write/);assert.match(text,/review=no-write/);assert.match(text,/fix=>bug-fix\+C:implementation/);assert.match(text,/docs=>C:documentation/);assert.match(text,/verify!=test-authoring/)
