...
test('C6 TaskRuntime consumes the generic adapter entrypoint and capability docs state the exact support boundary',()=>{
  ...
  const context=readFileSync(new URL('../../docs/CONTEXT.md',import.meta.url),'utf8')
  ...
  assert.match(context,/`typescript` and `typescriptreact`/)
  assert.match(context,/JavaScript, LSP-backed and Tree-sitter-backed semantic adapters are not implemented or advertised/)
})
