...
test('ecosystem integration view composes existing owners without copying secrets, executable paths or authority payloads',()=>{
  ...
  assert.deepEqual(view.compatibility_summary.capabilities,{supported:1,degraded:1,unsupported:1})
  assert.deepEqual(view.compatibility_summary.mcp,{configured:2,selected:1})
  assert.deepEqual(view.compatibility_summary.operational_tools,{observed:1,healthy:1,unhealthy:0})
  assert.equal(view.compatibility_summary.status,'ATTENTION')
  assert.deepEqual(view.compatibility_summary.attention,['degraded-capability:session-status','unsupported-capability:structured-human-decision-transport'])
  assert.equal(view.compatibility_summary.claim_boundary,'readiness-derived-from-current-projection-only')
...
test('ecosystem integration view is a projection, not an integration control plane',()=>{
  ...
  assert.equal(view.compatibility_summary.status,'READY')
  assert.deepEqual(view.compatibility_summary.attention,[])
})
