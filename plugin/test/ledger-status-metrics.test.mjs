...
test('ledger is bounded by event count and payload size while retaining critical mission lifecycle',()=>{
  ...
  assert.ok(m.ledger.some(e=>e.type==='mission.provisional'));assert.ok(m.ledger.some(e=>e.type==='semantic.assessed'))
...
test('economy metrics report minimum-team methodology and handoff signals without fabricating tokens',()=>{
  ...
  m.workers.push({id:'w1',task_id:'t1',role:'coder',category:'quick',parent_session_id:m.session_id,parent_mission_id:m.mission_id,fallbacks:[],selected_methodologies:[],methodologies:[],fingerprint:'f1',status:'completed'})
