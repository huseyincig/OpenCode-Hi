...
test('ledger is bounded by event count and payload size while retaining critical mission lifecycle',()=>{
  const s=new MissionStore();const m=s.start('ledger-bounded','fix src/a.ts bug test it')
  ...
  assert.equal(m.ledger.length,200)
  assert.ok(m.ledger.some(e=>e.type==='mission.started'))
  const sample=m.ledger.find(e=>e.type==='diagnostic.sample')
...
test('economy metrics report minimum-team skill and handoff signals without fabricating tokens',()=>{
  const s=new MissionStore();const m=s.start('metrics','fix src/a.ts typo')
  m.workers.push({id:'w1',task_id:'t1',role:'coder',category:'quick',parent_session_id:m.session_id,parent_mission_id:m.mission_id,fallbacks:[],loaded_skills:[],methodologies:[],fingerprint:'f1',status:'completed'})
  m.tasks.push({id:'t1',objective:'fix typo',status:'completed',role:'coder',category:'quick',scope:['src/a.ts'],constraints:[],dependencies:[],requiredEvidence:[],obligation_ids:[],context_artifacts:[],gate_ids:[],worker_id:'w1',created_at:Date.now(),updated_at:Date.now()})
  appendLedger(m,'worker.handoff',{task_id:'t1',worker_id:'w1',payload:{chars:420,skills:0,context_budget:8000,handoff_budget:12000}})
  appendLedger(m,'worker.resumed',{task_id:'t1',worker_id:'w1'})
  const row=missionMetrics(m)
  assert.equal(row.agents_spawned,1)
  assert.equal(row.zero_skill_workers,1)
  assert.equal(row.skills_loaded_total,0)
  assert.equal(row.average_handoff_chars,420)
  assert.equal(row.same_session_resumes,1)
  const agg=aggregateMissionMetrics([m])
  assert.equal(agg.zero_skill_workers,1)
