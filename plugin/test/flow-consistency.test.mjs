...
test('Flow-01 stopped mission is not implicitly resumed by a new task',()=>{
  ...
  const old=store.start('s1','bugı düzelt')
  store.stop('s1')
  const fresh=store.start('s1','README başlığını düzelt')
...
test('Flow-03 active follow-up widens the completion contract without invalidating generation',()=>{
  const store=new MissionStore()
  const m=store.start('s1','README yazım hatasını düzelt')
  const g=m.generation
  store.amend('s1','bir de CHANGELOG satırını güncelle')
...
test('Flow-04 security follow-up escalates risk and verification policy',()=>{
  const store=new MissionStore()
  const m=store.start('s1','README yazım hatasını düzelt')
  store.amend('s1','ayrıca auth permission davranışını değiştir')
...
test('Flow-05 permission wait is WAIT rather than stagnation recovery',()=>{
  const store=new MissionStore(); const m=store.start('s1','bugı düzelt')
...
test('Flow-07 mutation after evidence prevents deterministic completion',()=>{
  const store=new MissionStore(); const m=store.start('s1','README yazım hatasını düzelt')
  ...
  assert.equal(adjudicateCompletion(m).complete,false)
})
