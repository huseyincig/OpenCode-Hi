...
test('primary mode config defaults to auto and preserves explicit manager/working-manager overrides',()=>{
  assert.equal(resolveHhcConfig({}).primaryMode,'auto')
  assert.equal(resolveHhcConfig({primaryMode:'manager'}).primaryMode,'manager')
  assert.equal(resolveHhcConfig({primaryMode:'working-manager'}).primaryMode,'working-manager')
  assert.equal(resolveHhcConfig({primaryMode:'invalid'}).primaryMode,'auto')
})
