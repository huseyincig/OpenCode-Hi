...
test('runtime services resolve browser requirement through project-local operational-tool provisioning and durable smoke receipt',async()=>{
  ...
  let ready=false,bootstraps=0,healthCalls=0
  ...
  try{
    ...
    const first=await services.ensureBrowserAvailable();assert.equal(first.available,true);assert.equal(first.status,'provisioned');assert.equal(first.scope,'project-local');assert.equal(bootstraps,1);assert.equal(healthCalls,1,'operational-tool smoke is the single real health launch for one ensure')
    ...
    const second=await services.ensureBrowserAvailable();assert.equal(second.available,true);assert.equal(second.status,'cached');assert.equal(bootstraps,1);assert.equal(healthCalls,2,'each explicit demand-time ensure gets one fresh operational smoke, never a duplicate launch')
