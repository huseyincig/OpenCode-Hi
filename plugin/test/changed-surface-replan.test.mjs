...
function repo(){const root=mkdtempSync(join(tmpdir(),'hhc-replan-'));writeFileSync(join(root,'package.json'),JSON.stringify({scripts:{test:'vitest run',typecheck:'tsc --noEmit',build:'vite build'}}));mkdirSync(join(root,'src','auth'),{recursive:true});writeFileSync(join(root,'src/auth/token.ts'),'x');writeFileSync(join(root,'src/other.ts'),'x');writeFileSync(join(root,'src/extra.ts'),'x');return root}
function runtime(root){return new TaskRuntime({},new BackgroundRegistry(),new ConcurrencyScheduler(()=>({global:2,providers:{},models:{}})),root,root,()=>DEFAULT_HHC_CONFIG,()=>[],()=>({}))}

test('unexpected multi-file changed surface strengthens a local verification contract with one static check',()=>{
  const root=repo(),s=new MissionStore(),m=s.start('p','src/other.ts bugını düzelt test et')
...
test('follow-up reviewer defaults to actual changed surface and replanned required evidence',async()=>{
  const root=repo(),s=new MissionStore(root),m=s.start('p','src/other.ts bugını düzelt test et')
  ...
  const rt=new TaskRuntime(client,new BackgroundRegistry(),new ConcurrencyScheduler(()=>({global:2,providers:{},models:{}})),root,root,()=>DEFAULT_HHC_CONFIG,()=>[],()=>({}))
