...
test('runtime-scoped stores are created once by the application composition service',()=>{
  ...
  assert.match(services,/const scopedStores=createRuntimeScopedStores\(projectRoot,packageRoot,projectMemoryProvider\)/)
  assert.match(scoped,/contextArtifacts:new ContextArtifactStore\(projectRoot\)/)
  assert.match(scoped,/const projectIntelligence=new ProjectIntelligenceRuntime\(projectRoot,projectMemoryProvider\)/)
  assert.match(scoped,/taskOutcomeMemory:projectIntelligence\.taskOutcomeMemory/)
...
test('scoped factory returns stable store identities for one runtime instance',()=>{
  ...
  try{
    ...
    assert.strictEqual(stores.taskOutcomeMemory,stores.projectIntelligence.taskOutcomeMemory)
    assert.strictEqual(stores.projectIntelligence.methodologyLearning,stores.projectIntelligence.methodologyLearning)
