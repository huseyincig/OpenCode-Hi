...
test('dispatch revalidates provider policy and skips a provider denied after initial role resolution',async()=>{
  ...
  const cfg=resolveHhcConfig({routing:{roleModels:{coder:['p/live','q/other']}}})
...
test('runtime provider fallback revalidates current model policy before sending the fallback prompt',async()=>{
  ...
  cfg=resolveHhcConfig({routing:{allowedProviders:['q']}})
...
async function pluginScenario(observedModel,includeModelMetadata=true){
  const dir=mkdtempSync(join(tmpdir(),'hhc-effective-model-'))
  ...
  const hooks=await HhcPlugin({directory:dir,worktree:dir,project:{},client});await hooks.config({});await hooks['chat.message']({sessionID:'parent',message:{role:'user',parts:[{type:'text',text:'fix it'}]}},{parts:[]})
  const started=JSON.parse(await hooks.tool.hhc_task_start.execute({objective:'implementation',role:'coder',category:'standard',model:'p/expected'},{sessionID:'parent'}))
  await hooks.event({event:{type:'session.idle',properties:{sessionID:'child-model'}}})
  const state=JSON.parse(await hooks.tool.hhc_task_peek.execute({id:started.task_id},{sessionID:'parent'}))
  const ledger=JSON.parse(await hooks.tool.hhc_ledger.execute({limit:80},{sessionID:'parent'}))
