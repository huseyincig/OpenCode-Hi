...
test('real branch+annotated-tag push and hosted release HTTP transaction are remotely verified end to end',{skip:process.platform==='win32'?'POSIX executable shim is exercised by the mandatory Ubuntu release-readiness job':false},async()=>{
  ...
  try{
    ...
    const push='git push origin main';authorize(m,push,work);await before({sessionID:m.identity.session_id,tool:'bash',args:{command:push,cwd:work}},{args:{command:push,cwd:work}});const po=git(work,'push','origin','main');await after({sessionID:m.identity.session_id,tool:'bash',args:{command:push,cwd:work}},{stdout:po,metadata:{exit:0}})
    const head=git(work,'rev-parse','HEAD');await after({sessionID:m.identity.session_id,tool:'bash',args:{command:'git rev-parse HEAD',cwd:work}},{stdout:head+'\n',metadata:{exit:0}});const br=git(work,'ls-remote','origin','refs/heads/main');await after({sessionID:m.identity.session_id,tool:'bash',args:{command:'git ls-remote origin refs/heads/main',cwd:work}},{stdout:br+'\n',metadata:{exit:0}});assert.equal(m.release.release_chain?.push?.remote_verified,true)

    const tagCmd='git tag -a v2.0.10 -m v2.0.10';await before({sessionID:m.identity.session_id,tool:'bash',args:{command:tagCmd,cwd:work}},{args:{command:tagCmd,cwd:work}});git(work,'tag','-a','v2.0.10','-m','v2.0.10');await after({sessionID:m.identity.session_id,tool:'bash',args:{command:tagCmd,cwd:work}},{stdout:'',metadata:{exit:0}});assert.equal(m.release.release_chain?.push?.remote_verified,true,'local tag creation must not invalidate current revision push proof')
    const tagPush='git push origin v2.0.10';authorize(m,tagPush,work);await before({sessionID:m.identity.session_id,tool:'bash',args:{command:tagPush,cwd:work}},{args:{command:tagPush,cwd:work}});const tpo=git(work,'push','origin','v2.0.10');await after({sessionID:m.identity.session_id,tool:'bash',args:{command:tagPush,cwd:work}},{stdout:tpo,metadata:{exit:0}});assert.equal(m.release.release_chain?.push?.remote_verified,true,'tag push must not overwrite branch/current revision proof');assert.equal(m.release.release_chain?.tag_push?.expected_tag,'v2.0.10');assert.equal(m.release.release_chain?.tag_push?.remote_verified,false)
    const tagProbe=git(work,'ls-remote','origin','refs/tags/v2.0.10','refs/tags/v2.0.10^{}');await after({sessionID:m.identity.session_id,tool:'bash',args:{command:'git ls-remote origin refs/tags/v2.0.10 refs/tags/v2.0.10^{}',cwd:work}},{stdout:tagProbe+'\n',metadata:{exit:0}});assert.equal(m.release.release_chain?.tag_push?.remote_verified,true);assert.equal(m.release.release_chain?.tag_push?.peeled_tag_hash,head)

    const releaseCmd=`gh release create v2.0.10 --target main ${asset}`;authorize(m,releaseCmd,work);await before({sessionID:m.identity.session_id,tool:'bash',args:{command:releaseCmd,cwd:work}},{args:{command:releaseCmd,cwd:work}});const created=await runAsync(work,bin,['release','create','v2.0.10','--target','main',asset],env);await after({sessionID:m.identity.session_id,tool:'bash',args:{command:releaseCmd,cwd:work}},{stdout:created,metadata:{exit:0}});assert.equal(m.release.release_chain?.release?.remote_verified,false)
    const viewCmd='gh release view v2.0.10 --json tagName,targetCommitish,assets';const viewed=await runAsync(work,bin,['release','view','v2.0.10','--json','tagName,targetCommitish,assets'],env);await after({sessionID:m.identity.session_id,tool:'bash',args:{command:viewCmd,cwd:work}},{stdout:viewed,metadata:{exit:0}});assert.equal(m.release.release_chain?.release?.view_verified,true);assert.deepEqual(m.release.release_chain?.release?.observed_assets?.map(x=>x.name),['artifact.zip']);assert.equal(m.release.release_chain?.release?.assets_verified,true);assert.equal(m.release.release_chain?.release?.remote_verified,false,'hosted metadata alone cannot replace exact remote tag commit verification')
    await after({sessionID:m.identity.session_id,tool:'bash',args:{command:'git ls-remote origin refs/tags/v2.0.10 refs/tags/v2.0.10^{}',cwd:work}},{stdout:tagProbe+'\n',metadata:{exit:0}});assert.equal(m.release.release_chain?.release?.remote_verified,true)
    m.execution.obligations.forEach(o=>o.status='closed');m.execution.tasks.forEach(t=>t.status='completed');m.execution.evidence.fresh=true;const c=evaluateCompletion(m);assert.ok(!c.reasons.includes('release-chain:release-remote-unverified'))
