...
test('real branch+annotated-tag push and hosted release HTTP transaction are remotely verified end to end',{skip:process.platform==='win32'?'POSIX executable shim is exercised by the mandatory Ubuntu release-readiness job':false},async()=>{
  ...
  try{
    ...
    const push='git push origin main';await before({sessionID:m.identity.session_id,tool:'bash',args:{command:push,cwd:work}},{args:{command:push,cwd:work}});const po=git(work,'push','origin','main');await after({sessionID:m.identity.session_id,tool:'bash',args:{command:push,cwd:work}},{stdout:po,metadata:{exit:0}})
