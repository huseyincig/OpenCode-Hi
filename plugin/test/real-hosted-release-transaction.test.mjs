...
const base=new URL(process.env.HHC_RELEASE_BASE),a=process.argv.slice(2);
...
test('real branch+annotated-tag push and hosted release HTTP transaction are remotely verified end to end', async()=>{
  const root=mkdtempSync(join(tmpdir(),'hhc-hosted-release-')), remote=join(root,'remote.git'), work=join(root,'work'), bin=join(root,'gh')
  ...
  const port=await listen(server), env={HHC_RELEASE_BASE:`http://127.0.0.1:${port}`}
  try{
    ...
    m.obligations.forEach(o=>o.status='closed');m.tasks.forEach(t=>t.status='completed');m.evidence.fresh=true;const c=adjudicateCompletion(m);assert.ok(!c.reasons.includes('release-chain:release-remote-unverified'))
