...
test('PROMPT B callback resolver fails closed on ambiguous duplicate native session ownership',()=>{
  const registry=new BackgroundRegistry(),child=new ChildExecutionCoordinator({}, {}, registry)
...
test('PROMPT B reordered callback from superseded child session cannot bind the current worker',()=>{
  const registry=new BackgroundRegistry(),child=new ChildExecutionCoordinator({}, {}, registry),w=worker('w1','t1','old-session')
