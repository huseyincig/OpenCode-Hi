...
test('contributor adding config has one canonical option owner and generated projections',()=>{
...
test('contributor adding methodology edits canonical catalog rather than duplicate runtime lists',()=>{
...
test('contributor adding host adapter behavior has explicit port and OpenCode boundary',()=>{
...
test('contributor adding validation rule has one architecture lint execution owner and documented acceptance path',()=>{
 ...
 assert.match(lint,/HI022/);assert.match(doc,/npm run check:product/);assert.match(doc,/npm run check/);assert.match(rootPkg.scripts['check:product'],/plugin run check/);assert.match(rootPkg.scripts['check:python'],/validate\.py/);assert.match(rootPkg.scripts['check:python'],/test:python/);assert.equal(rootPkg.scripts.check,'npm run check:product && npm run check:python');assert.equal(rootPkg.scripts['architecture:lint'],'npm --prefix plugin run architecture:lint');assert.match(pluginPkg.scripts['architecture:lint'],/architecture_lint\.mjs/)
