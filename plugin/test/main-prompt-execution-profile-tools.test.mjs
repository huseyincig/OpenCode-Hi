...
test('zero-skill task gets a complete bounded execution profile and per-message tool minimization',async()=>{
  ...
  assert.ok(profile.tools.includes('edit'));assert.ok(profile.tools.includes('write'));assert.ok(!profile.tools.includes('bash'));assert.ok(!profile.tools.includes('skill'));assert.ok(!profile.tools.includes('task'))
  ...
  assert.equal(tools.skill,false);assert.equal(tools.task,false);assert.equal(tools.bash,false);assert.equal(tools.webfetch,false);assert.equal(tools.websearch,false)
  ...
  assert.match(JSON.stringify(prompts[0]),/host ask-gated tools are intentionally unavailable on this child surface: bash/i)
  assert.match(JSON.stringify(prompts[0]),/Do not wait or retry for them/i)
})
