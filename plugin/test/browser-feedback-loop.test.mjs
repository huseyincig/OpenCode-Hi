...
test('browser finding transfers source remediation to a writer before fresh visual re-verification',async()=>{
  ...
  const store=new MissionStore(repoRoot),m=store.start('m13-feedback','verify the local UI and fix any regression you find')
  store.applyInitialSemanticAssessment('m13-feedback',{material:true,message_kind:'mission',task_kind:'bug-fix',scope:'local',risk:'medium',ambiguity:'none',dependency_class:'independent',required_capabilities:['implementation','visual-qa'],requested_external_actions:[],likely_verification:['browser-evidence'],likely_targets:['src/ui.tsx'],mutation_targets:['src/ui.tsx'],intent_signals:['intent.browser'],suppressed_intent_signals:[]})
  const initialImplementation=m.execution.obligations.find(o=>o.id==='o-implementation');assert.ok(initialImplementation);initialImplementation.status='closed';initialImplementation.closedAt=Date.now()
