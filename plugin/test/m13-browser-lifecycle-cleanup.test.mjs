...
const context=(task,owner)=>({task_id:task,execution_owner_ref:owner,executor_version:'hi-playwright-browser@1'})

function fakePlaywright(){
  const sessions=[]
  const chromium={launch:async()=>{const page={_url:'about:blank',url(){return this._url},setDefaultTimeout(){},on(){},async goto(url){this._url=url},locator(){return{evaluate:async()=>({body:'ready',items:[]})}}};const browser={newContext:async()=>({newPage:async()=>page}),close:async()=>{browser.closed=true},closed:false};sessions.push({browser,page});return browser}}
...
test('M13 exact browser owner cleanup closes only the owned task session',async()=>{
  ...
  const first=context('t1','m:w1:s1:g1'),second=context('t2','m:w2:s2:g1')
...
test('M13 TaskRuntime cancel cleans the exact active visual worker browser owner',async()=>{
  ...
  const started=await runtime.start(m,{objective:'verify local browser',role:'visual-qa',category:'visual',scope:['src/view.tsx']})
  ...
  assert.deepEqual(cleanupCalls[0],{task_id:task.id,execution_owner_ref:`${m.identity.mission_id}:${worker.id}:child-browser:${worker.generation_at_spawn}`,executor_version:'hi-playwright-browser@1'})
