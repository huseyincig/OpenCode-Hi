...
const EXISTING_REPO_SCOPE='src/runtime/task/task-runtime.ts'
...
test('repository explorer handoff projects clearance result contract without polluting task verification evidence',async()=>{
  for(const [suffix,ambiguity,critical] of [['resolvable','resolvable',false],['critical','contract-critical',true]]){
    ...
    assess(store,sid,{task_kind:'bug-fix',scope:'local',ambiguity,required_capabilities:['repository-analysis','implementation'],likely_targets:[EXISTING_REPO_SCOPE],likely_verification:[]})
...
test('repository explorer with no mission ambiguity gets no clearance result contract',async()=>{
  ...
  assess(store,'explorer-no-clearance-handoff',{task_kind:'bug-fix',scope:'local',ambiguity:'none',required_capabilities:['repository-analysis','implementation'],likely_targets:[EXISTING_REPO_SCOPE],likely_verification:[]})
  const out=await runtime.start(m,{objective:'inspect known repository surface',role:'repository-explorer',scope:[EXISTING_REPO_SCOPE]})
...
test('review evidence is fenced from non-reviewer analysis and implementation handoffs without weakening explorer clearance',async()=>{
  for(const [suffix,role,kind] of [['analysis','repository-explorer','analysis'],['implementation','coder','implementation']]){
    ...
    assess(store,sid,{task_kind:'bug-fix',scope:'multi-file',ambiguity:'none',required_capabilities:['repository-analysis','implementation','verification'],likely_targets:[EXISTING_REPO_SCOPE],likely_verification:['review-evidence']})
    ...
    const out=await runtime.start(m,{objective:`bounded ${suffix}`,role,scope:[EXISTING_REPO_SCOPE],requiredEvidence:['review-evidence'],obligationIds:[obligation.id]})
...
test('distinct visual verification owner keeps mission visual evidence off analysis and implementation task handoffs',async()=>{
  ...
  assess(store,'distinct-visual-owner',{task_kind:'bug-fix',scope:'multi-file',ambiguity:'none',required_capabilities:['repository-analysis','implementation','verification','visual-qa'],likely_targets:[EXISTING_REPO_SCOPE],likely_verification:['visual-check']})
  ...
  const explorer=await runtime.start(m,{objective:'inspect dashboard',role:'repository-explorer',scope:[EXISTING_REPO_SCOPE],obligationIds:[analysis.id]})
  const coder=await runtime.start(m,{objective:'repair dashboard',role:'coder',scope:[EXISTING_REPO_SCOPE],obligationIds:[implementation.id]})
...
test('model task evidence cannot widen beyond canonical mission verification admission',async()=>{
  ...
  assess(store,'model-evidence-widening',{task_kind:'bug-fix',scope:'multi-file',ambiguity:'none',required_capabilities:['repository-analysis','implementation','verification','visual-qa'],likely_targets:[EXISTING_REPO_SCOPE],likely_verification:['visual-check']})
  ...
  const explorer=await runtime.start(m,{objective:'inspect static dashboard',role:'repository-explorer',scope:[EXISTING_REPO_SCOPE],requiredEvidence:['build'],obligationIds:[analysis.id]})
  const coder=await runtime.start(m,{objective:'repair static dashboard',role:'coder',scope:[EXISTING_REPO_SCOPE],requiredEvidence:['build','visual-check'],obligationIds:[implementation.id]})
  const explorerTask=m.execution.tasks.find(t=>t.id===explorer.task_id),coderTask=m.execution.tasks.find(t=>t.id===coder.task_id);assert.ok(explorerTask&&coderTask)
...
test('same-session corrective resume preserves the original execution tool surface and does not spawn a new child',async()=>{
  ...
  await assert.rejects(()=>runtime.resume(m,first.task_id),/Same-model corrective recovery is exhausted/)
  assert.equal(created.length,1);assert.equal(prompts.length,2,'repeated same failure must not create a level-2 equivalent prompt')
