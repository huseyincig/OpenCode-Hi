...
test('hi_task_start accepts OpenCode nested input shape without losing role category scope or obligation binding',async()=>{
  ...
  try{
    ...
    assert.equal(task.role,'coder');assert.equal(task.category,'quick');assert.deepEqual(task.scope,['src/a.ts']);assert.deepEqual(task.requiredEvidence,['changed-surface-sanity'],'fail-closed minimum verification remains bound through nested input normalization');assert.deepEqual(task.obligation_ids,['o-implementation'])
    await hooks.dispose?.()
  }finally{rmSync(root,{recursive:true,force:true})}
})
