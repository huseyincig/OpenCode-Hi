...
test('M13 browser finding feeds the same visual task/session correction loop and requires fresh attempt proof',async()=>{
  ...
  const runtime=new TaskRuntime(opencodeChildPort(c),registry,scheduler,repoRoot,repoRoot,()=>resolveHiConfig({},repoRoot),()=>[],()=>host,undefined,[],undefined,undefined,()=>new Set(['host-capability:browser-execution']),browser)
  ...
  const surface=createHiToolSurface({state:{config:resolveHiConfig({}),hostConfig:host,openCodeVersion:'1.18.18'},store,tasks:runtime,processRuntime:{},browserExecutor:browser,projectRoot:repoRoot,capabilities:{contracts:[]},native:{},getModels:()=>[],scopedStores:{contextArtifacts:{}}}).toolSurface
