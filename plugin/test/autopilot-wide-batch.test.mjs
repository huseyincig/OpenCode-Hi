...
import {evaluateIdle,shouldCountStagnation} from '../dist/runtime/autopilot/evaluator.js'
...
test('action-id guard prevents an old same-generation continuation from clearing a newer continuation',async()=>{
  ...
  store.amend('s','testleri de yap','verification')
...
test('plugin parent idle while permission/worker wait preserves stagnation_count=0',async()=>{
  const dir=mkdtempSync(join(tmpdir(),'hhc-idle-wait-'))
  const {client}=baseClient(['child-wait'])
  const hooks=await HhcPlugin({directory:dir,worktree:dir,project:{},client});await hooks.config({})
  ...
  let report=JSON.parse(await hooks.tool.hhc_ledger.execute({limit:80},{sessionID:'p1'}))
  ...
  await hooks.tool.hhc_task_start.execute({objective:'inspect bug',role:'repository-explorer',category:'quick'},{sessionID:'p1'})
  await hooks.event({event:{type:'session.idle',properties:{sessionID:'p1'}}})
  report=JSON.parse(await hooks.tool.hhc_ledger.execute({limit:120},{sessionID:'p1'}));last=[...report.events].reverse().find(e=>e.type==='runtime.decision')
...
test('failed child defers parent continuation while a sibling worker is still pending',async()=>{
  const dir=mkdtempSync(join(tmpdir(),'hhc-parent-wake-'))
  const {client,promptCalls}=baseClient(['child-a','child-b'])
  const hooks=await HhcPlugin({directory:dir,worktree:dir,project:{},client});await hooks.config({})
  await hooks['chat.message']({sessionID:'parent',message:{role:'user',parts:[{type:'text',text:'iki ayrı görev repo araştırması yap'}]}},{parts:[]})
  const a=JSON.parse(await hooks.tool.hhc_task_start.execute({objective:'inspect A',role:'repository-explorer',category:'quick',scope:'src/a.ts'},{sessionID:'parent'}))
  const b=JSON.parse(await hooks.tool.hhc_task_start.execute({objective:'inspect B',role:'repository-explorer',category:'quick',scope:'src/b.ts'},{sessionID:'parent'}))
  ...
  const ledger=JSON.parse(await hooks.tool.hhc_ledger.execute({limit:120},{sessionID:'parent'}))
  assert.ok(ledger.events.some(e=>e.type==='parent.wake.deferred'&&e.payload.reason==='sibling-workers-pending'))
  const rows=JSON.parse(await hooks.tool.hhc_task_list.execute({},{sessionID:'parent'}))
...
test('late parent-session events after explicit STOP are ignored and cannot recreate permission wait',async()=>{
  const dir=mkdtempSync(join(tmpdir(),'hhc-stop-late-'))
  ...
  const ledger=JSON.parse(await hooks.tool.hhc_ledger.execute({limit:100},{sessionID:'stop-parent'}))
...
test('continuation transport failures use a separate bounded runtime retry budget, not reasoning stagnation',async()=>{
  ...
  for(let i=1;i<=2;i++){
    ...
    assert.equal(m.iteration,0,'failed transport delivery does not consume reasoning/autonomy turn budget')
    assert.equal(m.stagnation_count,0)
...
test('child permission session-error becomes explicit user action and never parent reasoning recovery',async()=>{
  const dir=mkdtempSync(join(tmpdir(),'hhc-permission-failure-'))
  const {client,promptCalls}=baseClient(['child-perm'])
  const hooks=await HhcPlugin({directory:dir,worktree:dir,project:{},client});await hooks.config({})
  await hooks['chat.message']({sessionID:'parent-perm',message:{role:'user',parts:[{type:'text',text:'repo dosyasını incele'}]}},{parts:[]})
  const started=JSON.parse(await hooks.tool.hhc_task_start.execute({objective:'inspect repository',role:'repository-explorer',category:'quick'},{sessionID:'parent-perm'}))
  ...
  const rows=JSON.parse(await hooks.tool.hhc_task_list.execute({},{sessionID:'parent-perm'})),row=rows.find(x=>x.task.id===started.task_id)
  assert.equal(row.task.status,'failed')
  assert.ok(row.task.result.open_issues.some(x=>x.startsWith('permission-failure:')))
  const ledger=JSON.parse(await hooks.tool.hhc_ledger.execute({limit:120},{sessionID:'parent-perm'}))
