...
function runtime(){return new TaskRuntime(opencodeChildPort({}),new BackgroundRegistry(),createConcurrencyPolicySource(()=>({global:2,providers:{},models:{}})),ROOT,ROOT,()=>DEFAULT_HI_CONFIG,()=>[],()=>({}))}

test('threat matrix is closed, complete, stable-ID and tests-only verification metadata',()=>{
...
test('every deterministic control resolves to an existing executable proof test',()=>{
...
test('T3/T4 claims are bound to existing receipts and exact claim anchors',()=>{
...
test('duplicate child result callback is idempotently ignored by canonical TaskRuntime',()=>{
...
test('pinned threat/process/path reference revisions are recorded exactly',()=>{
