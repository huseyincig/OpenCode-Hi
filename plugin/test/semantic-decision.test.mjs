...
test('semantic entry gate preserves the complete closed contract',()=>{
  ...
  assert.match(gate,/nonvisual_request_units\[\]/);assert.match(gate,/source_units:RU\[\]/);assert.match(gate,/all RU classified/);assert.match(gate,/intent_signals=\[\] by default/);assert.match(gate,/intent\.<slug>/);assert.match(gate,/intent\.tdd/);assert.match(gate,/unknown signals reject/)
