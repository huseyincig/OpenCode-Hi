...
import { verificationCaseValidationError } from '../dist/contracts/verification-case.js'
...
test('semantic entry gate preserves the complete closed contract',()=>{
  ...
  assert.match(gate,/continuation_required=bool\(same-Mission-named-later-turn=>true;auto-resume\/reconnect=>false\)/)
  assert.match(gate,/scope\/dependency=material units/)
  assert.match(gate,/unchanged tests excluded/)
  assert.match(gate,/change\+verify!=sequential/)
  assert.match(gate,/task_kind=implementation\|bug-fix\|diagnosis\|review/);assert.match(gate,/diagnosis\/review=no-write/);assert.match(gate,/review=no-write/);assert.match(gate,/fix=>bug-fix\+C:implementation/);assert.match(gate,/docs=>C:documentation/);assert.match(gate,/verify!=test-authoring/);assert.match(gate,/intent\.debugging=>diagnosis\+repository-analysis/)
...
test('semantic assessment separates context relevance from exact mutation surface and requires write ownership',()=>{
  ...
  const gate=semanticEntryGate();assert.match(gate,/mutation_targets⊆likely_targets;write/);assert.match(gate,/exclude context\/source-of-truth/);assert.match(gate,/diagnosis\/review=no-write/);assert.match(gate,/fix=>bug-fix\+C:implementation/)
})
...
test('semantic visual-case input canonicalizes only unambiguous ID near-misses while strict validator remains strict',()=>{
  ...
  const raw={id:'vc_theme_persistence',subject:'theme persists after reload',required_browser_actions:['open','click','navigate','inspect']}
  assert.match(verificationCaseValidationError(raw)??'',/id must match .*lowercase kebab-case after vc_/)
  const assessment=parseSemanticIntentAssessment({...base,verification_cases:[raw,{...raw,id:'vc-mobile-layout',subject:'mobile layout'}]})
  assert.deepEqual(assessment.verification_cases.map(x=>x.id),['vc_theme-persistence','vc_mobile-layout'])
  assert.throws(()=>parseSemanticIntentAssessment({...base,verification_cases:[raw,{...raw,id:'vc_theme-persistence'}]}),/verification_cases ids must be unique/)
  const cases=[
  ...
  assert.deepEqual(parseSemanticIntentAssessment({...base,verification_cases:cases}).verification_cases,cases)
})
