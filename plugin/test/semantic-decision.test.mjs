...
test('semantic entry gate preserves the complete closed contract',()=>{
  ...
  assert.match(gate,/likely_verification=V\[\];verification_cases\[\];nonvisual_request_units\[\]/);assert.match(gate,/source_units:RU\[\]/);assert.match(gate,/all RU classified/);assert.doesNotMatch(gate,/\bvisual_request_units\b/);assert.match(gate,/intent_signals=\[\] by default/);assert.match(gate,/intent\.<slug>/);assert.match(gate,/intent\.tdd/);assert.match(gate,/unknown signals reject/)
