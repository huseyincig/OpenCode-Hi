...
test('C4 provider usage is exact only when it is actually observed and model-compatible',()=>{
...
test('C4 mismatched provider usage is not reused for a different model',()=>{
...
test('C4 model-independent heuristic is never labeled exact tokens',()=>{
...
test('C4 no-model fallback reports exact characters rather than fabricated tokens',()=>{
...
test('C4 exact host-observed characters remain exact characters',()=>{
...
test('C4 provider usage extractor ignores user messages and malformed/negative token observations',()=>{
  assert.equal(providerUsageObservation([{info:{role:'user',tokens:{input:100}}},{info:{role:'assistant',tokens:{input:-1}}}]),undefined)
})

test('C4 exact token confidence cannot originate from estimated or fallback paths',()=>{
