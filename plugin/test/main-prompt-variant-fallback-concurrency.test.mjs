...
test('selected native model variant must be evidenced by assistant runtime metadata',()=>{
  ...
  assert.equal(m.workers[0].effective_model_variant_verified,false)
  ...
  assert.equal(m.workers[0].effective_model_variant_verified,true)
  assert.ok(m.ledger.some(e=>e.type==='model.effective.verified'&&e.payload?.variant_verified===true))
})
...
test('role-specific children respect model capacity and second worker remains queued until slot releases',async()=>{
  ...
  const wb=m.workers.find(w=>w.id===b.worker_id)
