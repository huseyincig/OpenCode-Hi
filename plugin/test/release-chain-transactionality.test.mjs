...
test('successful push permits release, but a later local commit invalidates that push proof',async()=>{
 ...
 await before({sessionID:m.identity.session_id,tool:'bash',args:{command:'git commit -m "late fix"'}},{args:{command:'git commit -m "late fix"'}})
 await after({sessionID:m.identity.session_id,tool:'bash',args:{command:'git commit -m "late fix"'}},{stdout:'[main abc] late fix',metadata:{exit:0}})
 assert.equal(m.release.release_chain?.push,undefined)
 await assert.rejects(()=>before({sessionID:m.identity.session_id,tool:'bash',args:{command:'gh release create v2'}},{args:{command:'gh release create v2'}}),/push-not-proven-after-local-revision/i)
})
...
test('explicit create-release mission cannot complete after push alone',async()=>{
 ...
 const {m}=setup();m.execution.obligations.forEach(o=>{o.status='closed'});m.execution.evidence.fresh=true;m.release.release_chain={push:{outcome:'success',at:Date.now(),command:'git push origin main'}}
 ...
 m.release.release_chain.release={outcome:'success',at:Date.now(),command:'gh release create v1',remote_verified:true};const d=evaluateCompletion(m);assert.ok(!d.reasons.includes('release-chain:release-not-completed'))
})
