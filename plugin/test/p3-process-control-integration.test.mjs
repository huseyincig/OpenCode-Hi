...
function scoped(){return{contextArtifacts:{},projectIntelligence:{},skillCatalog:{}}}
...
test('P3 parent surface exposes bounded process controls and child overrides disable every process control',()=>{
  ...
  const {toolSurface}=createHiToolSurface({state:state(),store,tasks:{},teams:{},processRuntime,projectRoot:'/repo',capabilities:detectOpenCodeCapabilities({}),native:{},getModels:()=>[],scopedStores:scoped()})
...
test('P3 semantic user STOP stops mission before process cleanup then teams/workers reconcile',async()=>{
  ...
  const teams={shutdownMission:async()=>{order.push('team')}}
  ...
  assert.equal(m.identity.status,'stopped');assert.equal(m.continuation.user_interrupted,true);assert.deepEqual(order,['process','team','tasks']);assert.equal(result.reconciled_workers,2)
})
