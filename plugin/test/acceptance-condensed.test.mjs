...
test('E2: authority-boundary risk without an external action does not fabricate user authority', () => {
  ...
  assert.equal(m.execution.verification_policy.requireReview, true, 'security-review capability requires independent review')
  const review = m.execution.obligations.find(x => x.kind === 'review')
  assert.ok(review, 'every requireReview mission must materialize a canonical review obligation')
  assert.deepEqual(review.requiredEvidence, ['review-evidence'])
...
test('Flow-04b: authority-boundary follow-up without external action keeps authority gate closed', () => {
  ...
  assert.ok(m.execution.obligations.some(x => x.kind === 'review' && x.status === 'open' && x.requiredEvidence.includes('review-evidence')), 'follow-up requireReview must materialize an owned review claim')
