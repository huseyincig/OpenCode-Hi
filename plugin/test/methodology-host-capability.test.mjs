...
test('browser runtime resource is supported only after live health observation; T3 remains external',()=>{
  ...
  assert.match(cap?.native_primitive??'',/BrowserExecutor adapter.*healthy/i)
  assert.match(cap?.adapter_entrypoint??'',/BrowserExecutor port.*OpenCode adapter.*ownership\/evidence/i)
