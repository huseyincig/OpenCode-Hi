...
test('commit -a and direct pathspec commit cannot bypass ownership verification',()=>{
  const {m}=mission();recordPreexistingUserBaseline(m,{'notes/user.md':'baseline-user'});m.changed_files=['src/a.ts'];recordStagingInspection(m,'git diff --cached --name-only',{stdout:'src/a.ts\n'})
...
test('hook-level flow invalidates staged proof after index mutation and after commit',async()=>{
  const {store,m}=mission();m.changed_files=['src/a.ts']
  const before=createToolBeforeHook(store),after=createToolAfterHook(store)
  await after({sessionID:m.session_id,tool:'bash',args:{command:'git diff --cached --name-only'}},{stdout:'src/a.ts\n',metadata:{exit:0}})
  assert.ok(m.staging_safety)
  await before({sessionID:m.session_id,tool:'bash',args:{command:'git add src/a.ts'}},{args:{command:'git add src/a.ts'}})
  assert.equal(m.staging_safety,undefined)
  await after({sessionID:m.session_id,tool:'bash',args:{command:'git diff --cached --name-only'}},{stdout:'src/a.ts\n',metadata:{exit:0}})
  await before({sessionID:m.session_id,tool:'bash',args:{command:'git commit -m "fix"'}},{args:{command:'git commit -m "fix"'}})
  await after({sessionID:m.session_id,tool:'bash',args:{command:'git commit -m "fix"'}},{stdout:'[main abc] fix',metadata:{exit:0}})
  assert.equal(m.staging_safety,undefined)
})
...
test('merge/rebase mutation stales evidence and topology proof is one-shot',async()=>{
  const {store,m}=mission();m.evidence.fresh=true;m.evidence.items.push({id:'ev',kind:'targeted-tests',summary:'pass',scope:[],observed_at:Date.now(),pass:true,outcome:'passed'})
  const before=createToolBeforeHook(store),after=createToolAfterHook(store)
  await after({sessionID:m.session_id,tool:'bash',args:{command:'git status --porcelain'}},{stdout:'',metadata:{exit:0}})
  assert.ok(m.git_topology_safety?.clean)
  await before({sessionID:m.session_id,tool:'bash',args:{command:'git merge feature'}},{args:{command:'git merge feature'}})
  assert.equal(m.git_topology_safety,undefined)
  assert.equal(m.evidence.fresh,false)
  assert.ok(m.git_topology_pending)
})
...
test('merge conflict staged files become topology-owned and can be committed without absorbing unrelated files',async()=>{
  ...
  await after({sessionID:m.session_id,tool:'bash',args:{command:'git status --porcelain'}},{stdout:'',metadata:{exit:0}})
  await before({sessionID:m.session_id,tool:'bash',args:{command:'git merge feature'}},{args:{command:'git merge feature'}})
  await after({sessionID:m.session_id,tool:'bash',args:{command:'git merge feature'}},{stdout:'CONFLICT (content): Merge conflict in src/shared.ts',metadata:{exit:1}})
  assert.ok(m.blockers.some(x=>x.startsWith('git-topology-conflict:')))
  await after({sessionID:m.session_id,tool:'bash',args:{command:'git diff --cached --name-only'}},{stdout:'src/shared.ts\n',metadata:{exit:0}})
