...
- `data/validation/external-opencode-hi-0.1.0-browser-1.18.18-head-707609b.json`: exact-source negative B3 T3 receipt; BrowserExecutor exists and controlled acceptance is green, but the host has no browser CLI/engine runtime, so navigation/DOM/screenshot are not run and `browser-execution` remains `UNSUPPORTED`.
