...
Section 28 Release engineering is closed locally by `data/validation/prompt-b-release-engineering.json` with **13 stages, 0 violations**, status `CLOSED_LOCAL_T4_BLOCKED`. Current development source checkpoint is `a27463a0586bb65598eae9aaee0e066faa34fcc8` (tree `716b33c0a5431309eaf24a90eae9739fb3bd5c27`) and is explicitly distinct from the immutable historical GitHub `v0.1.0` released source `f1a2c1c4358e5a63656da7a585b6b5793d1ed3be`. Current packed consumer acceptance was rerun against the current source checkpoint on exact OpenCode 1.18.18 and passed with 31 Hi tools, agent projection and provider-independent session creation. Read-only external verification on 2026-08-15 found the GitHub `v0.1.0` release still present at the historical source, npm `opencode-hi@0.1.0` still absent (`E404`), and `npm whoami` currently authenticated as `huseyincig`. Authentication availability is not publication authority: no tag rewrite, GitHub release mutation or npm publish was attempted. Current dev source cannot truthfully reuse `v0.1.0`; T4 remains blocked pending explicit user authority plus a valid new release/version identity (or explicit bootstrap publication of the exact historical release artifact under separate authority).

Section 29 Documentation in every defect cycle is closed by `data/validation/prompt-b-documentation-defect-cycle.json`: **5/5 cycle steps PASS, 0 violations**. Material source changes are traced through product-truth owner hashes and proof paths, canonical documentation meanings remain one-owner, generated projections are machine-derived, documentation inventory rejects unclassified/duplicate/historical-current ownership drift, and parity lint rejects stale package/release/host claims and broken links. A code change does not require meaningless prose churn; it requires documentation-owner impact evaluation and updates where the truth surface is affected. Documentation never outranks live source/contracts/runtime evidence.

Section 30 Test suite audit is closed by `data/validation/prompt-b-test-suite-audit.json`: **11/11 invariants PASS, 0 violations**. Source checkpoint `5210a12a7b607e0c9048749fa74a4c8b801cd924` (tree `0fe616e8c21da30addb0a01807b0185fa0556c4e`) closes four material defects. Repository-root fixtures no longer depend on the caller cwd; the same methodology/coexistence group passes 17/17 from both plugin and repository-root cwd. The canonical Node harness now isolates Hi/XDG state/cache roots in a temporary suite sandbox, uses 120s per-test and 300s suite timeouts, and only normalizes the known Node 24.19.0 libuv SIGABRT when the exact assertion occurs after a terminal zero-failure/zero-cancel summary; measured real HOME Hi-state directory count remained 5301→5301 (delta 0). Most importantly, runtime HostCapability no longer self-promotes Process/Workspace/Browser to `REAL_HOST_ACCEPTANCE` from adapter presence or a mock client: active-host probes can yield only `SUPPORTED|UNSUPPORTED` at `OBSERVED`; T3 remains external exact-receipt truth. Because capability-truth owner/adapter bytes changed, Process, workspace isolation, and real-Chromium browser execution were freshly reaccepted against exact OpenCode 1.18.18 / Linux / aarch64 on `5210a12...`; compatibility now selects all three `head-5210a12` receipts and lifecycle audit is again **61/61 PASS**. Conditional Windows symlink/POSIX-hosted-release skips remain explicit and are covered by the mandatory Ubuntu+Windows release-readiness matrix rather than silently disappearing.


Section 34 Failure injection is closed by `data/validation/prompt-b-failure-injection.json`: **12/12 required injections PASS, 0 violations**, backed by `data/validation/failure-injection-0.1.0.json` on source `29d3024fb3640a97f244185a393eb133542fb735` (tree `e685a589dcd06e8a300421b84cbad8fedb616222`). Controlled failures cover provider timeout, model unavailable, rate limit, tool error, permission deny, process crash/orphan reconciliation, workspace cleanup failure, disk-write failure, corrupt persisted state, missing child-session capability, browser timeout/crash, and network transport failure. Recovery remains bounded: provider fallback iterates only configured candidates and ends in an explicit blocker, continuation transport reaches user action after three failures, permission denial is non-retryable, workspace/process failures quarantine ownership, corrupt state fails closed, browser waits are bounded, and no infinite retry is claimed or permitted.


Section 38 Cross-platform acceptance is closed truthfully by `data/validation/prompt-b-cross-platform-acceptance.json`: current Prompt B source is Linux-certified across canonical Node/Python/release-build paths and 7 portability surfaces. Windows remains a mandatory CI target with historical v0.1.0 Ubuntu+Windows evidence, but the current local/unpushed Prompt B source is explicitly **not** claimed Windows-certified without an exact-source Windows run.

Section 37 Developer journey acceptance is closed by `data/validation/prompt-b-developer-journey-acceptance.json`: **4/4 contributor journeys PASS, 0 violations**. Config, methodology, host-adapter behavior and validation-rule changes each resolve to an obvious canonical owner plus generated/validation projection path; contributors are not required to duplicate semantic lists or hand-edit generated truth.
