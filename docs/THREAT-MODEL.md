...
| `Q3-ADV-013` | `workspace-cleanup-loss` | IsolationDecision / WorkspaceLease contracts + HostCapability; W2 adapter pending | W1 owns strict decision/lease lifecycle metadata and W2 owns provisioning/binding/restart/cleanup adapters; support stays UNSUPPORTED until W3 exact-host proof demonstrates child execution and writes remain inside the lease while the main/user-dirty workspace is preserved. | T3 |
