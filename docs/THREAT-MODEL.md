...
| `Q3-ADV-012` | `process-orphan` | HostCapability contract / future ProcessContract owner | Current ordinary bash process lifecycle is truthfully DEGRADED; Hi does not fabricate PID/orphan cleanup guarantees until an owned executor exists. | T3 |
| `Q3-ADV-013` | `workspace-cleanup-loss` | HostCapability contract / future isolation owner | Workspace isolation remains UNSUPPORTED until Hi owns selection/provisioning/cleanup and proves child execution binding; no cleanup claim is fabricated. | T3 |
