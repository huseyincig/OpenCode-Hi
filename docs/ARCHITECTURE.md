...
Scheduler capacity has one current-running owner: durable Mission `SchedulerLifecycleState.reservations`. The pure `SchedulingPlanner` consumes those reservations plus a stateless global/provider/model capacity policy; TaskRuntime, recovery, conflict-resume and constraint-rebase paths do not maintain a second process-local allocation map. `ConcurrencyScheduler` remains only as a standalone compatibility/equivalence utility for tests/telemetry and is not instantiated by the production runtime. Host child/session execution still belongs to OpenCode; Hi scheduler reservations only fence semantic Task/Worker admission and exact attempts.
