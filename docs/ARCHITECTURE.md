...
P2 adds a host-independent `ProcessExecutor` port and wires `OpenCodePtyAdapter` as the OpenCode implementation. The adapter keeps raw PTY output outside `ProcessContract`, exposes only bounded cursor-window reads, binds host PID and command identity, preserves OpenCode `ask`/`deny` permission semantics fail-closed, and separates signal/exit from cleanup. This still does **not** complete the product lifecycle: WAIT/STOP/restart/orphan ownership and exact T3 receipts remain P3 work, so the reference-host `process-lifecycle` capability remains `DEGRADED` until that evidence closes.
