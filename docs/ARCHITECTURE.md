...
Campaign-C layers are cross-integrated by `superProductIntegrationView()` rather than a new durable Super Product state machine. The projection recomputes Project Intelligence ownership metadata, Model Intelligence, Collaboration, Autonomous Mission UX, Observability/Economics and Ecosystem from their canonical inputs; Behavioral Evaluation attaches only from an explicit receipt-driven evaluation episode and is never fabricated from live runtime state. The read-only `hi_super_product` application surface consumes this composition for an active Mission. It declares one owner map (`MissionStore`, `EvidenceRuntime/VerificationEnvelope`, `AuthorityContract`, `RoutingPolicy`, OpenCode native state, `Worker.usage_observations`, `ContextArtifactStore`, and narrow Project Intelligence stores), owns no persistence, and cannot approve, complete, route, execute or promote sibling derived projections into canonical truth.
