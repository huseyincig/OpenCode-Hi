...
`TaskRuntime` is the canonical Task application facade. Mechanical child execution, result reconciliation and recovery are delegated to bounded collaborators; none owns a second Task store. New repository-explorer scope is authority-bound like Mission targets: a model-proposed scope must be a current project-contained filesystem identity or an exact canonical Mission target. When repository ambiguity is unresolved and no target is canonical yet, wholly unbound model scope is normalized to the existing empty-scope discovery contract so current-attempt read receipts can resolve exact source scope; mixed/unbound scope otherwise fails closed. This is not a magic `workspace`/`project`/`root` alias, does not apply to writer future-file scope, and never rewrites an exact resumed Task. A model-facing cancel cannot retire a `FIX_REQUIRED`/`NEEDS_CONTEXT` Task while it still owns an open obligation: that exact Task must be reconciled instead, so cancellation cannot manufacture a fresh identity for an equivalent recovery strategy. Canonical user STOP/runtime-owned cancellation remains a separate lifecycle path.
...
Collaboration is likewise projection-only at the current boundary. `collaborationView()` derives the primary session, canonical child Workers, project scheduling conflict counts, an open canonical `HumanDecision` reference and existing ContextArtifact handles without persisting a second participant/task/approval/context store. Project peer Missions are coordination surfaces rather than fabricated participants. Chat/remote transport acknowledgements cannot become Authority, completion or Evidence merely by appearing in a collaboration surface; they must be admitted through the existing owning contract.

Autonomous Mission UX is another bounded derived surface. `autonomousMissionUxView()` combines the canonical Mission runtime projection and control-decision projection into user-facing status/action/blocker/verification/authority fields, while explicitly naming `hi_settings`, `HumanDecisionContract`, and `MissionStore/evaluateCompletion` as the owning control surfaces. Current-dev `hi_status` uses `formatAutonomousMissionStatus()` to preserve the compact compatibility first line while adding bounded Goal / Now / Control / Verification / Authority / Blockers lines from that same projection. The view is recomputed from durable Mission state, excludes ledger payload detail, and owns no UI cache, completion, approval, routing, or session-history truth.

Observability/Economics uses `observabilityEconomicsView()` as a bounded, non-persistent operator projection over canonical `Worker.usage_observations`, Mission ledger metrics, causal attribution and the existing execution-budget view. It exposes explicit complete/partial coverage, separates OpenCode-derived money from provider-billed provenance, bounds worker detail to a presentation tail without truncating canonical history, and carries `routing_authority=false` / `completion_authority=false`. Current-dev active-session `hi_metrics` also exposes `product_summary`: exact complete token/context/cache volume, exact provider-billed money when present (otherwise separately labeled OpenCode-derived money), causal repeat-compute shares/top causes, bounded role/model contribution, and counts of existing hard/observed/unavailable budget axes. Cache volume is not called cache savings, and partial/derived money is never promoted into a hard budget. Its aggregate compatibility view remains derived. No second raw telemetry/history store is introduced.

Execution/model usage has one normalized durable observation source: `Worker.usage_observations`. Accepted `ExecutionUsageObservation` records are deterministic-ID deduplicated and append-only across the Worker lifetime; metrics, budget and causal-attribution projections derive from that stream rather than maintaining another telemetry truth store or querying native history ad hoc. `modelIntelligenceView()` is a non-persistent derived projection over the current OpenCode-owned live model inventory, mission-derived model feedback, and those canonical Worker usage observations. It keeps complete step totals separate from partial assistant-message reports and preserves OpenCode-derived versus provider-billed monetary provenance. Latest Worker effective-model fields are current execution-state projection, while usage observation model identity is historical provenance. Model Intelligence remains advisory/evaluation-only: it never owns live inventory, routing preference, completion, Evidence, Authority or configuration, and missing/partial telemetry remains missing/partial instead of being fabricated into exact cost/quality claims. If future storage compaction is required, the canonical observation schema must expose truncation/coverage/drop/checkpoint semantics so partial history can never masquerade as complete history.
...
Current OpenCode invokes plugin `tool.execute.before` before its ShellTool performs native permission `ctx.ask`. Hi therefore retains exact permission-request command metadata by parent session/request identity. If native permission rejects the privileged command before execution, Hi clears only the matching pre-execution authority receipt, leaves the semantic obligation unresolved, records an operational HumanDecision, and grants no reusable retry approval. Unknown post-dispatch acknowledgement remains owned by the exact executing hash until structured reconciliation; deterministic failure/reject requires a new exact semantic contract before retry. Destructive, credential, secret and nested-carrier safety remains a separate monotonic boundary and cannot be weakened by either semantic approval or native permission.

## Recovery, continuation and reconciliation

Recovery has one semantic ownership chain. `evaluateIdle` / `recoveryPlan` and the durable recovery governor select the legal strategy from canonical Mission progress, blockers and prior recovery history. `taskRecoveryDirective` is the single typed projection for strategies that are allowed to mutate an existing Task/Worker (`same-worker-resume` and bounded `model-escalation`); `TaskRecoveryCoordinator` executes that projection only after exact native-session quiescence, scheduler reservation, workspace and model-policy checks. Parent-owned strategies such as narrowing work, changing the parent plan or authorizing a bounded fresh worker remain parent continuation actions and cannot be reinterpreted by an event handler as TaskRuntime mutation.

Result adoption is exact-owner fenced. Child terminal results, native permission denial, cached assistant results and restart reconciliation must match the current Mission/Task/Worker/session/attempt/generation lineage before they can settle state. Duplicate or stale observations are historical/no-op. Cancellation and failed recovery release reservations/resources only after native execution is proven terminal/quiescent; unavailable or ambiguous abort acknowledgement retains explicit ownership/blocker state rather than pretending cancellation succeeded. A reconnect, restart, repeated idle event or scheduled invocation is never fresh-attempt authority. New attempts/sessions require an explicit canonical recovery/fallback/rebase transition and the next exact scheduler reservation after prior-owner reconciliation.
...
The Playwright adapter treats connection liveness as part of that execution-owner contract, not as a best-effort browser detail. A retained task session is usable only while the exact execution owner still matches and the browser/page generation is live. Playwright `disconnected` events evict the stale session immediately; every retained-session action rechecks liveness before use and is consumer-bounded. A connection loss or uncertain operation timeout never auto-replays a stateful click/key/viewport against a fresh DOM generation: the stale generation is discarded or quarantined, and only an explicit `open`/`navigate` may close/relaunch and establish fresh observation references. This preserves fail-closed ownership and evidence freshness while preventing dead browser handles from becoming unbounded tool calls.
...
Ecosystem/integration state is a derived composition over existing owners, not a new plugin registry or host truth plane. `ecosystemIntegrationView()` projects current `HostCapabilityContract` rows, exact task-selected MCP server exposure and bounded `OperationalToolProvisioner` receipts through `createRuntimeServices().getEcosystemView()`. Current dev adds a bounded `compatibility_summary` with supported/degraded/unsupported capability counts, configured/selected MCP counts, observed/healthy/unhealthy operational-tool counts, and explicit attention rows derived only from those current observations. Missing projection truth is not replaced with an empty success view. The projection deliberately strips raw host configuration values, executable/receipt paths, provisioning authority payloads and smoke details; credentials remain OpenCode/provider-owned. Unsupported/degraded native capabilities remain explicit rather than being synthesized by an adapter.

The composition has no persistence, install/execute/approve/complete API and cannot become Mission/Task/Worker, Authority, Evidence, Context, routing or completion truth. Consequential external effects continue through `ExternalAction` + exact Authority/native-permission/idempotent reconciliation; transport acknowledgement stays observation-only until the owning contract reconciles it. MCP exposure remains native-config-authoritative and fail-closed, while project-local operational dependencies remain owned by `OperationalToolProvisioner` and its declared cache/receipt/lock storage contracts.
...
Execution topology and model selection are bounded structured decisions. Semantic execution decisions also expose advisory counterfactual stability: a bounded deterministic sensitivity analysis over valid one-step structured intent neighbors records which execution-path/topology/team/model-class axes would change. It is explicitly not a probability or confidence score and cannot alter routing, authority, verification, completion, or user constraints; external-action authority invariants are held fixed while perturbations are generated. User follow-up constraints are also represented as revision-bound structured Constraint/Decision atoms in Mission state. Each atom has USER authority, subject/predicate/polarity/scope and explicit supersession lineage. Opposite active atoms do not use keyword or recency heuristics: without an explicit supersedes reference they remain a fail-closed conflict. Mutation-deny path atoms are enforced before write-capable task dispatch and again against reconciled changed-file evidence; the existing same-task constraint rebase remains the lifecycle owner. Legacy prose constraints remain only for compatibility/prompt projection. Small/local work prefers the direct minimum-sufficient path; parallel or specialist execution requires explicit structured benefit/risk/capability signals. Parallel write admission is fail-closed on uncertain mutable ownership: known-disjoint writer scopes may share the primary checkout, overlapping writer scopes serialize, and two writers cannot fan out when either scope is unknown. The same pure mutable-surface policy also covers project-runtime peers and direct `working-manager` implementation: an active DIRECT/EVIDENCE parent is projected from canonical Mission intent/current changed-file state as a virtual write claim, so a foreign child cannot bypass it and a parent mutation cannot bypass an earlier/active peer. Exact parent tool paths narrow the claim when available; otherwise the semantic likely-target scope is used, and unknown mutable scope fails closed. Delegated child ownership remains Task/Worker-owned; after terminal delegation, any parent re-claim is ordered from the existing terminal lifecycle timestamp instead of jumping ahead of queued foreign work. This projection creates no lock, lease, queue, or second scheduler state. Read-only workers are exempt from the unknown-write fence. Isolation is therefore selective rather than blanket: a worktree is used only when the Task owns an explicit isolation decision, while ordinary safe disjoint work avoids worktree/merge overhead. OpenCode owns transient provider retry/backoff and live `busy`/`retry` state. Normal provider fallback is host-terminal-only; generic context/tool failure is not sufficient proof for model switching. One narrower host-observed compatibility case is recoverable: OpenCode native `json_schema` settlement requires `tool_choice=required`, while the provider inventory exposes only generic `toolcall` capability and cannot distinguish auto-only from required-capable tool-choice modes. If a terminal provider `APIError` explicitly reports that only `auto` is supported and required/named tool choices are unsupported, or explicitly reports that its thinking mode does not support this `tool_choice`, Hi may consume an already-authorized automatic recovery candidate as a fresh child; ordinary 4xx errors and generic tool incompatibility remain fail-closed, and explicit task-model authority still cannot escape to an automatic candidate. Fresh implementation admission remains fail-closed when prior repository exploration clearance is stale. Exact same-Task corrective resume is different: it is continuation of the already-authoritative Task/Worker/session and may proceed without spawning a conflicting refresher, but the corrective handoff must re-read/revalidate the current bounded source/diff before any further mutation; unresolved repository ambiguity still fails closed and mutation scope cannot widen. Behavioral recovery is a separate bounded path: recovery identity hashes semantic-gain state rather than activity-only worker attempt churn and also binds the normalized current failure to exact Task/Worker/model/generation identity. One bounded same-session/same-model correction is allowed for a failure; if that correction reproduces the same normalized failure without semantic gain, same-model correction authority is exhausted. Hi may then open one recovery-only model hazard and start one fresh child using a precomputed capability-ranked candidate that has not already appeared as the current model or anywhere in the Worker fallback history; behavioral recovery therefore cannot bounce back through a provider-failed or otherwise previously attempted model. If no unattempted candidate is authorized, the runtime fails closed to a mechanically different parent strategy rather than manufacturing a level-number-only retry. Recovery candidates are constrained by the same live inventory, strict allowlist, role capability policy and `routing.maxFallbacks`; they are not normal fallbacks, routing telemetry, or persisted user preference. Explicit task-model ownership cannot silently escape to an automatic candidate. Any mutating recovery prompt is never speculatively replayed; ambiguous acknowledgement that cannot be proven quiescent keeps the exact child/session/attempt reservation quarantined. Recovery never substitutes for Evidence or Authority. OpenCode child WorkerResult turns use the host-native `json_schema` output format with `retryCount=0` when the prompt primitive supports it. That host schema is a transport envelope rather than a second canonical semantic contract: settlement first revalidates the static transport shape, then runs the existing WorkerResult normalizer, then requires the normalized canonical contract before TaskResultReconciler applies task-specific evidence, verification-case, provenance, changed-file and methodology rules. Transport-valid optional claims that are noncanonical (for example invented visual case IDs) are dropped by normalization and therefore cannot gain PASS authority; malformed core envelopes remain resumable contract `FIX_REQUIRED`. `StructuredOutputError` likewise becomes resumable contract `FIX_REQUIRED`. Compatibility text parsing remains only a fail-closed adapter path and never weakens WorkerResult, provenance, Evidence, or model-identity validation. OpenCode message-history pagination remains an adapter detail rather than a `HostPort` semantic: assistant-result reconciliation asks the OpenCode adapter for only the newest message. This preserves native structured output while avoiding broad-history response encoding on host versions where a persisted `json_schema` user format is accepted by `prompt_async` but cannot be re-encoded through `session.messages`; Hi never retries by disabling the format or by widening back to poisoned history.
...
The product/integration composition is built by `superProductIntegrationView()` rather than a new durable Super Product state machine. The projection recomputes Project Intelligence ownership metadata, Model Intelligence, Collaboration, Autonomous Mission UX, Observability/Economics and Ecosystem from their canonical inputs; Behavioral Evaluation attaches only from an explicit receipt-driven evaluation episode and is never fabricated from live runtime state. The read-only `hi_super_product` application surface consumes this composition for an active Mission and presents it as bounded product/integration readiness rather than requiring internal campaign vocabulary. It declares one owner map (`MissionStore`, `EvidenceRuntime/VerificationEnvelope`, `AuthorityContract`, `RoutingPolicy`, OpenCode native state, `Worker.usage_observations`, `ContextArtifactStore`, and narrow Project Intelligence stores), owns no persistence, and cannot approve, complete, route, execute or promote sibling derived projections into canonical truth.
