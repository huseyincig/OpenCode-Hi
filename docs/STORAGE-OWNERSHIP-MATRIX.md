...
| Project routing policy | explicit setup/user | config resolver | routing/model policy | PROJECT | Hi project policy | canonical | durable | optional | no | explicit edit | `.opencode/hi/policy/routing.json` | Hi uninstall/user |
| Project authority projection | native always approval | permission adapter | authority grants | PROJECT | Hi authority | canonical policy | durable | **no recommended** | security-sensitive authority, no secret | native approval/change | `.opencode/hi/policy/authority.json` | Hi uninstall/user |
...
| Durable ArtifactStore | artifact subsystem | context/review consumers | long result + hash | MISSION/PROJECT | Hi artifacts | derived durable | lazy durable when project root supplied; otherwise memory | optional/project choice | payload-dependent; privacy boundary applies before retention | `sourceHash` invalidation | `.opencode/hi/artifacts/<semantic-kind>/<id>.json` when retained | ArtifactStore |
...
The complete 29-skill capability/output audit is recorded in `docs/SKILL-ARTIFACT-OWNERSHIP.md`. It confirms that no skill-specific Hi storage tree is needed: project-created skills use OpenCode-native `.opencode/skills/`, while retained knowledge/artifacts converge on the shared semantic owners above.
