...
| Process state | `ProcessRuntime` / `ProcessContract` | continuation/STOP/restart/cleanup | owned PID/status/identity/cleanup metadata; no raw stdout/stderr | MISSION/TASK/WORKER | Hi Mission execution slice | canonical runtime lifecycle | persisted in existing Mission runtime-state | no | operational; secret persistence forbidden | process lifecycle mutation/restart reconciliation | existing `runtime-state.json` Mission envelope | ProcessRuntime / Mission persistence |
...
| Worktree isolation | Hi `IsolationDecision` / `WorkspaceLease` / `WorkspaceRuntime` + `OpenCodeWorkspaceAdapter` | child execution / verification / restart cleanup | isolated workspace + exact lease identity | TASK | Hi Mission execution slice + OpenCode/Git mechanism | runtime capability | temporary native worktree; lease persisted in Mission runtime-state | WorkspaceLease metadata only; no second worktree DB | project code | task/mission cleanup/restart reconciliation | host/Git worktree location, never a Hi data mirror | WorkspaceRuntime / host adapter |
...
| PTY | process/session state | Process lifecycle metadata is retained only through the existing Mission runtime-state owner; raw PTY output is not copied into a second project/process database |
