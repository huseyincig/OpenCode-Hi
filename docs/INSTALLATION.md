...
For published `0.2.4`, mandatory Hi-owned local browser verification has a bounded first-use recovery path: if no usable Chromium executable is observed, Hi may invoke the package's pinned optional `playwright-core@1.62.1` CLI once to install Chromium into the Hi-owned platform cache (`HI_BROWSER_CACHE` when explicitly set, otherwise the platform cache root). A timeout, missing runtime package/CLI, network/install failure, or still-missing executable is recorded as unavailable environment/capability state and is not retried indefinitely on unchanged state.

Current `dev` keeps that bounded installer mechanic but places managed fallback Chromium behind the Tool Capability & Provisioning resolver. Its default cache is `<project>/.opencode/hi/tools/browser-execution/playwright-chromium/<version>`; this is Hi-owned operational-tool state, not an application dependency. Resolution prefers an existing implementation first, requires smoke verification before use, and never edits application `package.json`, lockfiles, or dependency directories.
...
After the published `0.2.4` baseline, current development adds unified settings while preserving OpenCode ownership boundaries. Use runtime `hi_settings` when live connected-model validation matters; in a current source checkout use `node scripts/opencode-hi.mjs config <project>` for deterministic project preference inspection/change. The immutable published `0.2.4` package does not expose this dev-only `config` command. Work Mode is `Adaptive`, `Single`, or `Multi`; it is separate from the primary Manager/Working Manager behavior and from advanced effort/profile policy. A runtime multi-field update is validated completely before `.opencode/hi/policy/routing.json` is replaced, so an unavailable model or non-vision `visual-qa` selection cannot leave a partially changed mode/role configuration. Successful runtime changes apply to new worker dispatches without restart. When no explicit settings file exists, the first pending chat session receives one bounded setup hint if live models are already available; a material task is never blocked by that hint and uses Adaptive + Automatic defaults. Opening `hi_settings` performs an explicit inventory refresh. Current `dev` first applies OpenCode's explicit `connected` provider membership to `/api/model`, then keeps the scoped list authoritative for model membership inside each connected provider; only a connected provider absent from that projection may be supplemented from OpenCode provider state. This lets newly connected/plugin-backed providers appear, disconnected providers disappear, and scoped model filtering remain intact without relying on a host event that OpenCode 1.18.21 does not expose.

The published `0.2.4` exact-host evidence below remains immutable and therefore still observes 34 tools; current `dev` additionally exposes `hi_settings`, `hi_browser_viewport`, and the read-only `hi_super_product` integration projection, for 37 `hi_*` tools in the current source checkout.
