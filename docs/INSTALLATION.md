...
For OpenCode `1.18.21`, Hi's runtime inventory comes only from OpenCode-owned runtime surfaces. Current `dev` reads the directory-scoped enabled `/api/model` projection and keeps it authoritative for every provider represented there; if OpenCode's provider state explicitly exposes a `connected` set, models for a connected provider missing entirely from the scoped projection may supplement the inventory. The supplement is provider-level only: it cannot resurrect a model filtered out of a provider already present in `/api/model`, and no supplement occurs when the host does not prove the provider is connected. If the supplement read fails, the valid scoped projection remains usable. Hi does not scrape the full models.dev catalog and does not fabricate an offline model list when host inventory is unavailable.
