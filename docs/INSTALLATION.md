...
Published `0.2.2` treats `install` as a `setup` alias. Development `0.2.3` gives `install` ensure semantics while keeping `setup` strict; `upgrade` remains an `update` alias.
