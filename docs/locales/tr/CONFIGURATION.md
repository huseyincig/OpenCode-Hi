...
Normal kullanıcı Work Mode olarak `Adaptive`, `Single` veya `Multi` seçer. Provider authentication ve primary session model ownership'i OpenCode'da kalır. `Single` tek-ajan topology olduğu için yeni missionlarda effective primary davranış `working-manager` olur; kayıtlı `manager` tercihi silinmez ve Single dışına çıkınca yeniden geçerli olur. Runtime `hi_settings` canlı effective connected inventory ile mode + limit + strict global child-model allowlist (`routing.allowedModels`) + birden fazla child-role model/fallback değişikliğini tek transaction içinde doğrular. Güncel adapter OpenCode explicit `connected` set verdiğinde önce directory `/api/model` provider membership'ini bu set ile kesiştirir. Connected kalan provider içinde scoped liste model membership otoritesidir; yalnız bu surface'te hiç bulunmayan ama OpenCode tarafından açıkça connected bildirilen provider modelleri tamamlanır. Böylece disconnected provider geri sızmaz, represented connected provider içinden filtrelenmiş model geri eklenmez, explicit connected kanıtı yokken inventory genişletilmez ve provider read başarısız olsa bile geçerli scoped liste korunur. `allowedModels` yalnız child eligibility'yi daraltır ve OpenCode inventory gerçeğini kopyalamaz. Automatic routing'de capability ve variant uyumu önce gelir; explicit liste sırası yalnız diğer ölçütlerde eşit kalan uygun modeller arasında deterministik tie-break olarak kullanılır. Güncel source checkout içinde `node scripts/opencode-hi.mjs config <project>` aynı proje tercihlerini CLI'dan yönetir; immutable published `0.2.4` paketi dev-only `config` komutunu içermez ve source CLI canlı provider inventory uydurmaz. `hi_role_models` compatibility surface olarak korunur; set/clear işlemleri aynı canonical settings mutation policy'sine delege edilir ve yeni akışlar `hi_settings` kullanır. Explicit routing dosyası olmaması hata değildir: Adaptive + Automatic varsayılanı geçerlidir. İlk pending sohbet, canlı model varsa setup yönlendirmesini bir kez alır; material iş kesilmez. Runtime settings açılışı inventory refresh ettiği için sonradan bağlanan provider yeni OpenCode oturumu gerektirmeden görünür.
...
Automatic routing ayrıca sınırlı sayıda capability-ranked **yalnız-recovery** adayını geçici olarak tutabilir. Bu liste normal fallback zinciri veya kullanıcı tercihi değildir ve sağlıklı execution sırasında kullanılmaz. Aynı Task/generation/model üzerindeki bir bounded corrective resume, semantic ilerleme olmadan aynı normalize edilmiş failure'ı tekrar üretirse aynı-model correction yetkisi tükenir ve recovery circuit bir fresh alternate child açabilir. Aday o anda OpenCode inventory, `routing.allowedModels`, provider policy, hard role capability ve scheduler capacity kurallarıyla yeniden doğrulanır. Explicit per-task model, explicit fallback yetkisi yoksa bu mekanizma üzerinden başka modele kaçamaz; yetkili recovery adayı yoksa Hi eşdeğer aynı-model prompt'u yeniden göndermek yerine mekanik olarak farklı bir parent stratejisi ister. Host destekliyorsa child WorkerResult'ları OpenCode native JSON-schema structured output kullanır; schema-output failure resumable `FIX_REQUIRED` olur, compatibility text ise fail-closed kalır ve prose'u evidence yerine geçirmez.

`visual-qa` için ayrıca host tarafından doğrulanmış image-input capability gerekir. Kalıcı role tercihi yalnız kullanıcı açıkça `hi_settings`, compatibility `hi_role_models` veya `roles/role-models --set` ile yazdığında oluşur.
