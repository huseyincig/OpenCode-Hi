...
Mutable current release status is generated into `data/validation/release-status-0.1.0.json` and the marker block in `docs/RELEASE.md`. This section preserves the immutable 0.1.0 acceptance history rather than acting as a second current-status owner.
...
The only remaining publication blocker is the npm registry channel: `npm whoami` returns `ENEEDAUTH`, and `opencode-hi@0.1.0` is not present in the registry. No npm publish has been attempted. R1 now provides a tokenless GitHub Actions Trusted Publishing workflow for future releases, but npm Trusted Publisher configuration requires the package to exist first and the workflow was added after immutable tag `v0.1.0`. Therefore the exact released `0.1.0` artifact must first be bootstrap-published under explicit registry authentication without retagging or substituting its source; registry version/integrity/shasum and fresh install must then be verified. After that package exists, configure its trusted publisher for `huseyincig/OpenCode-Hi` + `npm-publish.yml` + `npm publish`; subsequent release publication uses OIDC. The remaining npm T4 gate stays open until real registry proof exists.
