...
def main():
 ...
 release_identity = (f"- Published release: `{version}` (`v{version}`) — **{status}**." if status=='CERTIFIED_T4' else f"- Release candidate: `{version}` (`v{version}`) — **{status}**.")
 publication_identity = (f"- GitHub release: **{out['candidate']['github_status']}**; npm package: **{out['candidate']['npm_status']}**." if status=='CERTIFIED_T4' else f"- GitHub candidate: **{out['candidate']['github_status']}**; npm candidate: **{out['candidate']['npm_status']}**.")
 block='\n'.join([BEGIN,'## Current release status — generated','',release_identity,'- Historical `v0.1.0` remains immutable and is not retagged or source-substituted.',publication_identity,('- Publication verification is complete: GitHub Release and npm registry are both PASS_T4.' if status=='CERTIFIED_T4' else '- Publication authority is granted only after final engineering/certification completes; until real publication verification exists, T4 remains pending.'),f"- Reference host: OpenCode `{host.get('opencode_version')}` on `{host.get('platform')}/{host.get('architecture')}`; Hi-owned process/workspace/browser surfaces are exact-receipt `SUPPORTED_T3`.",'- Test counts are intentionally not persisted here; final certification owns fresh totals.',f'- Machine source: `{out_rel}`.',END])
