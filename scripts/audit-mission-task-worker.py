...
CHECKS=[
 ...
 ('stale-worker-result','plugin/src/runtime/task/task-result-reconciler.ts','worker.result.stale-generation-ignored','plugin/test/prompt-b-mission-task-worker-adversarial.test.mjs','PROMPT B callback disposition rejects stale mission generation and restart-pending worker before accepting current owner'),
 ...
 ('task-recovery','plugin/src/runtime/task/task-recovery-coordinator.ts','recoverRuntimeFailure','plugin/test/crash-restart-survival.test.mjs','explicit task restart quiesces and reconciles the durable reservation before the next same-session attempt'),
