...
def main():
    ...
    out={'schema':1,'release':(ROOT/'VERSION').read_text().strip(),'kind':'PRODUCT_TRUTH_TRACE_INVENTORY','status':status,'claim_boundary':'Derived documentation/reconstruction trace map only; listed runtime/contracts remain semantic owners.','areas':rows,'violations':{'missing_paths':missing}}
