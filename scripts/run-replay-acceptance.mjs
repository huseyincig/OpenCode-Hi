...
function completionMission(id){const store=new MissionStore(),m=store.start(id,'replay completion');const intent={objective:'replay completion',likelyTargets:[],taskKind:'implementation',scope:'local',risk:'low',ambiguity:'none',dependencyClass:'independent',requiredCapabilities:['implementation'],requestedExternalActions:[],likelyVerification:[],avoid:[]};store.applyInitialSemanticAssessment(id,assessment(intent));for(const o of m.execution.obligations)o.status='closed';m.execution.evidence.fresh=true;return m}
...
function executeReplay(){
 ...
 for(const row of corpus.completion){const m=completionMission(`replay-${row.id}`);if(row.scenario==='stopped'){m.identity.status='stopped';m.continuation.user_interrupted=true}else if(row.scenario==='active-worker'){m.execution.workers.push({id:'w-replay',task_id:'t-replay',role:'coder',category:'standard',parent_session_id:m.identity.session_id,parent_mission_id:m.identity.mission_id,fallbacks:[],selected_methodologies:[],loaded_methodologies:[],methodologies:[],fingerprint:'replay',status:'busy',attempt:0,generation_at_spawn:m.continuation.generation,updated_at:1})}else if(row.scenario==='verification-missing'){const v=m.execution.obligations.find(o=>o.kind==='verification');if(!v)throw new Error('verification obligation missing');v.status='open';v.requiredEvidence=['targeted-tests'];m.execution.verification_policy.requiredKinds=['targeted-tests'];m.execution.evidence.fresh=false}else if(row.scenario==='reconcile'){m.execution.tasks.push({id:'t-replay',dependencies:[],status:'waiting',result:{status:'FIX_REQUIRED'}})}else if(row.scenario!=='clean')throw new Error(`${row.id}: unknown completion scenario`);const c=evaluateCompletion(m);out.completion.push({id:row.id,actual:{complete:c.complete,next:c.next??null,reason_classes:reasonClasses(c.reasons)},expected:row.expected})}
