...
build=run(['npm','run','build']); node=run(['node','../scripts/run-node-test-suite.mjs'],ROOT/'plugin'); arch=run(['npm','run','architecture:lint']); docs=run(['npm','run','docs:check']); val=run([sys.executable,'scripts/validate.py'])
