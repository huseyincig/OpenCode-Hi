...
pm=re.search(r'(\d+) passed',both(py)); nt=re.findall(r'(?:ℹ\s*)?tests\s+(\d+)',both(node)); npass=re.findall(r'(?:ℹ\s*)?pass\s+(\d+)',both(node)); nfail=re.findall(r'(?:ℹ\s*)?fail\s+(\d+)',both(node)); nm=(nt[-1],npass[-1],nfail[-1]) if nt and npass and nfail else None; am=re.search(r'ARCHITECTURE LINT PASS rules=(\d+)',both(arch)); dm=re.search(r'documentation parity PASS: docs=(\d+) violations=(\d+)',both(docs))
checks={'python':py.returncode==0 and bool(pm),'node':node.returncode==0 and bool(nm) and nm[0]==nm[1] and nm[2]=='0','architecture':arch.returncode==0 and bool(am),'docs':docs.returncode==0 and bool(dm) and dm.group(2)=='0'}
