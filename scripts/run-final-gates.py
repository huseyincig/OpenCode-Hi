...
build=run(['npm','run','build']); node=run(['node','../scripts/run-node-test-suite.mjs'],ROOT/'plugin'); arch=run(['npm','run','architecture:lint']); docs=run(['npm','run','docs:check']); val=run(['/home/node/.local/share/hi-validation-tools/pyyaml/bin/python','scripts/validate.py'])
