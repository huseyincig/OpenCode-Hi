...
version=(ROOT/'VERSION').read_text().strip();fresh=json.loads((ROOT/'data/validation/fresh-consumer-opencode-1.18.18.json').read_text());fresh_host=fresh.get('host') or {};registry=fresh_host.get('registry_observation') or {}
...
data={'schema':1,'kind':'PROMPT_B_EXACT_CURRENT_OPENCODE_T3_AUDIT','program':'PROMPT_B','section':39,'status':status,
 ...
 'claim_boundary':f'Current candidate certification requires a fresh packed {version} artifact loaded by exact OpenCode 1.18.18 plus exact real-host T3 capability receipts whose complete capability-relevant runtime owner hashes are byte-identical to current source. Registry latest observations are informational and do not silently change the pinned certified host. Historical receipt prose or API presence alone is insufficient; receipt source commits remain explicit and are never relabeled as current.'}
