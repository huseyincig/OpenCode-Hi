...
expected_commit='aa9a402f04fc40d53823c3e1b5e0190362dc5e75'
...
data={'schema':1,'kind':'PROMPT_B_EXACT_CURRENT_OPENCODE_T3_AUDIT','program':'PROMPT_B','section':39,'status':status,'current_version_observation':{'installed':'1.18.18','npm_registry_latest':'1.18.18','sdk_registry_latest':'1.18.18','locked_sdk':'1.18.18','captured_at':'2026-08-16'},'exact_source_commit':expected_commit,'exact_source_tree':'50cd33f1454193685e92dab2fd5dd339aef6b902','compatibility_projection':'data/validation/compatibility-matrix-0.1.0.json','compatibility_sha256':sha('data/validation/compatibility-matrix-0.1.0.json'),'lifecycle_audit':'data/validation/prompt-b-process-workspace-browser-lifecycle.json','lifecycle_sha256':sha('data/validation/prompt-b-process-workspace-browser-lifecycle.json'),'capabilities':{cap:{'receipt':caps[cap]['receipt'],'receipt_sha256':sha(caps[cap]['receipt']),'status':caps[cap]['status'],'tested_git_commit':caps[cap]['tested_git_commit']} for cap in ['process-lifecycle','workspace-isolation-binding','browser-execution']},'summary':{'required_capabilities':3,'exact_current_capabilities':3 if not violations else sum(1 for cap in ['process-lifecycle','workspace-isolation-binding','browser-execution'] if (caps.get(cap) or {}).get('status')=='SUPPORTED_T3' and (caps.get(cap) or {}).get('tested_git_commit')==expected_commit),'lifecycle_invariants':61,'violations':len(violations)},'violations':violations,'claim_boundary':'Section 39 requires fresh exact-current host invocation. API presence and historical receipts are insufficient; all three material Hi-owned capabilities were rerun on OpenCode 1.18.18 Linux/aarch64 against the exact section-39 source.'}
