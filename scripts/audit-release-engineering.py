...
fresh=load('data/validation/fresh-consumer-opencode-1.18.18.json');head=fresh.get('source',{}).get('commit');tree=fresh.get('source',{}).get('tree')
...
if t4:
 stages=[
  ...
  {'stage':'fresh-registry-install','status':'PASS_T4','detail':'fresh registry consumer loaded by exact OpenCode 1.18.18','proof':'data/validation/t4-registry-exact-host-0.1.1.json'},
 ...
 stages=[
  ...
  {'stage':'tag','status':'PENDING_FINAL_AUTHORIZED_PUBLICATION','detail':f'historical v0.1.0 remains {hist_tag_commit}; current candidate tag is {current_tag}','proof':'user authority effective only after final certification'},
  {'stage':'github-release','status':'PENDING_FINAL_AUTHORIZED_PUBLICATION','detail':f'historical v0.1.0 remains immutable; {current_tag} publication waits for final certification','proof':'conditional user publication authority'},
...
if t4:
 ...
 checks.update({'registry_still_unpublished':True,'publish_not_attempted':True,'no_current_tag_rewrite':True})
