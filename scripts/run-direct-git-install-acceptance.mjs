...
if(!/^\d+\.\d+\.\d+$/.test(hostTarget))throw new Error(`runtime SDK dependency is not exact: ${hostTarget||'<missing>'}`)
if(pkg.peerDependencies?.['@opencode-ai/plugin']!==hostTarget||pkg.peerDependenciesMeta?.['@opencode-ai/plugin']?.optional!==true)throw new Error(`OpenCode host plugin peer is not optional or does not match exact ${hostTarget}`)
