...
accept=ROOT/'data/validation/cross-platform-acceptance-0.1.1.json';out=ROOT/'data/validation/prompt-b-cross-platform-acceptance.json'
...
row={'schema':1,'kind':'PROMPT_B_CROSS_PLATFORM_ACCEPTANCE_AUDIT','program':'PROMPT_B','section':38,'status':'PASS' if not viol else 'FAIL','acceptance_receipt':'data/validation/cross-platform-acceptance-0.1.1.json','acceptance_sha256':hashlib.sha256(accept.read_bytes()).hexdigest() if accept.exists() else None,'source_checkpoint':{'commit':source,'tree':tree},'github_actions':{'run_id':ga.get('run_id'),'ubuntu_job_id':u.get('job_id'),'windows_job_id':w.get('job_id'),'conclusion':ga.get('conclusion')},'summary':summary,'surfaces':rows,'linux_current_certified':not viol,'windows_current_certified':not viol,'windows_historical_release_evidence':True,'post_ci_material_drift':material_drift,'violations':viol,'claim_boundary':'Current-source Windows/Linux certification is bound to the exact external CI source checkpoint. Subsequent evidence-only commits may consume it only while runtime/package material paths remain byte-identical to that checkpoint.'}
