...
- **Models:** Automatic by default, an optional strict global child-model allowlist (`routing.allowedModels`), or explicit per-role primary/fallback choices from OpenCode's effective connected inventory. On current `dev`, the OpenCode adapter keeps directory-scoped `/api/model` authoritative for providers present there, while supplementing only providers absent from that surface when OpenCode explicitly reports them as `connected`; it never resurrects a filtered model from an already represented provider and never widens from an unproven provider catalog. The allowlist narrows child eligibility without replacing OpenCode runtime inventory truth; list order does not become Adaptive routing priority. Automatic routing also records bounded capability-ranked **recovery-only** candidates (never normal fallbacks or preferences): only after two same-model corrective attempts make no semantic gain may recovery open a fresh alternate-model child, still inside the allowlist/capability boundary.
...
- **Live refresh:** opening runtime settings refreshes OpenCode model inventory first. If a connected provider is absent from the directory model projection, the adapter can recover that provider from OpenCode's explicit connected-provider state without replacing scoped filtering for providers that are present. A newly connected provider can therefore appear without restarting Hi even though exact OpenCode 1.18.21 does not expose a dedicated provider/config-updated plugin event.
