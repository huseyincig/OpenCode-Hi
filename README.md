...
- exact attempt-level usage diagnostics with lifecycle-bound causal repeat/context attribution, plus a bounded user-facing economics summary for exact token/cache volume, monetary provenance, repeat-compute causes, role/model contribution and existing budget state, without turning cost/telemetry into routing authority;
...
- **Browser execution:** supported on the Hi-owned, runtime-health-gated browser surface. The immutable published `0.2.4` runtime performs at most one bounded bootstrap attempt through pinned `playwright-core@1.62.1` into its Hi-owned platform cache when Chromium is absent. Current `dev` routes the same requirement through the code-owned Tool Capability & Provisioning lifecycle; managed fallback Chromium now defaults to `.opencode/hi/tools/browser-execution/playwright-chromium/<version>` and remains operational-tool state, not application dependency state. Hi does not edit application package manifests, lockfiles, or dependency directories to obtain it. A failed/unavailable bootstrap becomes explicit environment/capability state; it does not self-feed verification continuations. Browser observations and screenshots are never automatically Evidence or PASS.
...
- **Models:** Automatic by default, an optional strict global child-model allowlist (`routing.allowedModels`), or explicit per-role primary/fallback choices from OpenCode's effective connected inventory. On current `dev`, an explicit OpenCode `connected` provider set first constrains provider membership. Within those connected providers, directory `/api/model` remains authoritative for model membership; a connected provider missing entirely from that projection can be supplemented from OpenCode provider state. This prevents a disconnected provider leaked by `/api/model` from bypassing OpenCode `enabled_providers`, while still never resurrecting a model filtered from an already represented connected provider. The allowlist narrows child eligibility without replacing OpenCode runtime inventory truth; capability and variant fit remain primary, while an explicit allowlist order is used only as the deterministic tie-break among otherwise equally ranked eligible models. Automatic routing also records bounded capability-ranked **recovery-only** candidates (never normal fallbacks or preferences): after one bounded same-model corrective attempt reproduces the same normalized task failure without semantic gain, recovery may open one fresh alternate-model child, still inside the allowlist/capability boundary; without an authorized candidate, the same model is not blindly retried again.
...
- **Live refresh:** opening runtime settings refreshes OpenCode model inventory first. Explicit connected-provider state constrains provider membership, then missing connected providers may be recovered without replacing scoped model filtering for providers that are present. A newly connected provider can therefore appear—and a provider disabled at the OpenCode server layer can disappear—without restarting Hi even though exact OpenCode 1.18.21 does not expose a dedicated provider/config-updated plugin event.
...
- **Visibility vs availability:** exact OpenCode 1.18.21 (and the currently fetched upstream source) stores App “Manage Models” show/hide choices in client-local persisted UI state and exposes no model-visibility HTTP/SDK contract. Hi therefore does not scrape or mirror that UI store. For a server-wide OpenCode model set use native provider `whitelist`/`blacklist`; for a Hi-only child pool use `routing.allowedModels`.

The immutable published `0.2.4` runtime evidence below remains **34 tools**. Current `dev` exposes **37 `hi_*` tools**: the development additions relative to that immutable evidence are `hi_settings`, `hi_browser_viewport`, and the read-only `hi_super_product` integration projection. This development-source count must not be back-written into the `0.2.4` release evidence. `hi_settings` is the current settings owner; `hi_role_models` is a compatibility adapter that preserves its older ID/response surface while delegating role-model mutation policy to the same canonical settings path. Current-dev `hi_super_product` presents one derived product/integration-readiness composition; its ecosystem section summarizes real supported/degraded/unsupported host capabilities, configured/selected MCP counts and observed operational-tool health. Missing ecosystem truth remains explicitly blocked, and the view creates no plugin registry, credential store, approval, routing, Evidence or completion owner.

Published `0.2.4` exposes **34 `hi_*` tools**. The main user-facing diagnostics are `hi_doctor`, `hi_status`, `hi_readiness`, `hi_metrics`, and `hi_ledger`. On current `dev`, `hi_status` keeps its compact compatibility first line and adds bounded Goal / Now / Control / Verification / Authority / Blockers lines derived from canonical Mission/control projections; it creates no UI state owner and does not surface ledger payloads. Current-dev `hi_metrics` adds a `product_summary` over the existing canonical usage/budget projection: exact complete token/context/cache volume, exact provider-billed money when available, separately labeled OpenCode-derived money otherwise, repeat-compute attribution, bounded role/model rows, and hard/observed/unavailable budget counts. It never promotes partial/derived money into a hard budget or routing/completion authority. The remaining tools are bounded control-plane primitives for task/worker dispatch, process execution, browser execution, context artifacts, temporary mutations, semantic assessment, and direct progress. The exact loaded tool IDs are host-verifiable through OpenCode's documented `/experimental/tool/ids` endpoint.
...
The normal-user wizard asks only for primary behavior (`auto` / `working-manager` / `manager`). Hi owns task topology, specialist selection, verification depth, and parallelism internally. On the current development runtime, after restart ask **“Hi ayarlarını göster”** in the OpenCode chat. Runtime `hi_settings` shows only effective connected models plus Work Mode and current child-role assignments, and applies explicit model/fallback changes through the single transactional settings owner; `visual-qa` still accepts only vision-capable models. `hi_role_models` remains compatibility-only for older callers and is not the recommended current-development onboarding path. Without an explicit Hi mapping, Hi uses the OpenCode agent model when one exists, otherwise an ephemeral capability/variant recommendation over the live inventory. Automatic choices are not written back as preferences and are not reranked by cost/quality/feedback telemetry. `manager` / `working-manager` primary model selection and provider authentication remain OpenCode-owned. Use `reconfigure` to reopen only the primary-mode question; use `--non-interactive` for automation. The package `roles` command remains a deterministic CLI fallback.
...
Hi-owned project state lives under `.opencode/hi/` according to explicit storage ownership. OpenCode-native plugin/skill directories remain OpenCode-owned. Durable state is current-schema only; restart reconciliation adopts exact owned resources or quarantines mismatches instead of inventing continuity. Accepted sessionless queued Task/Worker work also keeps its exact persisted identity across restart: Hi rebuilds only the bounded in-process dispatch cache after host resource reconciliation, preserves `waiting-user` pauses, and rechecks required browser capability before child creation. Semantic follow-up assessment also pauses accepted queued work in place until meaning is known; non-material/resume input keeps the same queued identity, while material changes remain fail-closed.
